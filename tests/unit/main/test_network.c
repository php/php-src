...
// Mocked clock_gettime
int __wrap_clock_gettime(clockid_t clockid, struct timespec *tp)
{
	function_called();
	struct timespec *now = mock_ptr_type(struct timespec *);
	if (now) {
...
// Signal handler stub: request a syscall restart so the EINTR retry loop
// continues.
static zend_signal_interrupt_result restart_on_signal(void)
{
	return ZEND_SIGNAL_RESTART;
}

// Signal handler stub: request interruption.
static zend_signal_interrupt_result interrupt_on_signal(void)
{
...
static void test_php_network_connect_socket_progress_success(void **state) {
	...
	// Mock clock_gettime when initializing the deadline
	expect_function_call(__wrap_clock_gettime);
	will_return(__wrap_clock_gettime, &now);
	will_return(__wrap_clock_gettime, 0);

	// Mock clock_gettime when computing the remaining timeout
	expect_function_call(__wrap_clock_gettime);
	will_return(__wrap_clock_gettime, &now);
	will_return(__wrap_clock_gettime, 0);
...
static void test_php_network_connect_socket_eintr_restart(void **state) {
	struct timeval timeout_tv = { .tv_sec = 2, .tv_usec = 500000 };
	struct timespec start_time = { .tv_sec = 1000, .tv_nsec = 0 };  // Initial time
	struct timespec retry_time = { .tv_sec = 1001, .tv_nsec = 200000000 };  // Time after EINTR
	php_socket_t sockfd = 12;
	int error_code = 0;

	zend_signal_interrupt_function = restart_on_signal;
	...
	// Mock clock_gettime when initializing the deadline
	expect_function_call(__wrap_clock_gettime);
	will_return(__wrap_clock_gettime, &start_time);
	will_return(__wrap_clock_gettime, 0);

	// Mock clock_gettime for the first remaining-timeout computation (same instant)
	expect_function_call(__wrap_clock_gettime);
	will_return(__wrap_clock_gettime, &start_time);
	will_return(__wrap_clock_gettime, 0);
	...
	// Mock clock_gettime after EINTR (time has advanced)
	expect_function_call(__wrap_clock_gettime);
	will_return(__wrap_clock_gettime, &retry_time);
	will_return(__wrap_clock_gettime, 0);
...
static void test_php_network_connect_socket_eintr_interrupt(void **state) {
	struct timeval timeout_tv = { .tv_sec = 3, .tv_usec = 500000 };
	struct timespec start_time = { .tv_sec = 1000, .tv_nsec = 300000000 };  // Initial time
	php_socket_t sockfd = 12;
	int error_code = 0;

	zend_signal_interrupt_function = interrupt_on_signal;
	...
	assert_int_equal(error_code, EINTR);
}
...
int main(void) {
	const struct CMUnitTest tests[] = {
		...
		cmocka_unit_test(test_php_network_connect_socket_eintr_restart),
		cmocka_unit_test(test_php_network_connect_socket_eintr_interrupt),
