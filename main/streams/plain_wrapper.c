...
static int php_plain_files_mkdir(php_stream_wrapper *wrapper, const char *dir, int mode, int options, php_stream_context *context)
{
	...
	while (true) {
		...
		if (ret < 0 && errno != EEXIST) {
		    /* stop running and issue a warning to client if ret < 0 and errno != EEXIST */
...
<<<<<<< HEAD
		if (p == e || !replaced_slash) {
			...
			/* issue a warning to client if the last directory was created failed */
			if (ret < 0) {
			    if (options & REPORT_ERRORS) {
			        php_error_docref(NULL, E_WARNING, "%s", strerror(errno));
			    }
...
>>>>>>> 88fe4c8322 (modify if condition)
		}
	}
}
