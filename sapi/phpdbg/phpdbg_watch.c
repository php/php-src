...
static phpdbg_watchpoint_t *phpdbg_check_for_watchpoint(phpdbg_btree *tree, void *addr) {
	...
	phpdbg_btree_result *result = phpdbg_btree_find_closest(tree, ZEND_PTR_TO_ZEND_ULONG(phpdbg_get_page_boundary(addr)) + phpdbg_pagesize - 1);
...
#if ZEND_DEBUG
	phpdbg_btree_result *res = phpdbg_btree_find(&PHPDBG_G(watchpoint_tree), ZEND_PTR_TO_ZEND_ULONG(watch->addr.ptr));
	ZEND_ASSERT(res == NULL || res->ptr == watch);
#endif
	phpdbg_btree_insert(&PHPDBG_G(watchpoint_tree), ZEND_PTR_TO_ZEND_ULONG(watch->addr.ptr), watch);
}

static inline void phpdbg_remove_watchpoint_btree(phpdbg_watchpoint_t *watch) {
	phpdbg_btree_delete(&PHPDBG_G(watchpoint_tree), ZEND_PTR_TO_ZEND_ULONG(watch->addr.ptr));
}
...
static bool phpdbg_watch_element_collides(void *addr, phpdbg_watchtype type, phpdbg_watch_element *element) {
	...
	phpdbg_btree_result *res = phpdbg_btree_find(&PHPDBG_G(watchpoint_tree), ZEND_PTR_TO_ZEND_ULONG(addr));
...
phpdbg_watch_element *phpdbg_add_watch_element(phpdbg_watchpoint_t *watch, phpdbg_watch_element *element, bool *is_new) {
	...
	if ((res = phpdbg_btree_find(&PHPDBG_G(watchpoint_tree), ZEND_PTR_TO_ZEND_ULONG(watch->addr.ptr))) == NULL) {
...
void phpdbg_watch_parent_ht(phpdbg_watch_element *element) {
	if (element->watch->type == WATCH_ON_BUCKET || element->watch->type == WATCH_ON_ZVAL) {
		...
		if (!(res = phpdbg_btree_find(&PHPDBG_G(watch_HashTables), ZEND_PTR_TO_ZEND_ULONG(element->parent_container)))) {
			hti = emalloc(sizeof(*hti));
			hti->ht = element->parent_container;

			zend_hash_init(&hti->watches, 0, NULL, ZVAL_PTR_DTOR, 0);
			phpdbg_btree_insert(&PHPDBG_G(watch_HashTables), ZEND_PTR_TO_ZEND_ULONG(hti->ht), hti);
...
void phpdbg_unwatch_parent_ht(phpdbg_watch_element *element) {
	if (element->flags & PHPDBG_WATCH_HT_REGISTERED) {
		phpdbg_btree_result *res = phpdbg_btree_find(&PHPDBG_G(watch_HashTables), ZEND_PTR_TO_ZEND_ULONG(element->parent_container));
		ZEND_ASSERT(element->parent_container);
		element->flags &= ~PHPDBG_WATCH_HT_REGISTERED;
		if (res) {
			...
			if (zend_hash_num_elements(&hti->watches) == 1) {
				...
				phpdbg_btree_delete(&PHPDBG_G(watch_HashTables), ZEND_PTR_TO_ZEND_ULONG(hti->ht));
...
void phpdbg_check_watchpoint(phpdbg_watchpoint_t *watch) {
	...
	if (watch->type == WATCH_ON_HASHTABLE
		...
		ZEND_HASH_MAP_FOREACH_PTR(&watch->elements, element) {
			if (element->flags & PHPDBG_WATCH_RECURSIVE) {
				phpdbg_btree_result *res = phpdbg_btree_find(&PHPDBG_G(watch_HashTables), ZEND_PTR_TO_ZEND_ULONG(HT_WATCH_HT(watch)));
...
void phpdbg_reenable_memory_watches(void) {
	...
	ZEND_HASH_MAP_FOREACH_NUM_KEY(PHPDBG_G(watchlist_mem), page) {
		...
		if (res) {
			watch = res->ptr;
			if ((char *) ZEND_ULONG_TO_PTR(page) < (char *) watch->addr.ptr + watch->size) {
#ifdef HAVE_USERFAULTFD_WRITEFAULT
				if (PHPDBG_G(watch_userfaultfd)) {
					struct uffdio_writeprotect protect = {
						.mode = UFFDIO_WRITEPROTECT_MODE_WP,
...
#endif
				{
					mprotect(ZEND_ULONG_TO_PTR(page), phpdbg_pagesize, PROT_READ);
				}
			}
		}
	} ZEND_HASH_FOREACH_END();
...
void phpdbg_watch_efree(void *ptr ZEND_FILE_LINE_DC ZEND_FILE_LINE_ORIG_DC) {
	...
	if (zend_hash_num_elements(&PHPDBG_G(watch_elements))) {
		if ((result = phpdbg_btree_find(&PHPDBG_G(watchpoint_tree), ZEND_PTR_TO_ZEND_ULONG(ptr)))) {
		...
		if ((result = phpdbg_btree_find(&PHPDBG_G(watchpoint_tree), HT_WATCH_OFFSET + ZEND_PTR_TO_ZEND_ULONG(ptr)))) {
			phpdbg_watchpoint_t *watch = result->ptr;
			if (watch->type == WATCH_ON_HASHTABLE) {
				phpdbg_remove_watchpoint(watch);
			}
