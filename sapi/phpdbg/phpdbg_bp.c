...
PHPDBG_API int phpdbg_resolve_op_array_break(phpdbg_breakopline_t *brake, zend_op_array *op_array) /* {{{ */
{
	...
	opline_break.opline = brake->opline = ZEND_PTR_TO_ZEND_ULONG(op_array->opcodes + brake->opline_num);
...
PHPDBG_API void phpdbg_set_breakpoint_opline_ex(phpdbg_opline_ptr_t opline) /* {{{ */
{
	if (!zend_hash_index_exists(&PHPDBG_G(bp)[PHPDBG_BREAK_OPLINE], (zend_ulong)(uintptr_t) opline)) {
		...
		PHPDBG_BREAK_INIT(new_break, PHPDBG_BREAK_OPLINE);
		new_break.opline = ZEND_PTR_TO_ZEND_ULONG(opline);
		new_break.base = NULL;
		...
		phpdbg_error("Breakpoint exists for opline #"ZEND_ULONG_FMT, ZEND_PTR_TO_ZEND_ULONG(opline));
	}
} /* }}} */
...
static inline bool phpdbg_find_breakpoint_param(phpdbg_param_t *param, zend_execute_data *execute_data) /* {{{ */
{
	...
	switch (param->type) {
		...
		case ADDR_PARAM: {
			return (ZEND_PTR_TO_ZEND_ULONG((phpdbg_opline_ptr_t)execute_data->opline) == param->addr);
		} break;
