...
zend_result fpm_pctl_kill(pid_t pid, int how) /* {{{ */
{
	...
	return kill(pid, s) == 0 ? SUCCESS : FAILURE;
}
/* }}} */
...
zend_result fpm_pctl_init_main(void)
{
	...
	if (fpm_cleanup_add(FPM_CLEANUP_ALL, fpm_pctl_cleanup, 0) != SUCCESS) {
		return FAILURE;
	}
	return SUCCESS;
}
...
static void fpm_pctl_perform_idle_server_maintenance(struct timeval *now) /* {{{ */
{
	...
	for (wp = fpm_worker_all_pools; wp; wp = wp->next) {
		...
		if (wp->listen_address_domain == FPM_AF_INET) {
			if (fpm_socket_get_listening_queue(wp->listening_socket, &cur_lq, NULL) != SUCCESS) {
				cur_lq = 0;
#if 0
			} else {
				if (cur_lq > 0) {
