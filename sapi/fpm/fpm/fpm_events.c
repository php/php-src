...
static zend_result fpm_event_queue_add(struct fpm_event_queue_s **queue, struct fpm_event_s *ev);
static zend_result fpm_event_queue_del(struct fpm_event_queue_s **queue, struct fpm_event_s *ev);
	...
	do {
		...
		switch (c) {
			...
			case '1' :                  /* SIGUSR1 */
				...
				if (fpm_stdio_open_error_log(1) == SUCCESS) {
...
zend_result fpm_event_pre_init(char *mechanism) /* {{{ */
{
	/* kqueue */
...
{
	return module != NULL && module->support_edge_trigger;
}
...
zend_result fpm_event_init_main(void)
{
	...
	if (fpm_cleanup_add(FPM_CLEANUP_ALL, fpm_event_cleanup, NULL) != SUCCESS) {
		return FAILURE;
	}
	return SUCCESS;
}
...
zend_result fpm_event_add(struct fpm_event_s *ev, unsigned long int frequency) /* {{{ */
{
	...
	if (ev->flags & FPM_EV_READ) {
		...
		if (fpm_event_queue_add(&fpm_event_queue_fd, ev) != SUCCESS) {
			return FAILURE;
		}
		return SUCCESS;
	}
	...
	if (fpm_event_queue_add(&fpm_event_queue_timer, ev) != SUCCESS) {
		return FAILURE;
	}
...
{
	if (ev->index >= 0 && fpm_event_queue_del(&fpm_event_queue_fd, ev) != SUCCESS) {
		return FAILURE;
	}

	if (ev->index < 0 && fpm_event_queue_del(&fpm_event_queue_timer, ev) != SUCCESS) {
		return FAILURE;
	}
