...
// Constants that are substituted at compile time must not be substituted in write
// context, otherwise the property opcodes receive an IS_CONST container operand.
try {
    TRUE->prop = 1;
} catch (Error $e) {
    echo $e->getMessage(), "\n";
...
// Read context still substitutes constants at compile time.
