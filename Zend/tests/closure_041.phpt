...
$d();
echo "\n";
$d = $staticScoped->bindTo(null);
$d();
echo "\n";
$d = $nonstaticScoped->bindTo(null);
var_dump($d);
echo "\n";

echo "After binding, with same-class instance for the bound ones", "\n";
$d = $staticUnscoped->bindTo(new A());
$d = $nonstaticUnscoped->bindTo(new A());
$d();
echo " (should be scoped to dummy class)\n";
$d = $staticScoped->bindTo(new A());
$d = $nonstaticScoped->bindTo(new A());
$d();
echo "\n";

echo "After binding, with different instance for the bound ones", "\n";
$d = $nonstaticUnscoped->bindTo(new B());
$d();
echo " (should be scoped to dummy class)\n";
$d = $nonstaticScoped->bindTo(new B());
$d();
echo "\n";
