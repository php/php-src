...
void zend_exception_set_previous(zend_object *exception, zend_object *add_previous) /* {{{ */
{
	...
	ex = &zv;
	do {
		ancestor = zend_read_property_ex(i_get_exception_base(&pv), &pv, zend_known_strings[ZEND_STR_PREVIOUS], 1, &rv);
		while (Z_TYPE_P(ancestor) == IS_OBJECT) {
			if (Z_OBJ_P(ancestor) == Z_OBJ_P(ex)) {
				OBJ_RELEASE(add_previous);
				return;
			}
			ancestor = zend_read_property_ex(i_get_exception_base(ancestor), ancestor, zend_known_strings[ZEND_STR_PREVIOUS], 1, &rv);
		}
		base_ce = i_get_exception_base(ex);
		previous = zend_read_property_ex(base_ce, ex, zend_known_strings[ZEND_STR_PREVIOUS], 1, &rv);
		if (Z_TYPE_P(previous) == IS_NULL) {
			zend_update_property_ex(base_ce, ex, zend_known_strings[ZEND_STR_PREVIOUS], &pv);
...
static zend_object *zend_default_exception_new_ex(zend_class_entry *class_type, int skip_top_traces) /* {{{ */
{
	...
	if (EXPECTED(class_type != zend_ce_parse_error || !(filename = zend_get_compiled_filename()))) {
		...
		zend_update_property_ex(base_ce, &obj, zend_known_strings[ZEND_STR_FILE], &tmp);
		zval_ptr_dtor(&tmp);
		ZVAL_LONG(&tmp, zend_get_executed_lineno());
		zend_update_property_ex(base_ce, &obj, zend_known_strings[ZEND_STR_LINE], &tmp);
	} else {
	...
	zend_update_property_ex(base_ce, &obj, zend_known_strings[ZEND_STR_TRACE], &trace);
...
ZEND_METHOD(exception, __construct)
{
	...
	if (message) {
		...
		zend_update_property_ex(base_ce, object, zend_known_strings[ZEND_STR_MESSAGE], &tmp);
	}

	if (code) {
		ZVAL_LONG(&tmp, code);
		zend_update_property_ex(base_ce, object, zend_known_strings[ZEND_STR_CODE], &tmp);
	}

	if (previous) {
		zend_update_property_ex(base_ce, object, zend_known_strings[ZEND_STR_PREVIOUS], previous);
	}
}
/* }}} */
...
#define CHECK_EXC_TYPE(id, type) \
	pvalue = zend_read_property_ex(i_get_exception_base(object), (object), zend_known_strings[id], 1, &value); \
	if (Z_TYPE_P(pvalue) != IS_NULL && Z_TYPE_P(pvalue) != type) { \
		zend_unset_property(i_get_exception_base(object), object, ZSTR_VAL(zend_known_strings[id]), ZSTR_LEN(zend_known_strings[id])); \
	}
...
ZEND_METHOD(error_exception, __construct)
{
	...
	if (message) {
		...
		zend_update_property_ex(zend_ce_exception, object, zend_known_strings[ZEND_STR_MESSAGE], &tmp);
		zval_ptr_dtor(&tmp);
	}

	if (code) {
		ZVAL_LONG(&tmp, code);
		zend_update_property_ex(zend_ce_exception, object, zend_known_strings[ZEND_STR_CODE], &tmp);
	}

	if (previous) {
		zend_update_property_ex(zend_ce_exception, object, zend_known_strings[ZEND_STR_PREVIOUS], previous);
	}

	ZVAL_LONG(&tmp, severity);
	zend_update_property_ex(zend_ce_exception, object, zend_known_strings[ZEND_STR_SEVERITY], &tmp);

	if (argc >= 4) {
		ZVAL_STRING(&tmp, filename);
		zend_update_property_ex(zend_ce_exception, object, zend_known_strings[ZEND_STR_FILE], &tmp);
		zval_ptr_dtor(&tmp);
    	if (argc < 5) {
    	    lineno = 0; /* invalidate lineno */
    	}
		ZVAL_LONG(&tmp, lineno);
		zend_update_property_ex(zend_ce_exception, object, zend_known_strings[ZEND_STR_LINE], &tmp);
	}
}
/* }}} */
...
#define GET_PROPERTY(object, id) \
	zend_read_property_ex(i_get_exception_base(object), (object), zend_known_strings[id], 0, &rv)
#define GET_PROPERTY_SILENT(object, id) \
	zend_read_property_ex(i_get_exception_base(object), (object), zend_known_strings[id], 1, &rv)
...
static void _build_trace_string(smart_str *str, HashTable *ht, uint32_t num) /* {{{ */
{
	...
	file = zend_hash_find(ht, zend_known_strings[ZEND_STR_FILE]);
	if (file) {
		if (Z_TYPE_P(file) != IS_STRING) {
			...
			zend_long line;
			tmp = zend_hash_find(ht, zend_known_strings[ZEND_STR_LINE]);
			if (tmp) {
	...
	TRACE_APPEND_KEY(zend_known_strings[ZEND_STR_CLASS]);
	TRACE_APPEND_KEY(zend_known_strings[ZEND_STR_TYPE]);
	TRACE_APPEND_KEY(zend_known_strings[ZEND_STR_FUNCTION]);
	smart_str_appendc(str, '(');
	tmp = zend_hash_find(ht, zend_known_strings[ZEND_STR_ARGS]);
	if (tmp) {
...
ZEND_METHOD(exception, getTraceAsString)
{
	...
	trace = zend_read_property_ex(base_ce, object, zend_known_strings[ZEND_STR_TRACE], 1, &rv);
...
ZEND_METHOD(exception, __toString)
{
	...
	ZVAL_STR(&tmp, str);
	zend_update_property_ex(base_ce, exception, zend_known_strings[ZEND_STR_STRING], &tmp);
...
ZEND_API ZEND_COLD zend_object *zend_throw_exception(zend_class_entry *exception_ce, const char *message, zend_long code) /* {{{ */
{
	...
	if (message) {
		ZVAL_STRING(&tmp, message);
		zend_update_property_ex(exception_ce, &ex, zend_known_strings[ZEND_STR_MESSAGE], &tmp);
		zval_ptr_dtor(&tmp);
	}
	if (code) {
		ZVAL_LONG(&tmp, code);
		zend_update_property_ex(exception_ce, &ex, zend_known_strings[ZEND_STR_CODE], &tmp);
	}
...
ZEND_API ZEND_COLD zend_object *zend_throw_error_exception(zend_class_entry *exception_ce, const char *message, zend_long code, int severity) /* {{{ */
{
	...
	ZVAL_OBJ(&ex, obj);
	ZVAL_LONG(&tmp, severity);
	zend_update_property_ex(zend_ce_error_exception, &ex, zend_known_strings[ZEND_STR_SEVERITY], &tmp);
	return obj;
}
/* }}} */
	...
	if (ce_exception == zend_ce_parse_error) {
		...
		if (!EG(exception)) {
			if (Z_TYPE(tmp) != IS_STRING) {
				...
				zend_update_property_ex(i_get_exception_base(&exception), &exception, zend_known_strings[ZEND_STR_STRING], &tmp);
			}
		}
		zval_ptr_dtor(&tmp);
