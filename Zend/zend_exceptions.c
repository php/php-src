...
void zend_exception_set_previous(zend_object *exception, zend_object *add_previous)
{
	...
	if (!instanceof_function(Z_OBJCE(tmp), throwable_ce)) {
		zend_error_noreturn(E_CORE_ERROR, "Cannot set non throwable as previous");
		return;
	}
	ZVAL_OBJ(&zv, exception);
	pzv = &zv;
	do {
		previous = zend_read_property(throwable_ce, pzv, "previous", sizeof("previous")-1, 1, &rv);
		if (Z_TYPE_P(previous) == IS_NULL) {
			zend_update_property(throwable_ce, pzv, "previous", sizeof("previous")-1, &tmp);
...
static zend_object *zend_default_exception_new_ex(zend_class_entry *class_type, int skip_top_traces) /* {{{ */
{
	...
	if (EXPECTED(class_type != parse_error_ce)) {
		zend_update_property_string(throwable_ce, &obj, "file", sizeof("file")-1, zend_get_executed_filename());
		zend_update_property_long(throwable_ce, &obj, "line", sizeof("line")-1, zend_get_executed_lineno());
	} else {
		zend_update_property_string(throwable_ce, &obj, "file", sizeof("file")-1, zend_get_compiled_filename()->val);
		zend_update_property_long(throwable_ce, &obj, "line", sizeof("line")-1, zend_get_compiled_lineno());
	}
	zend_update_property(throwable_ce, &obj, "trace", sizeof("trace")-1, &trace);
...
ZEND_METHOD(exception, __construct)
{
	...
	if (zend_parse_parameters_ex(ZEND_PARSE_PARAMS_QUIET, argc, "|SlO!", &message, &code, &previous, throwable_ce) == FAILURE) {
	...
	if (message) {
		zend_update_property_str(throwable_ce, object, "message", sizeof("message")-1, message);
	}

	if (code) {
		zend_update_property_long(throwable_ce, object, "code", sizeof("code")-1, code);
	}

	if (previous) {
		zend_update_property(throwable_ce, object, "previous", sizeof("previous")-1, previous);
	}
}
/* }}} */
...
ZEND_METHOD(error_exception, __construct)
{
	...
	if (zend_parse_parameters_ex(ZEND_PARSE_PARAMS_QUIET, argc, "|sllslO!", &message, &message_len, &code, &severity, &filename, &filename_len, &lineno, &previous, throwable_ce) == FAILURE) {
	...
	if (message) {
		zend_update_property_string(throwable_ce, object, "message", sizeof("message")-1, message);
	}
	...
	zend_update_property_long(throwable_ce, object, "severity", sizeof("severity")-1, severity);

	if (argc >= 4) {
	    zend_update_property_string(throwable_ce, object, "file", sizeof("file")-1, filename);
    	if (argc < 5) {
    	    lineno = 0; /* invalidate lineno */
    	}
		zend_update_property_long(throwable_ce, object, "line", sizeof("line")-1, lineno);
	}
}
/* }}} */
...
#define GET_PROPERTY(object, name) \
	zend_read_property(throwable_ce, (object), name, sizeof(name) - 1, 0, &rv)
#define GET_PROPERTY_SILENT(object, name) \
	zend_read_property(throwable_ce, (object), name, sizeof(name) - 1, 1, &rv)
...
ZEND_METHOD(exception, getTraceAsString)
{
	...
	trace = zend_read_property(throwable_ce, getThis(), "trace", sizeof("trace")-1, 1, &rv);
...
ZEND_METHOD(exception, __toString)
{
	...
	zend_update_property_str(throwable_ce, getThis(), "string", sizeof("string")-1, str);
...
void zend_register_default_exception(void) /* {{{ */
{
	...
	INIT_CLASS_ENTRY(ce, "Throwable", default_exception_functions);
	throwable_ce = zend_register_internal_class(&ce);
	throwable_ce->ce_flags |= ZEND_ACC_EXPLICIT_ABSTRACT_CLASS;
	throwable_ce->create_object = NULL;
	memcpy(&default_exception_handlers, zend_get_std_object_handlers(), sizeof(zend_object_handlers));
	default_exception_handlers.clone_obj = NULL;

	zend_declare_property_string(throwable_ce, "message", sizeof("message")-1, "", ZEND_ACC_PROTECTED);
	zend_declare_property_string(throwable_ce, "string", sizeof("string")-1, "", ZEND_ACC_PRIVATE);
	zend_declare_property_long(throwable_ce, "code", sizeof("code")-1, 0, ZEND_ACC_PROTECTED);
	zend_declare_property_null(throwable_ce, "file", sizeof("file")-1, ZEND_ACC_PROTECTED);
	zend_declare_property_null(throwable_ce, "line", sizeof("line")-1, ZEND_ACC_PROTECTED);
	zend_declare_property_null(throwable_ce, "trace", sizeof("trace")-1, ZEND_ACC_PRIVATE);
	zend_declare_property_null(throwable_ce, "previous", sizeof("previous")-1, ZEND_ACC_PRIVATE);

	INIT_CLASS_ENTRY(ce, "Exception", NULL);
	default_exception_ce = zend_register_internal_class_ex(&ce, throwable_ce);
	default_exception_ce->create_object = zend_default_exception_new;

	/* A trick, to make visible private properties of Throwable */
	ZEND_HASH_FOREACH_PTR(&default_exception_ce->properties_info, prop) {
		if (prop->flags & ZEND_ACC_SHADOW) {
			if (prop->name->len == sizeof("\0Throwable\0string")-1) {
				prop->flags &= ~ZEND_ACC_SHADOW;
				prop->flags |= ZEND_ACC_PRIVATE;
				prop->ce = default_exception_ce;
			} else if (prop->name->len == sizeof("\0Throwable\0trace")-1) {
				prop->flags &= ~ZEND_ACC_SHADOW;
				prop->flags |= ZEND_ACC_PRIVATE;
				prop->ce = default_exception_ce;
			} else if (prop->name->len == sizeof("\0Throwable\0previous")-1) {
				prop->flags &= ~ZEND_ACC_SHADOW;
				prop->flags |= ZEND_ACC_PRIVATE;
				prop->ce = default_exception_ce;
			}
	...
	error_ce = zend_register_internal_class_ex(&ce, throwable_ce);
	...
	engine_error_ce = zend_register_internal_class_ex(&ce, error_ce);
	engine_error_ce->create_object = zend_default_exception_new;
	...
	type_error_ce = zend_register_internal_class_ex(&ce, engine_error_ce);
...
ZEND_API zend_object *zend_throw_exception(zend_class_entry *exception_ce, const char *message, zend_long code) /* {{{ */
{
	...
	if (exception_ce) {
		if (!instanceof_function(exception_ce, throwable_ce)) {
	...
	if (message) {
		zend_update_property_string(throwable_ce, &ex, "message", sizeof("message")-1, message);
	}
	if (code) {
		zend_update_property_long(throwable_ce, &ex, "code", sizeof("code")-1, code);
	}
...
ZEND_API zend_object *zend_throw_error_exception(zend_class_entry *exception_ce, const char *message, zend_long code, int severity) /* {{{ */
{
	...
	ZVAL_OBJ(&ex, obj);
	zend_update_property_long(throwable_ce, &ex, "severity", sizeof("severity")-1, severity);
	return obj;
}
/* }}} */
...
ZEND_API void zend_exception_error(zend_object *ex, int severity) /* {{{ */
{
	...
	if (ce_exception == parse_error_ce || ce_exception == type_error_ce) {
		...
		zend_string_release(file);
		zend_string_release(message);
	} else if (instanceof_function(ce_exception, throwable_ce)) {
		zval tmp, rv;
		...
		if (!EG(exception)) {
			if (Z_TYPE(tmp) != IS_STRING) {
				...
				zend_update_property_string(throwable_ce, &exception, "string", sizeof("string")-1, EG(exception) ? ce_exception->name->val : Z_STRVAL(tmp));
			}
		}
		zval_ptr_dtor(&tmp);
		...
		if (EG(exception)) {
			...
			if (instanceof_function(ce_exception, throwable_ce)) {
...
ZEND_API void zend_throw_exception_object(zval *exception) /* {{{ */
{
	...
	if (!exception_ce || !instanceof_function(exception_ce, throwable_ce)) {
