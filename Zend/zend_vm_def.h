	...
	if (ce == NULL) {
		...
		if (zv) {
			...
			if (UNEXPECTED(!zv)) {
				...
				if (zend_do_link_class_ex(ce, Z_STR_P(RT_CONSTANT(opline, opline->op2))) == FAILURE) {
...
ZEND_VM_HANDLER(146, ZEND_DECLARE_ANON_CLASS, ANY, ANY, CACHE_SLOT)
{
	...
	if (UNEXPECTED(ce == NULL)) {
		...
		if (!(ce->ce_flags & ZEND_ACC_LINKED)) {
			SAVE_OPLINE();
			if (zend_do_link_class_ex(ce, (OP2_TYPE == IS_CONST) ? Z_STR_P(RT_CONSTANT(opline, opline->op2)) : NULL) == FAILURE) {
