...
ZEND_VM_HELPER(zend_binary_assign_op_obj_helper, VAR|UNUSED|CV, CONST|TMPVAR|CV, binary_op_type binary_op)
{
	...
	do {
		...
		cache_slot = (OP2_TYPE == IS_CONST) ? CACHE_ADDR(Z_CACHE_SLOT_P(property)) : NULL;
		if (EXPECTED(Z_OBJ_HT_P(object)->get_property_ptr_ptr)
			&& EXPECTED((zptr = Z_OBJ_HT_P(object)->get_property_ptr_ptr(object, property, BP_VAR_RW, cache_slot)) != NULL)) {
			if (UNEXPECTED(Z_ISERROR_P(zptr))) {
				...
				if (OP2_TYPE == IS_CONST) {
					...
					prop_info = zend_object_fetch_property_type_info(Z_OBJCE_P(object), Z_STR_P(property), NULL);
				}
				if (UNEXPECTED(prop_info || ref_type)) {
			...
			zend_assign_op_overloaded_property(object, property, cache_slot, value, binary_op, (UNEXPECTED(RETURN_VALUE_USED(opline)) ? EX_VAR(opline->result.var) : NULL));
		}
	} while (0);
...
ZEND_VM_C_LABEL(assign_dim_op_new_array):
		...
		if (OP2_TYPE != IS_UNUSED && UNEXPECTED(Z_ISREF_P(var_ptr))) {
			zend_type type = Z_REFTYPE_P(var_ptr);
			var_ptr = Z_REFVAL_P(var_ptr);
			if (UNEXPECTED(type)) {
				zval tmp;
				binary_op(&tmp, var_ptr, value);
				if (EXPECTED(zend_verify_ref_type_assignable_zval(type, &tmp, EX_USES_STRICT_TYPES()))) {
					zval_ptr_dtor(var_ptr);
					ZVAL_COPY_VALUE(var_ptr, &tmp);
				} else {
					zend_throw_ref_type_error(type, &tmp);
					zval_ptr_dtor(&tmp);
					FREE_OP2();
					FREE_OP(free_op_data1);
					FREE_OP1_VAR_PTR();
...
ZEND_VM_HELPER(zend_binary_assign_op_simple_helper, VAR|CV, CONST|TMPVAR|CV, binary_op_type binary_op)
{
	...
	if (OP1_TYPE == IS_VAR && UNEXPECTED(Z_ISERROR_P(var_ptr))) {
		...
		if (UNEXPECTED(Z_TYPE_P(var_ptr) == IS_REFERENCE)) {
			ref_type = Z_REFTYPE_P(var_ptr);
			var_ptr = Z_REFVAL_P(var_ptr);
		}

		if (UNEXPECTED(ref_type)) {
			zval old;
			...
			if (EXPECTED(zend_verify_ref_type_assignable_zval(ref_type, var_ptr, EX_USES_STRICT_TYPES()))) {
				zval_ptr_dtor(&old);
			} else {
				zend_throw_ref_type_error(ref_type, var_ptr);
				zval_ptr_dtor(var_ptr);
		...
		if (EXPECTED(Z_OBJ_HT_P(object)->get_property_ptr_ptr)
			...
			if (UNEXPECTED(Z_ISERROR_P(zptr))) {
				...
				if (EXPECTED(Z_TYPE_P(zptr) == IS_LONG)) {
					...
					if (UNEXPECTED(Z_TYPE_P(zptr) != IS_LONG)) {
						if (OP2_TYPE == IS_CONST) {
							prop_info = (zend_property_info*)CACHED_PTR_EX(cache_slot + 2);
						} else {
							prop_info = zend_object_fetch_property_type_info(Z_OBJCE_P(object), Z_STR_P(property), NULL);
			...
			zend_pre_incdec_overloaded_property(object, property, cache_slot, inc, (UNEXPECTED(RETURN_VALUE_USED(opline)) ? EX_VAR(opline->result.var) : NULL));
		}
	} while (0);
...
ZEND_VM_HELPER(zend_post_incdec_property_helper, VAR|UNUSED|CV, CONST|TMPVAR|CV, int inc)
{
	...
	do {
		...
		if (EXPECTED(Z_OBJ_HT_P(object)->get_property_ptr_ptr)
			...
			zend_post_incdec_overloaded_property(object, property, cache_slot, inc, EX_VAR(opline->result.var));
		}
	} while (0);
...
ZEND_VM_HANDLER(36, ZEND_POST_INC, VAR|CV, ANY)
{
	...
	if (UNEXPECTED(ref_type) && UNEXPECTED(!zend_verify_ref_type_assignable_zval(ref_type, var_ptr, EX_USES_STRICT_TYPES()))) {
		zend_throw_ref_type_error(ref_type, var_ptr);
		zval_ptr_dtor(var_ptr);
...
ZEND_VM_HANDLER(85, ZEND_FETCH_OBJ_W, VAR|UNUSED|THIS|CV, CONST|TMPVAR|CV, FETCH_REF)
{
	USE_OPLINE
	zend_free_op free_op1, free_op2;
	zval *property;
	...
	zend_fetch_property_address(EX_VAR(opline->result.var), container, OP1_TYPE, property, OP2_TYPE, ((OP2_TYPE == IS_CONST) ? CACHE_ADDR(Z_CACHE_SLOT_P(property)) : NULL), BP_VAR_W, opline->extended_value);
	FREE_OP2();
	if (OP1_TYPE == IS_VAR && READY_TO_DESTROY(free_op1)) {
		EXTRACT_ZVAL_PTR(EX_VAR(opline->result.var));
	}
...
ZEND_VM_HANDLER(88, ZEND_FETCH_OBJ_RW, VAR|UNUSED|THIS|CV, CONST|TMPVAR|CV)
{
	...
	zend_fetch_property_address(EX_VAR(opline->result.var), container, OP1_TYPE, property, OP2_TYPE, ((OP2_TYPE == IS_CONST) ? CACHE_ADDR(Z_CACHE_SLOT_P(property)) : NULL), BP_VAR_RW, 0);
	FREE_OP2();
	if (OP1_TYPE == IS_VAR && READY_TO_DESTROY(free_op1)) {
		EXTRACT_ZVAL_PTR(EX_VAR(opline->result.var));
	}
...
ZEND_VM_HANDLER(94, ZEND_FETCH_OBJ_FUNC_ARG, CONST|TMP|VAR|UNUSED|THIS|CV, CONST|TMPVAR|CV, NUM|FETCH_REF)
{
	USE_OPLINE
	zval *container;
...
ZEND_VM_HANDLER(97, ZEND_FETCH_OBJ_UNSET, VAR|UNUSED|THIS|CV, CONST|TMPVAR|CV)
{
	...
	zend_fetch_property_address(EX_VAR(opline->result.var), container, OP1_TYPE, property, OP2_TYPE, ((OP2_TYPE == IS_CONST) ? CACHE_ADDR(Z_CACHE_SLOT_P(property)) : NULL), BP_VAR_UNSET, 0);
	FREE_OP2();
	if (OP1_TYPE == IS_VAR && READY_TO_DESTROY(free_op1)) {
		EXTRACT_ZVAL_PTR(EX_VAR(opline->result.var));
	}
...
ZEND_VM_HANDLER(136, ZEND_ASSIGN_OBJ, VAR|UNUSED|THIS|CV, CONST|TMPVAR|CV, SPEC(OP_DATA=CONST|TMP|VAR|CV))
{
	...
	if (OP2_TYPE == IS_CONST &&
		...
		void **cache_slot = CACHE_ADDR(Z_CACHE_SLOT_P(property_name));
...
ZEND_VM_C_LABEL(fast_assign_obj):
				value = zend_assign_to_variable(property, value, OP_DATA_TYPE, EX_USES_STRICT_TYPES());
				if (UNEXPECTED(RETURN_VALUE_USED(opline))) {
...
ZEND_VM_HANDLER(39, ZEND_ASSIGN_REF, VAR|CV, VAR|CV, SRC)
{
	...
	if (OP1_TYPE == IS_VAR &&
	   ...
	   UNEXPECTED(!Z_ISREF_P(variable_ptr))) {
		...
		value_ptr = zend_assign_to_variable(variable_ptr, value_ptr, OP2_TYPE, EX_USES_STRICT_TYPES());
		if (UNEXPECTED(RETURN_VALUE_USED(opline))) {
...
ZEND_VM_HANDLER(100, ZEND_ASSIGN_OBJ_REF, VAR|UNUSED|THIS|CV, CONST|TMPVAR|CV, SRC, SPEC(OP_DATA=VAR|CV))
{
	USE_OPLINE
	zend_free_op free_op1, free_op2, free_op_data;
	zval *property;
	...
	zend_fetch_property_address(variable_ptr, container, OP1_TYPE, property, OP2_TYPE, cache_addr, BP_VAR_W, 0);

	if (UNEXPECTED(Z_TYPE(variable) != IS_INDIRECT) &&
	    UNEXPECTED(!Z_ISERROR(variable))) {
	...
	if ((OP_DATA_TYPE == IS_VAR && UNEXPECTED(Z_ISERROR_P(value_ptr)))
	 || UNEXPECTED(Z_ISERROR(variable))) {
		...
		variable_ptr = Z_INDIRECT(variable);

		if ((OP2_TYPE == IS_CONST) ? EXPECTED(ZEND_DYNAMIC_PROPERTY_OFFSET != (uint32_t)(intptr_t) CACHED_PTR_EX(cache_addr + 1)) : EXPECTED(Z_TYPE_P(property) == IS_STRING)) {
			zend_property_info *prop_info;

			if (OP2_TYPE == IS_CONST) {
				prop_info = (zend_property_info *) CACHED_PTR_EX(cache_addr + 2);
			} else {
				prop_info = zend_object_fetch_property_type_info(Z_OBJCE_P(container), Z_STR_P(property), NULL);
			}
			...
			if (UNEXPECTED(prop_info)) {
				...
				if (Z_TYPE_P(value_ptr) == IS_REFERENCE) {
					...
					if (old_type) {
						...
						if (ZEND_TYPE_WITHOUT_NULL(old_type) != ref_type) {
							if (ZEND_TYPE_IS_CLASS(ref_type) && ZEND_TYPE_IS_CLASS(old_type)) {
								if (instanceof_function(ZEND_TYPE_CE(ref_type), ZEND_TYPE_CE(old_type))) {
									ref_type = ZEND_TYPE_ALLOW_NULL(ref_type) ? old_type : ZEND_TYPE_WITHOUT_NULL(old_type);
						...
						if (UNEXPECTED(error) && !EG(exception)) {
							zend_throw_exception_ex(
								zend_ce_type_error, ZEND_TYPE_IS_CLASS(prop_info->type) ? IS_OBJECT : ZEND_TYPE_CODE(prop_info->type),
								"Property and reference types %s%s and %s%s are not compatible",
								ZEND_TYPE_ALLOW_NULL(prop_info->type) ? "?" : "",
								ZEND_TYPE_IS_CLASS(prop_info->type) ? ZSTR_VAL(ZEND_TYPE_CE(prop_info->type)->name) : zend_get_type_by_const(ZEND_TYPE_CODE(prop_info->type)),
								ZEND_TYPE_ALLOW_NULL(old_type) ? "?" : "",
								ZEND_TYPE_IS_CLASS(old_type) ? ZSTR_VAL(ZEND_TYPE_CE(old_type)->name) : zend_get_type_by_const(ZEND_TYPE_CODE(old_type))
							);
						}
					}
				}
				if (!i_zend_verify_ref_type_assignable_zval(ref_type, value_ptr, EX_USES_STRICT_TYPES())) {
					ZVAL_DEREF(value_ptr);
					zend_throw_ref_type_error(ref_type, value_ptr);
					FREE_OP1_VAR_PTR();
					FREE_OP2();
		...
		zend_assign_to_variable_reference(variable_ptr, value_ptr);
		if (ref_type) {
			Z_REFTYPE_P(variable_ptr) = ref_type;
		}
	}
