...
tail_call:
	...
	case ZEND_AST_NEW:
		...
		if (ast->child[0]->kind == ZEND_AST_CLASS)
		{
			...
			smart_str_appends(str, "class");
			if (!zend_ast_is_list(ast->child[1]) || zend_ast_get_list(ast->child[1])->children)
			{
				smart_str_appendc(str, '(');
				zend_ast_export_ex(str, ast->child[1], 0, indent);
				smart_str_appendc(str, ')');
