...
void zend_interned_strings_init(void)
{
	...
	HT_SET_DATA(&CG(interned_strings), pemalloc(HT_SIZE(&CG(interned_strings)), 1));
...
static zend_string *zend_new_interned_string_int(zend_string *str)
{
	...
	while (idx != HT_INVALID_IDX) {
		p = HT_DATA(&CG(interned_strings)) + idx;
	...
	if (CG(interned_strings).nNumUsed >= CG(interned_strings).nTableSize) {
		...
		if ((CG(interned_strings).nTableSize << 1) > 0) {
			void *new_data;
			void *old_data = HT_GET_DATA(&CG(interned_strings));
			Bucket *old_buckets = HT_DATA(&CG(interned_strings));

			HANDLE_BLOCK_INTERRUPTIONS();
			CG(interned_strings).nTableSize = CG(interned_strings).nTableSize << 1;
			CG(interned_strings).nTableMask = CG(interned_strings).nTableSize - 1;
			//TODO: memory overflow check using same_pemalloc() and NULL check ???
			...
			if (new_data) {
				HT_SET_DATA(&CG(interned_strings), new_data);
				memcpy(HT_DATA(&CG(interned_strings)), old_buckets, sizeof(Bucket) * CG(interned_strings).nNumUsed);
	...
	HT_HASH(&CG(interned_strings), nIndex) = idx;
...
static void zend_interned_strings_restore_int(void)
{
	...
	while (idx > 0) {
		...
		if (HT_HASH(&CG(interned_strings), nIndex) == idx) {
			HT_HASH(&CG(interned_strings), nIndex) = Z_NEXT(p->val);
		} else {
			uint prev = HT_HASH(&CG(interned_strings), nIndex);
			while (Z_NEXT(HT_DATA(&CG(interned_strings))[prev].val) != idx) {
				prev = Z_NEXT(HT_DATA(&CG(interned_strings))[prev].val);
 			}
			Z_NEXT(HT_DATA(&CG(interned_strings))[prev].val) = Z_NEXT(p->val);
 		}
 	}
#endif
}
