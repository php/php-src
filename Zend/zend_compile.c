...
void zend_compile_anon_declare(zend_class_entry *ce, zend_ast *use_ast) { /* {{{ */
	...
	zend_hash_init(&used, 8, NULL, NULL, 0);
	...
	for (i = 0; i < use_list->children; i++) {
		...
		switch (use_var->kind) {
			case ZEND_AST_ZVAL: {
				var_name = zend_ast_get_str(use_var);

				if (zend_hash_exists(&used, var_name)) {
					zend_error_noreturn(E_COMPILE_ERROR, "Cannot use property name %s twice", ZSTR_VAL(var_name));
				}
				...
				if (!zend_hash_exists(&ce->properties_info, var_name)) {
					zend_declare_property(ce, ZSTR_VAL(var_name), ZSTR_LEN(var_name), &nil, ZEND_ACC_PRIVATE);
				}
			} break;

			case ZEND_AST_PROP: {
				var_name = zend_ast_get_str(use_var->child[1]);
		...
		zend_hash_add_empty_element(&used, var_name);
	}
	...
	for (i = 0; i < use_list->children; i++) {
		...
		switch (use_var->kind) {
			case ZEND_AST_ZVAL: {
				...
				if (zend_hash_exists(&used, var_name)) {
					zend_error_noreturn(E_COMPILE_ERROR, "Cannot use variable $%s twice", ZSTR_VAL(var_name));
				}
...
zend_ast* zend_compile_class_decl(zend_ast *ast) /* {{{ */
{
	...
	if (use_ast) {
		zend_compile_anon_declare(ce, use_ast);
	}
