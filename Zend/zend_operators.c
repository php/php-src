...
try_again:
	switch (Z_TYPE_P(op)) {
		...
		case IS_TRUE:
			ZVAL_NEW_STR(op, ZSTR_INIT("1", 1, 0));
			break;
		case IS_STRING:
			break;
		case IS_RESOURCE: {
			...
			zval_ptr_dtor(op);
			ZVAL_NEW_STR(op, ZSTR_INIT(buf, len, 0));
			break;
		}
		case IS_LONG: {
		...
		case IS_ARRAY:
			...
			zval_ptr_dtor(op);
			ZVAL_NEW_STR(op, ZSTR_INIT("Array", sizeof("Array")-1, 0));
			break;
		case IS_OBJECT: {
			zval dst;
			...
			if (Z_TYPE(dst) == IS_STRING) {
				...
				zval_dtor(op);
				ZVAL_NEW_STR(op, ZSTR_INIT("Object", sizeof("Object")-1, 0));
			}
			break;
		}
		case IS_REFERENCE:
...
try_again:
	switch (Z_TYPE_P(op)) {
		...
		case IS_ARRAY:
			zend_error(E_NOTICE, "Array to string conversion");
			return ZSTR_INIT("Array", sizeof("Array")-1, 0);
		case IS_OBJECT: {
			zval tmp;
			if (Z_OBJ_HT_P(op)->cast_object) {
		...
		case IS_STRING: {
			...
			if (Z_STRLEN_P(op1) == 1) {
				...
				if (CG(one_char_string)[not]) {
					...
					ZVAL_NEW_STR(result, ZSTR_INIT((char *) &not, 1, 0));
				}
			} else {
				ZVAL_NEW_STR(result, ZSTR_ALLOC(Z_STRLEN_P(op1), 0));
...
ZEND_API int ZEND_FASTCALL bitwise_or_function(zval *result, zval *op1, zval *op2) /* {{{ */
{
	...
	if (Z_TYPE_P(op1) == IS_STRING && EXPECTED(Z_TYPE_P(op2) == IS_STRING)) {
		...
		if (EXPECTED(Z_STRLEN_P(op1) >= Z_STRLEN_P(op2))) {
			if (EXPECTED(Z_STRLEN_P(op1) == Z_STRLEN_P(op2)) && Z_STRLEN_P(op1) == 1) {
				...
				if (CG(one_char_string)[or]) {
					...
					ZVAL_NEW_STR(result, ZSTR_INIT((char *) &or, 1, 0));
				}
				return SUCCESS;
			}
			longer = op1;
		...
		str = ZSTR_ALLOC(Z_STRLEN_P(longer), 0);
...
ZEND_API int ZEND_FASTCALL bitwise_and_function(zval *result, zval *op1, zval *op2) /* {{{ */
{
	...
	if (Z_TYPE_P(op1) == IS_STRING && Z_TYPE_P(op2) == IS_STRING) {
		...
		if (EXPECTED(Z_STRLEN_P(op1) >= Z_STRLEN_P(op2))) {
			if (EXPECTED(Z_STRLEN_P(op1) == Z_STRLEN_P(op2)) && Z_STRLEN_P(op1) == 1) {
				...
				if (CG(one_char_string)[and]) {
					...
					ZVAL_NEW_STR(result, ZSTR_INIT((char *) &and, 1, 0));
				}
				return SUCCESS;
			}
			longer = op1;
		...
		str = ZSTR_ALLOC(Z_STRLEN_P(shorter), 0);
		for (i = 0; i < Z_STRLEN_P(shorter); i++) {
		...
		if (EXPECTED(Z_STRLEN_P(op1) >= Z_STRLEN_P(op2))) {
			if (EXPECTED(Z_STRLEN_P(op1) == Z_STRLEN_P(op2)) && Z_STRLEN_P(op1) == 1) {
				...
				if (CG(one_char_string)[xor]) {
					...
					ZVAL_NEW_STR(result, ZSTR_INIT((char *) &xor, 1, 0));
				}
				return SUCCESS;
			}
			longer = op1;
...
ZEND_API int ZEND_FASTCALL concat_function(zval *result, zval *op1, zval *op2) /* {{{ */
{
...

	{
		...
		if (result == op1 && Z_REFCOUNTED_P(result)) {
			...
			result_str = ZSTR_EXTEND(Z_STR_P(result), result_len, 0);
		} else {
			result_str = ZSTR_ALLOC(result_len, 0);
...
static void ZEND_FASTCALL increment_string(zval *str) /* {{{ */
{
	...
	if (Z_STRLEN_P(str) == 0) {
		...
		Z_STR_P(str) = ZSTR_INIT("1", sizeof("1")-1, 0);
	...
	if (carry) {
		t = ZSTR_ALLOC(Z_STRLEN_P(str)+1, 0);
...
ZEND_API zend_string* ZEND_FASTCALL zend_long_to_str(zend_long num) /* {{{ */
{
	...
	return ZSTR_INIT(res, buf + sizeof(buf) - 1 - res, 0);
}
/* }}} */
