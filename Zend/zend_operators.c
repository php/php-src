...
try_again:
	switch (Z_TYPE_P(op)) {
		...
		case IS_TRUE:
			ZVAL_INTERNED_STR(op, ZSTR_ONE_CHAR('1'));
			break;
		case IS_STRING:
			break;
		case IS_RESOURCE: {
	...
	switch (Z_TYPE_P(op1)) {
		...
		case IS_STRING: {
			...
			if (Z_STRLEN_P(op1) == 1) {
				...
				ZVAL_INTERNED_STR(result, ZSTR_ONE_CHAR(not));
			} else {
	...
	if (Z_TYPE_P(op1) == IS_STRING && EXPECTED(Z_TYPE_P(op2) == IS_STRING)) {
		...
		if (EXPECTED(Z_STRLEN_P(op1) >= Z_STRLEN_P(op2))) {
			if (EXPECTED(Z_STRLEN_P(op1) == Z_STRLEN_P(op2)) && Z_STRLEN_P(op1) == 1) {
				...
				ZVAL_INTERNED_STR(result, ZSTR_ONE_CHAR(or));
				return SUCCESS;
			}
			longer = op1;
			shorter = op2;
	...
	if (Z_TYPE_P(op1) == IS_STRING && Z_TYPE_P(op2) == IS_STRING) {
		...
		if (EXPECTED(Z_STRLEN_P(op1) >= Z_STRLEN_P(op2))) {
			if (EXPECTED(Z_STRLEN_P(op1) == Z_STRLEN_P(op2)) && Z_STRLEN_P(op1) == 1) {
				...
				ZVAL_INTERNED_STR(result, ZSTR_ONE_CHAR(and));
				return SUCCESS;
			}
			longer = op1;
			shorter = op2;
	...
	if (Z_TYPE_P(op1) == IS_STRING && Z_TYPE_P(op2) == IS_STRING) {
		...
		if (EXPECTED(Z_STRLEN_P(op1) >= Z_STRLEN_P(op2))) {
			if (EXPECTED(Z_STRLEN_P(op1) == Z_STRLEN_P(op2)) && Z_STRLEN_P(op1) == 1) {
				...
				ZVAL_INTERNED_STR(result, ZSTR_ONE_CHAR(xor));
				return SUCCESS;
			}
			longer = op1;
			shorter = op2;
...
static void ZEND_FASTCALL increment_string(zval *str) /* {{{ */
{
	...
	if (Z_STRLEN_P(str) == 0) {
		...
		ZVAL_INTERNED_STR(str, ZSTR_ONE_CHAR('1'));
		return;
	}
