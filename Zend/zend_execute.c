...
static zend_never_inline zend_bool zend_check_ref_stdClass_assignable(zend_reference *ref);
static zend_never_inline zend_bool zend_check_ref_array_assignable(zend_reference *ref);
...
static zend_never_inline ZEND_COLD zval* ZEND_FASTCALL make_real_object(zval *object, zval *property OPLINE_DC EXECUTE_DATA_DC)
{
	...
	if (ref && ZEND_REF_HAS_TYPE_SOURCES(Z_REF_P(ref))) {
		if (UNEXPECTED(!zend_check_ref_stdClass_assignable(Z_REF_P(ref)))) {
...
fetch_from_array:
		...
		if (EXPECTED(Z_TYPE_P(container) == IS_ARRAY)) {
			...
			if (type != BP_VAR_UNSET) {
				if (ZEND_REF_HAS_TYPE_SOURCES(ref)) {
					if (UNEXPECTED(!zend_check_ref_array_assignable(ref))) {
						ZVAL_ERROR(result);
						return;
					}
				}
