...
static inline void make_real_object(zval **object_ptr TSRMLS_DC)
{
	if (Z_TYPE_PP(object_ptr) == IS_NULL
		|| (Z_TYPE_PP(object_ptr) == IS_BOOL && Z_IVAL_PP(object_ptr) == 0)
...
ZEND_API int zend_verify_arg_error(int error_type, const zend_function *zf, zend_size_t arg_num, const char *need_msg, const char *need_kind, const char *given_msg, const char *given_kind TSRMLS_DC)
{
	...
	if (ptr && ptr->op_array) {
		zend_error(error_type, "Argument %pu passed to %s%s%s() must %s%s, %s%s given, called in %s on line %pu and defined", arg_num, fclass, fsep, fname, need_msg, need_kind, given_msg, given_kind, ptr->op_array->filename, ptr->opline->lineno);
	} else {
		zend_error(error_type, "Argument %pu passed to %s%s%s() must %s%s, %s%s given", arg_num, fclass, fsep, fname, need_msg, need_kind, given_msg, given_kind);
	}
	return 0;
}
...
static inline void zend_assign_to_object(zval **retval, zval **object_ptr, zval *property_name, int value_type, znode_op *value_op, const zend_execute_data *execute_data, int opcode, const zend_literal *key TSRMLS_DC)
{
	...
	if (Z_TYPE_P(object) != IS_OBJECT) {
		...
		if (Z_TYPE_P(object) == IS_NULL ||
		    (Z_TYPE_P(object) == IS_BOOL && Z_IVAL_P(object) == 0) ||
...
fetch_string_dim:
		...
		case IS_DOUBLE:
			...
			goto num_index;
		case IS_RESOURCE:
			zend_error(E_STRICT, "Resource ID#" ZEND_INT_FMT " used as offset, casting to integer (" ZEND_INT_FMT ")", Z_IVAL_P(dim), Z_IVAL_P(dim));
			/* Fall Through */
		case IS_BOOL:
		case IS_INT:
			hval = Z_IVAL_P(dim);
...
convert_to_array:
		...
		case IS_STRING: {
				...
				result->str_offset.offset = Z_IVAL_P(dim);
		...
		case IS_BOOL:
			if (type != BP_VAR_UNSET && Z_IVAL_P(container)==0) {
				goto convert_to_array;
			}
			/* break missing intentionally */
...
static void zend_fetch_dimension_address_read(temp_variable *result, zval *container, zval *dim, int dim_type, int type TSRMLS_DC)
{
	...
	switch (Z_TYPE_P(container)) {
		...
		case IS_STRING: {
				...
				if (Z_IVAL_P(dim) < 0 || Z_STRSIZE_P(container) <= Z_IVAL_P(dim)) {
					if (type != BP_VAR_IS) {
						zend_error(E_NOTICE, "Uninitialized string offset: " ZEND_INT_FMT, Z_IVAL_P(dim));
					}
					Z_STRVAL_P(ptr) = STR_EMPTY_ALLOC();
					Z_STRSIZE_P(ptr) = 0;
				} else {
					Z_STRVAL_P(ptr) = (char*)emalloc(2);
					Z_STRVAL_P(ptr)[0] = Z_STRVAL_P(container)[Z_IVAL_P(dim)];
					Z_STRVAL_P(ptr)[1] = 0;
					Z_STRSIZE_P(ptr) = 1;
				}
				result->var.ptr = ptr;
	...
	if (Z_TYPE_P(container) != IS_OBJECT) {
		...
		if (type != BP_VAR_UNSET &&
		    ((Z_TYPE_P(container) == IS_NULL ||
		     (Z_TYPE_P(container) == IS_BOOL && Z_IVAL_P(container)==0) ||
