...
static ZEND_COLD void zend_verify_type_error_common(
		...
		zend_bool strict, const char *error_adjective,
	...
	*fname = ZSTR_VAL(zf->common.function_name);
	...
	if (ZEND_TYPE_IS_CLASS(arg_info->type)) {
		...
		switch (ZEND_TYPE_CODE(arg_info->type)) {
			case IS_LONG:
				*need_msg = "be an ";
				*need_kind = (strict ? expected_error_strict : expected_error_weak)[Z_EXPECTED_LONG];
				break;
			case IS_DOUBLE:
				*need_msg = "be a ";
				*need_kind = (strict ? expected_error_strict : expected_error_weak)[Z_EXPECTED_DOUBLE];
				break;
			case IS_STRING:
				*need_msg = "be a ";
				*need_kind = (strict ? expected_error_strict : expected_error_weak)[Z_EXPECTED_STRING];
				break;
			case _IS_BOOL:
				*need_msg = "be a ";
				*need_kind = (strict ? expected_error_strict : expected_error_weak)[Z_EXPECTED_BOOL];
				break;
			case IS_OBJECT:
				*need_msg = "be an ";
				*need_kind = "object";
		...
		zend_bool strict, const char *error_adjective)
{
	...
	if (value) {
		zend_verify_type_error_common(
			zf, arg_info, ce, value, strict, error_adjective,
...
static zend_always_inline int zend_verify_arg_type(zend_function *zf, uint32_t arg_num, zval *arg, zval *default_value, void **cache_slot)
{
	...
	if (UNEXPECTED(!zend_check_type(cur_arg_info->type, arg, &ce, cache_slot, default_value, zf->common.scope, 0, &error_adjective))) {
		zend_verify_arg_error(zf, cur_arg_info, arg_num, ce, arg, ZEND_ARG_USES_STRICT_TYPES(), error_adjective);
		return 0;
	}
...
static ZEND_COLD void zend_verify_return_error(
		const zend_function *zf, const zend_class_entry *ce, zval *value, zend_bool strict, const char *error_adjective)
{
		...
		const zend_function *zf, const zend_class_entry *ce, zval *value, const char *error_adjective)
{
	...
	zend_verify_type_error_common(
		zf, arg_info, ce, value, 1, error_adjective,
		&fname, &fsep, &fclass, &need_msg, &need_kind, &need_or_null, &given_msg, &given_kind); 
...
static zend_always_inline void zend_verify_return_type(zend_function *zf, zval *ret, void **cache_slot)
{
	...
	if (UNEXPECTED(!zend_check_type(ret_info->type, ret, &ce, cache_slot, NULL, NULL, 1, &error_adjective))) {
		zend_verify_return_error(zf, ce, ret, ZEND_RET_USES_STRICT_TYPES(), error_adjective);
	}
}
...
static ZEND_COLD int zend_verify_missing_return_type(zend_function *zf, void **cache_slot)
{
	...
	if (ZEND_TYPE_IS_SET(ret_info->type) && UNEXPECTED(ZEND_TYPE_CODE(ret_info->type) != IS_VOID)) {
		...
		zend_verify_return_error(zf, ce, NULL, ZEND_RET_USES_STRICT_TYPES(), "");
		return 0;
	}
	return 1;
}
