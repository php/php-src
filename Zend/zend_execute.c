		...
		const zend_function *zf, const zend_arg_info *arg_info, uint32_t arg_num, zval *value)
{
	...
	zend_string_release_outline(need_msg);
}
...
ZEND_COLD static zend_result ZEND_FASTCALL get_deprecation_suffix_from_attribute(HashTable *attributes, zend_class_entry* scope, zend_string **message_suffix)
{
 ...
 out:
	...
	zend_string_release_outline(since);
...
ZEND_API ZEND_COLD void ZEND_FASTCALL zend_deprecated_function(const zend_function *fbc)
{
	...
	zend_string_release_outline(message_suffix);
}
...
ZEND_API ZEND_COLD zval* ZEND_FASTCALL zend_undefined_index_write(HashTable *ht, zend_string *offset)
{
	...
	zend_string_release_outline(offset);
	return retval;
}
...
ZEND_API ZEND_COLD void zend_throw_ref_type_error_type(const zend_property_info *prop1, const zend_property_info *prop2, const zval *zv) {
	...
	zend_string_release_outline(type1_str);
	zend_string_release_outline(type2_str);
}
	...
	if ((colon = zend_memrchr(ZSTR_VAL(function), ':', ZSTR_LEN(function))) != NULL &&
		...
		*(colon-1) == ':'
		...
		if (UNEXPECTED((func = zend_hash_find(EG(function_table), lcname)) == NULL)) {
			...
			zend_string_release_ex_outline(lcname, 0);
			return NULL;
		}
		zend_string_release_ex(lcname, 0);
