...
static void zend_generator_remove_child(zend_generator_node *node, zend_generator *child)
{
	...
	if (node->children == 1) {
		...
		zend_hash_index_del(ht, ZEND_PTR_TO_ZEND_ULONG(child));
		if (node->children == 2) {
	...
	if (node->children == 0) {
		...
		if (node->children == 1) {
			...
			zend_hash_index_add_new_ptr(ht,
				ZEND_PTR_TO_ZEND_ULONG(node->child.single), node->child.single);
			node->child.ht = ht;
		}

		zend_hash_index_add_new_ptr(node->child.ht, ZEND_PTR_TO_ZEND_ULONG(child), child);
	}
