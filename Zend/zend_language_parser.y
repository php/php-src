...
reserved_non_modifiers:
    ...
    | T_THROW | T_USE | T_FROM | T_INSTEADOF | T_GLOBAL | T_VAR | T_UNSET | T_ISSET | T_EMPTY | T_CONTINUE | T_GOTO
...
group_use_declaration:
    ...
    |   '{' unprefixed_use_declarations possible_comma '}' T_FROM legacy_namespace_name
            { $$ = zend_ast_create(ZEND_AST_GROUP_USE, $6, $2); }
;
...
;
    ...
    |   '{' inline_use_declarations possible_comma '}' T_FROM legacy_namespace_name
                { $$ = zend_ast_create(ZEND_AST_GROUP_USE, $6, $2); }
...
use_declaration:
    ...
    |   T_NAME_FULLY_QUALIFIED
            { $$ = zend_ast_create(ZEND_AST_USE_ELEM, $1, NULL); }
    |   T_NAME_FULLY_QUALIFIED T_AS T_STRING
            { $$ = zend_ast_create(ZEND_AST_USE_ELEM, $1, $3); }
    |   namespace_name T_FROM legacy_namespace_name {
                                zend_string *s = zend_string_concat3(
                                        ZSTR_VAL(zend_ast_get_str($3)), ZSTR_LEN(zend_ast_get_str($3)),
                                        "\\", 1,
                                        ZSTR_VAL(zend_ast_get_str($1)), ZSTR_LEN(zend_ast_get_str($1)));
                                zend_ast *name = zend_ast_create_zval_from_str(s);
                zend_ast_destroy($1);
                zend_ast_destroy($3);
                $$ = zend_ast_create(ZEND_AST_USE_ELEM, name, NULL);
            }
    |   namespace_name T_FROM legacy_namespace_name T_AS T_STRING {
                                zend_string *s = zend_string_concat3(
                                        ZSTR_VAL(zend_ast_get_str($3)), ZSTR_LEN(zend_ast_get_str($3)),
                                        "\\", 1,
                                        ZSTR_VAL(zend_ast_get_str($1)), ZSTR_LEN(zend_ast_get_str($1)));
                ...
                zend_ast_destroy($1);
                zend_ast_destroy($3);
                $$ = zend_ast_create(ZEND_AST_USE_ELEM, name, $5);
            }
    |   namespace_name T_AS T_STRING T_FROM legacy_namespace_name {
                            zend_string *s = zend_string_concat3(
                                    ZSTR_VAL(zend_ast_get_str($5)), ZSTR_LEN(zend_ast_get_str($5)),
                                    "\\", 1,
                                    ZSTR_VAL(zend_ast_get_str($1)), ZSTR_LEN(zend_ast_get_str($1)));
                            zend_ast *name = zend_ast_create_zval_from_str(s);
                            zend_ast_destroy($1);
                            zend_ast_destroy($5);
                            $$ = zend_ast_create(ZEND_AST_USE_ELEM, name, $3);
                        }
;
