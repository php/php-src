...
static int parse_arg_object_to_string(zval **arg, char **p, zend_str_size_int *pl, int type TSRMLS_DC) /* {{{ */
{
...
#endif
		case 'P': /* New zend_str_size size */
		case 'S': /* New zend_str_size size */
			{
				char **p = va_arg(*va, char **);
				zend_str_size pl;
				zend_str_size *plsize;
				int *plint;

				if (c == 'p' || c == 's') {
					plint = va_arg(*va, int *);
				} else {
					plsize = va_arg(*va, zend_str_size *);
				}
				switch (Z_TYPE_PP(arg)) {
					case IS_NULL:
						if (check_null) {
...
ZEND_API int add_index_long(zval *arg, zend_uint_t index, zend_int_t n) /* {{{ */
{
	zval *tmp;
...
ZEND_API int add_index_resource(zval *arg, zend_uint_t index, int r) /* {{{ */
{
	zval *tmp;
...
ZEND_API int add_index_string(zval *arg, zend_uint_t index, const char *str, int duplicate) /* {{{ */
{
	zval *tmp;
...
ZEND_API int add_index_stringl(zval *arg, zend_uint_t index, const char *str, zend_str_size_uint length, int duplicate) /* {{{ */
{
	zval *tmp;
...
ZEND_API int add_get_index_long(zval *arg, zend_uint_t index, zend_int_t l, void **dest) /* {{{ */
{
	zval *tmp;
...
ZEND_API int add_get_index_double(zval *arg, zend_uint_t index, double d, void **dest) /* {{{ */
{
	zval *tmp;
...
ZEND_API int add_get_index_string(zval *arg, zend_uint_t index, const char *str, void **dest, int duplicate) /* {{{ */
{
	zval *tmp;
...
ZEND_API int add_get_index_stringl(zval *arg, zend_uint_t index, const char *str, zend_str_size_uint length, void **dest, int duplicate) /* {{{ */
{
	zval *tmp;
...
ZEND_API int array_set_zval_key(HashTable *ht, zval *key, zval *value) /* {{{ */
{
	...
	switch (Z_TYPE_P(key)) {
		...
		case IS_NULL:
			...
			break;
		case IS_RESOURCE:
			zend_error(E_STRICT, "Resource ID#" ZEND_INT_FMT " used as offset, casting to integer (" ZEND_INT_FMT ")", Z_LVAL_P(key), Z_LVAL_P(key));
...
ZEND_API int add_property_long_ex(zval *arg, const char *key, zend_str_size_uint key_len, zend_int_t n TSRMLS_DC) /* {{{ */
{
	zval *tmp;
	zval *z_key;
...
ZEND_API int add_property_resource_ex(zval *arg, const char *key, zend_str_size_uint key_len, zend_int_t n TSRMLS_DC) /* {{{ */
{
	zval *tmp;
	zval *z_key;
...
ZEND_API int zend_register_functions(zend_class_entry *scope, const zend_function_entry *functions, HashTable *function_table, int type TSRMLS_DC) /* {{{ */
{
	...
	zend_str_size fname_len;
	const char *lc_class_name = NULL;
	zend_str_size class_name_len = 0;
	zend_uint_t hash;
...
static int zend_is_callable_check_class(const char *name, zend_str_size_int name_len, zend_fcall_info_cache *fcc, int *strict_class, char **error TSRMLS_DC) /* {{{ */
{
	int ret = 0;
	zend_class_entry **pce;
	char *lcname = zend_str_tolower_dup(name, name_len);
...
{
	zend_bool ret;
	zend_str_size callable_name_len_local;
...
ZEND_API int zend_declare_property_ex(zend_class_entry *ce, const char *name, zend_str_size_int name_length, zval *property, int access_type, const char *doc_comment, zend_str_size_int doc_comment_len TSRMLS_DC) /* {{{ */
{
	...
	zend_uint_t h = zend_get_hash_value(name, name_length+1);
	...
	switch (access_type & ZEND_ACC_PPP_MASK) {
		case ZEND_ACC_PRIVATE: {
				char *priv_name;
				zend_str_size priv_name_length;
		...
		case ZEND_ACC_PROTECTED: {
				char *prot_name;
				zend_str_size prot_name_length;
...
{
	zval *property;
	zend_str_size len = strlen(value);
...
ZEND_API void zend_update_property_stringl(zend_class_entry *scope, zval *object, const char *name, zend_str_size_int name_length, const char *value, zend_str_size_int value_len TSRMLS_DC) /* {{{ */
{
	zval *tmp;
...
ZEND_API int zend_update_static_property_stringl(zend_class_entry *scope, const char *name, zend_str_size_int name_length, const char *value, zend_str_size_int value_len TSRMLS_DC) /* {{{ */
{
	zval *tmp;
