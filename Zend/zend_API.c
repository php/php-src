	...
	if ((colon = zend_memrchr(ZSTR_VAL(callable), ':', ZSTR_LEN(callable))) != NULL &&
		...
		*(colon-1) == ':'
		...
		if (ce_org && !instanceof_function(ce_org, fcc->calling_scope)) {
			if (error) zend_spprintf(error, 0, "class %pS is not a subclass of %pS", ce_org->name, fcc->calling_scope->name);
			return 0;
		}
		...
		if (error) {
			zend_spprintf(error, 0, "function \"%pS\" not found or invalid function name", callable);
		}
		return 0;
	}
	...
	if (retval) {
		if (fcc->calling_scope && !call_via_handler) {
			if (fcc->function_handler->common.fn_flags & ZEND_ACC_ABSTRACT) {
				retval = false;
				if (error) {
					zend_spprintf(error, 0, "cannot call abstract method %pS::%pS()", fcc->calling_scope->name, fcc->function_handler->common.function_name);
				}
			} else if (!fcc->object && !(fcc->function_handler->common.fn_flags & ZEND_ACC_STATIC)) {
				retval = false;
				if (error) {
					zend_spprintf(error, 0, "non-static method %pS::%pS() cannot be called statically", fcc->calling_scope->name, fcc->function_handler->common.function_name);
				}
			}
			if (retval
			 && !(fcc->function_handler->common.fn_flags & ZEND_ACC_PUBLIC)) {
				...
				if (!zend_check_method_accessible(fcc->function_handler, scope)) {
					if (error) {
						...
						zend_spprintf(error, 0, "cannot access %s method %pS::%pS()", zend_visibility_string(fcc->function_handler->common.fn_flags), fcc->calling_scope->name, fcc->function_handler->common.function_name);
					}
					retval = false;
				}
			}
		...
		if (fcc->calling_scope) {
			zend_spprintf(error, 0, "class %pS does not have a method \"%pS\"", fcc->calling_scope->name, mname);
		} else {
			zend_spprintf(error, 0, "function %pS() does not exist", mname);
		}
	}
