...
static int parse_arg_object_to_string(zval **arg, char **p, zend_size_t *pl, int type TSRMLS_DC) /* {{{ */
{
...
ZEND_API int add_index_stringl(zval *arg, zend_uint_t index, const char *str, zend_size_t length, int duplicate) /* {{{ */
{
	zval *tmp;
...
ZEND_API int add_get_index_int(zval *arg, zend_uint_t index, zend_int_t l, void **dest) /* {{{ */
{
	zval *tmp;
...
ZEND_API int add_get_index_stringl(zval *arg, zend_uint_t index, const char *str, zend_size_t length, void **dest, int duplicate) /* {{{ */
{
	zval *tmp;
...
ZEND_API int array_set_zval_key(HashTable *ht, zval *key, zval *value) /* {{{ */
{
	...
	switch (Z_TYPE_P(key)) {
		...
		case IS_NULL:
			...
			break;
		case IS_RESOURCE:
			zend_error(E_STRICT, "Resource ID#" ZEND_INT_FMT " used as offset, casting to integer (" ZEND_INT_FMT ")", Z_IVAL_P(key), Z_IVAL_P(key));
			/* break missing intentionally */
		case IS_BOOL:
		case IS_INT:
			result = zend_hash_index_update(ht, Z_IVAL_P(key), &value, sizeof(zval *), NULL);
			break;
		case IS_DOUBLE:
			result = zend_hash_index_update(ht, zend_dval_to_ival(Z_DVAL_P(key)), &value, sizeof(zval *), NULL);
			break;
		default:
...
ZEND_API int add_property_int_ex(zval *arg, const char *key, zend_size_t key_len, zend_int_t n TSRMLS_DC) /* {{{ */
{
	zval *tmp;
	zval *z_key;
...
ZEND_API int add_property_resource_ex(zval *arg, const char *key, zend_size_t key_len, zend_int_t n TSRMLS_DC) /* {{{ */
{
	zval *tmp;
	zval *z_key;
...
ZEND_API int zend_register_functions(zend_class_entry *scope, const zend_function_entry *functions, HashTable *function_table, int type TSRMLS_DC) /* {{{ */
{
	...
	zend_size_t fname_len;
	const char *lc_class_name = NULL;
	zend_size_t class_name_len = 0;
	zend_uint_t hash;
...
static int zend_is_callable_check_class(const char *name, zend_size_t name_len, zend_fcall_info_cache *fcc, int *strict_class, char **error TSRMLS_DC) /* {{{ */
{
	int ret = 0;
	zend_class_entry **pce;
	char *lcname = zend_str_tolower_dup(name, name_len);
...
{
	zend_bool ret;
	zend_size_t callable_name_len_local;
...
ZEND_API void zend_update_property_stringl(zend_class_entry *scope, zval *object, const char *name, zend_size_t name_length, const char *value, zend_size_t value_len TSRMLS_DC) /* {{{ */
{
	zval *tmp;
...
ZEND_API int zend_update_static_property_stringl(zend_class_entry *scope, const char *name, zend_size_t name_length, const char *value, zend_size_t value_len TSRMLS_DC) /* {{{ */
{
	zval *tmp;
