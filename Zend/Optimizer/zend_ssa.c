...
//			long val = 0;
			...
			int var = EX_VAR_TO_NUM((opline-1)->op1.var);
			zend_string *class_name = Z_STR_P(CRT_CONSTANT_EX(op_array, (opline-1), (opline-1)->op2));
			zend_class_entry *ce = zend_optimizer_get_class_entry(script, op_array, class_name);
			if (!ce) {
				continue;
			}
