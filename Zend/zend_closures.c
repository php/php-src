...
ZEND_API zend_function *zend_get_closure_invoke_method(zend_object *object) /* {{{ */
{
	...
	invoke->internal_function.function_name = zend_known_strings[ZEND_STR_MAGIC_INVOKE];
	return invoke;
}
/* }}} */
...
static HashTable *zend_closure_get_debug_info(zval *object, int *is_temp) /* {{{ */
{
	...
	*is_temp = 1;
	...
	if (closure->func.type == ZEND_USER_FUNCTION && closure->func.op_array.static_variables) {
		...
		zend_hash_update(debug_info, zend_known_strings[ZEND_STR_STATIC], &val);
	}

	if (Z_TYPE(closure->this_ptr) != IS_UNDEF) {
		Z_ADDREF(closure->this_ptr);
		zend_hash_update(debug_info, zend_known_strings[ZEND_STR_THIS], &closure->this_ptr);
	}
