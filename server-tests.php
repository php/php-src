...
class HTTPRequest
{
    ...
    post data is, well, post data.  It is not processed so
        ...
        (or add it to class)
    */
    public function HTTPRequest($URL, $headers=array(), $options=array(), $postdata=null)
    {
...
class testHarness
{
    ...
    public function testHarness()
    {
        ...
        $this->contextHeader();
        if ($this->conf['TEST_CONTEXT_INFO']) {
            return;
        }
        $this->loadFileList();
        $this->moveTestFiles();
    ...
    public function runscript($script, $removeheaders=false, $cwd=null)
    {
        if ($this->conf['TEST_WEB']) {
            $pi = '/testscript.' . $this->conf['TEST_WEB_EXT'];
            if (!$cwd) {
                $cwd = $this->conf['TEST_BASE_PATH'];
            }
            $tmp_file = "$cwd$pi";
    ...
    public function showstatus($item, $status, $reason = '')
    {
        switch ($status) {
        case 'PASSED':
    ...
    public function removeSensitiveEnvVars()
    {
    ...
    public function setEnvConfigVar($name)
    {
...
//		}
    ...
    public function findFilesInDir($dir, $is_ext_dir=false, $ignore=false)
    {
        $skip = array('.', '..', 'CVS');
    ...
    public function getINISettings(&$section_text)
    {
    ...
    public function getINIParams(&$section_text)
    {
        if (!$section_text) {
            return '';
        }
    ...
    public function calculateDocumentRoot()
    {
    ...
    public function evalSettings($filename, $data)
    {
    ...
    public function getENVSettings(&$section_text, $testfile)
    {
        $env = $this->env;
    ...
    public function getEvalTestSettings($section_text, $testfile)
    {
        $rq = array();
        // Any special environment settings
        // these may overwrite the test defaults...
    ...
    //
    public function getSectionText($file)
    {
        // Load the sections of the test file.
        $section_text = array(
            'TEST'   => '(unnamed test)',
    ...
    //
    public function getSkipReason($file, &$section_text, $docgi=false)
    {
    ...
    */
    ...
    //
    public function error_report($testname, $logname, $tested)
    {
