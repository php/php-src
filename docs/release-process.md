...
4. Follow all steps to the letter.
   ...
   > `web-php` and `web-qa` repositories, try to have someone from the webmaster
   > team on hand.
...
11. Wait for the [Release workflow][] to finish running. When it has finished,
    there should be a new release entry for your tag on the [GitHub releases page][].
    For non-stable releases, it should show the "Pre-release" label.

    Check to make sure the release has the correct release notes and all the
    assets are available. You should see the following assets listed with the
    release:

    ```
    php-8.y.zRC1.manifest
    php-8.y.zRC1.tar.bz2
    php-8.y.zRC1.tar.bz2.asc
    php-8.y.zRC1.tar.gz
    php-8.y.zRC1.tar.gz.asc
    php-8.y.zRC1.tar.xz
    php-8.y.zRC1.tar.xz.asc
    ```

12. Once the release is tagged, contact the release-managers@ distribution
    ...
    Tag: php-8.1.6RC1
    Tarballs: https://github.com/php/php-src/releases/tag/php-8.1.6RC1
    Manifest: https://github.com/php/php-src/releases/download/php-8.1.6RC1/php-8.1.6RC1.manifest
...
4. Send *separate* announcement emails to:
   ...
   * `primary-qa-tester@lists.php.net`
   ...
   include the manifest generated by the [Release workflow][].
...
10. Wait for the [Release workflow][] to finish running. When it has finished,
    there should be a new release entry for your tag on the [GitHub releases page][].
    ...
    php-8.y.z.manifest
    php-8.y.z.tar.bz2
    php-8.y.z.tar.bz2.asc
    php-8.y.z.tar.gz
    php-8.y.z.tar.gz.asc
    php-8.y.z.tar.xz
    php-8.y.z.tar.xz.asc
    ```

11. Once the release is tagged, contact the release-managers@ distribution
    list so that Windows binaries can be created. Once those are made, they may
    be found at https://windows.php.net/qa/.
    ...
    Tag: php-8.1.6
    Tarballs: https://github.com/php/php-src/releases/tag/php-8.1.6
    Manifest: https://github.com/php/php-src/releases/download/php-8.1.6/php-8.1.6.manifest
...
7. 🚨 **Before sending announcement emails, check to make sure the websites have
   ...
   * Make sure the tarballs are available from, e.g.,
     `https://github.com/php/php-src/releases/tag/php-X.Y.Z`
...
8. Please note the sha256 hashes and the PGP signatures found in the `.manifest`
   file. These *must* be included in the release mail.
...
release and tag from GitHub and go through the full packaging process again, as
described above.
...
1. Tag the patch-level version, and push the tag. This will kick off the
   [Release workflow][] to build the release packages and create a release on
   GitHub.

   ```shell
   git tag -s -u YOURKEYID php-X.Y.Zpl1 -m "Tag for php-X.Y.Zpl1"
   git push upstream php-X.Y.Zpl1
   ```

2. Update $data['X.Y'] in `web-php:/include/version.inc`
   (X.Y=major.minor release, e.g. '8.0'):

    * `version` to the full version number (e.g. '8.0.1pl1')
...
3. Create a [GPG key][] for your @php.net address.
   ...
   Add your public key to `php-keyring.gpg` in `web-php`. To do this, you will
   need to import all keys from the current PHP keyring file to your local GPG
   keyring. You will need to take note of the key IDs for each of the release
   managers listed in `php-keyring.gpg`. Then, you will export, specifying your
   key ID in addition to the key IDs of every other release manager. Save this
   export back to `php-keyring.gpg`, commit the changes, and push.

   ```shell
   cd /path/to/repos/php/web-php
   gpg --show-keys --keyid-format short php-keyring.gpg  # lists all keys in the keyring
   ...
   gpg --export \
       ...
       > php-keyring.gpg
   gpg --show-keys php-keyring.gpg  # verify all the keys are present, including yours
   git add -p
...
[release workflow]: https://github.com/php/php-src/actions/workflows/release.yml
[GitHub releases page]: https://github.com/php/php-src/releases
