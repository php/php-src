	...
	if (strcasecmp(filtername, "bzip2.decompress") == 0) {
		...
		if (filterparams) {
			...
			if (Z_TYPE_P(filterparams) == IS_ARRAY || Z_TYPE_P(filterparams) == IS_OBJECT) {
				if ((tmpzval = zend_hash_str_find(HASH_OF(filterparams), "concatenated", strlen("concatenated")))) {
					data->expect_concatenated = zend_is_true(tmpzval);
					tmpzval = NULL;
				}

				tmpzval = zend_hash_str_find(HASH_OF(filterparams), "small", strlen("small"));
			} else {
		...
		if (filterparams) {
			...
			if (Z_TYPE_P(filterparams) == IS_ARRAY || Z_TYPE_P(filterparams) == IS_OBJECT) {
				if ((tmpzval = zend_hash_str_find(HASH_OF(filterparams), "blocks", strlen("blocks")))) {
				...
				if ((tmpzval = zend_hash_str_find(HASH_OF(filterparams), "work", strlen("work")))) {
					/* Work Factor (0 - 250) */
