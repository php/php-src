...
static inline int accel_is_inactive(void)
{
	...
	if (ZCG(accel_directives).force_restart_timeout
		...
		zend_accel_error(ACCEL_LOG_WARNING, "Forced restart at %ld (after " ZEND_LONG_FMT " seconds), locked by %d", (long)time(NULL), (zend_long)ZCG(accel_directives).force_restart_timeout, mem_usage_check.l_pid);
