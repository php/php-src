...
int zend_optimizer_lookup_cv(zend_op_array *op_array, zend_string* name)
{
	...
	op_array->vars[i] = ZSTR_DUP(name, 0);
		...
		if (Z_TYPE_P(val) == IS_STRING) {
			switch (opline->opcode) {
				...
				case ZEND_NEW:
					...
					ZSTR_HASH(Z_STR(ZEND_OP1_LITERAL(opline)));
					...
					ZSTR_HASH(Z_STR(op_array->literals[opline->op1.constant+1]));
					break;
				default:
					opline->op1.constant = zend_optimizer_add_literal(op_array, val);
					ZSTR_HASH(Z_STR(ZEND_OP1_LITERAL(opline)));
                                     ...
                                     zval          *val)
{
	...
	if (opline->opcode == ZEND_INIT_FCALL) {
		...
		ZSTR_HASH(Z_STR(ZEND_OP2_LITERAL(opline)));
	...
	if (Z_TYPE_P(val) == IS_STRING) {
		...
		switch (opline->opcode) {
			...
			case ZEND_INSTANCEOF:
				...
				zend_str_tolower(Z_STRVAL_P(val), Z_STRLEN_P(val));
				zend_optimizer_add_literal(op_array, val);
				ZSTR_HASH(Z_STR(op_array->literals[opline->op2.constant+1]));
				break;
