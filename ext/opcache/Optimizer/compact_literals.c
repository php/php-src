					...
					if (opline->op1_type == IS_CONST) {
						...
						if (opline->op2_type == IS_CONST) {
							opline->extended_value = add_static_slot(&hash, op_array,
								...
								opline->op1.constant,
								LITERAL_STATIC_PROPERTY,
								&cache_size) | (opline->extended_value & (ZEND_FETCH_REF|ZEND_ISEMPTY));
						} else {
							opline->extended_value = cache_size | (opline->extended_value & (ZEND_FETCH_REF|ZEND_ISEMPTY));
							cache_size += 3 * sizeof(void *);
						}
					} else if (opline->op2_type == IS_CONST) {
						// op2 class
						if (class_slot[opline->op2.constant] >= 0) {
							opline->extended_value = class_slot[opline->op2.constant] | (opline->extended_value & (ZEND_FETCH_REF|ZEND_ISEMPTY));
						} else {
							opline->extended_value = cache_size | (opline->extended_value & (ZEND_FETCH_REF|ZEND_ISEMPTY));
							class_slot[opline->op2.constant] = cache_size;
							cache_size += sizeof(void *);
