...
#ifdef ZEND_JIT_IR
static void ZEND_FASTCALL zend_jit_undefined_offset(zend_execute_data *execute_data)
{
	const zend_op *opline = EX(opline);
	zval *op2;
	...
	ZEND_ASSERT(Z_TYPE_P(op2) == IS_LONG);
	zend_error(E_WARNING, "Undefined array key " ZEND_LONG_FMT, Z_LVAL_P(op2));
}

static void ZEND_FASTCALL zend_jit_undefined_key(zend_execute_data *execute_data)
{
	const zend_op *opline = EX(opline);
	zval *op2;
	...
	ZEND_ASSERT(Z_TYPE_P(op2) == IS_STRING);
	zend_error(E_WARNING, "Undefined array key \"%s\"", Z_STRVAL_P(op2));
}

static void ZEND_FASTCALL zend_jit_cannot_add_element(zend_execute_data *execute_data)
{
	const zend_op *opline = EX(opline);
