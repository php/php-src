...
#if ZEND_VM_TAIL_CALL_DISPATCH
# define IR_OPCODE_HANDLER_FUNC IR_PRESERVE_NONE_FUNC
#else
...
{
#if defined(IR_TARGET_X86) || ZEND_VM_TAIL_CALL_DISPATCH
	if (!IR_IS_CONST_REF(handler)) {
		handler = ir_CAST_OPCODE_HANDLER_FUNC(handler);
	}
#endif
	if (GCC_GLOBAL_REGS) {
		ir_TAILCALL(IR_VOID, handler);
	} else {
...
static int zend_jit_handler(zend_jit_ctx *jit, const zend_op *opline, int may_throw)
{
	...
	if (zend_jit_vm_kind == ZEND_VM_KIND_HYBRID || ZEND_VM_TAIL_CALL_DISPATCH) {
	...
	if (GCC_GLOBAL_REGS) {
		...
		/* zend_vm_opcode_handler_*func*_t is FASTCALL */
