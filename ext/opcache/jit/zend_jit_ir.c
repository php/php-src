		...
		if (insn->op == IR_LOAD) {
			...
			if (insn->op2 == addr) {
				if (type2 == type) {
					...
					return ir_emit1(ctx, IR_OPT(IR_TRUNC, type), insn->op3); /* partial store forwarding (S2L) */
				} else {
					return IR_UNUSED;
				}
...
static void zend_jit_snapshot(zend_jit_ctx *jit)
{
	 ...
	 && jit->ra) {
		...
		if (stack_size) {
			...
			if (snapshot_size) {
				...
				snapshot = ir_emit_N(&jit->ctx, IR_SNAPSHOT, snapshot_size + 1);
...
static ir_ref zend_jit_mem_zval_addr(zend_jit_ctx *jit, zend_jit_addr addr)
{
	if (Z_MODE(addr) == IS_REF_ZVAL) {
		...
		return ir_fold2(&jit->ctx, IR_OPT(IR_ADD, IR_ADDR), zend_jit_fp(jit),
...
static ir_ref zend_jit_zval_type(zend_jit_ctx *jit, zend_jit_addr addr)
{
	if (Z_MODE(addr) == IS_CONST_ZVAL) {
		...
		return zend_jit_load(jit, IR_U8,
			ir_fold2(&jit->ctx, IR_OPT(IR_ADD, IR_ADDR), zend_jit_fp(jit),
...
//					if_not_zero = ir_emit2(&jit->ctx, IR_IF, jit->control,
//						ir_fold2(&jit->ctx, IR_OPT(IR_NE, IR_BOOL), ref2, ir_const_u32(&jit->ctx, 0)));
...
//???|4:
		if (if_refcounted && if_not_zero) {
			ir_ref ref1, ref2;
			ref1 = ir_emit1(&jit->ctx, IR_END, jit->control);
			jit->control = ir_emit1(&jit->ctx, IR_IF_TRUE, if_not_zero);
			ref2 = ir_emit1(&jit->ctx, IR_END, jit->control);
			jit->control = ir_emit1(&jit->ctx, IR_IF_FALSE, if_refcounted);
			jit->control = ir_emit1(&jit->ctx, IR_END, jit->control);
			jit->control = ir_emit3(&jit->ctx, IR_MERGE, ref1, ref2, jit->control);
			jit->ctx.ir_base[jit->control].inputs_count = 3; //???
		} else if (if_not_zero) {
			ir_ref ref1;
			ref1 = ir_emit1(&jit->ctx, IR_END, jit->control);
			jit->control = ir_emit1(&jit->ctx, IR_IF_TRUE, if_not_zero);
			jit->control = ir_emit1(&jit->ctx, IR_END, jit->control);
			jit->control = ir_emit2(&jit->ctx, IR_MERGE, ref1, jit->control);
		} else if (if_refcounted) {
			ir_ref ref1;
			ref1 = ir_emit1(&jit->ctx, IR_END, jit->control);
			jit->control = ir_emit1(&jit->ctx, IR_IF_FALSE, if_refcounted);
			jit->control = ir_emit1(&jit->ctx, IR_END, jit->control);
...
{
#ifdef ZTS
	return ir_fold2(&jit->ctx, IR_OPT(IR_ADD, IR_ADDR),
		zend_jit_tls(jit),
		zend_jit_const_addr(jit, EG_TLS_OFFSET(uninitialized_zval)));
#else
	return zend_jit_const_addr(jit, (uintptr_t)&EG(uninitialized_zval));
#endif
}
...
static int zend_jit_check_timeout(zend_jit_ctx *jit, const zend_op *opline, const void *exit_addr)
{
	...
	ref = ir_fold2(&jit->ctx, IR_OPT(IR_ADD, IR_ADDR),
		zend_jit_tls(jit),
		zend_jit_const_addr(jit, EG_TLS_OFFSET(vm_interrupt)));
#else
	ref = zend_jit_const_addr(jit, (uintptr_t)&EG(vm_interrupt));
#endif
...
static void zend_jit_check_exception(zend_jit_ctx *jit)
{
	...
	ref = ir_fold2(&jit->ctx, IR_OPT(IR_ADD, IR_ADDR),
		zend_jit_tls(jit),
		zend_jit_const_addr(jit, EG_TLS_OFFSET(exception)));
#else
...
static int zend_jit_exception_handler_undef_stub(zend_jit_ctx *jit)
{
	...
	ref = ir_fold2(&jit->ctx, IR_OPT(IR_ADD, IR_ADDR),
		zend_jit_tls(jit),
		zend_jit_const_addr(jit, EG_TLS_OFFSET(opline_before_exception)));
#else
	ref = zend_jit_const_addr(jit, (uintptr_t)&EG(opline_before_exception));
#endif
...
{
	ir_ref ref, if_timeout, timeout_path;
	...
	tls = zend_jit_tls(jit);
	zend_jit_store(jit,
		ir_fold2(&jit->ctx, IR_OPT(IR_ADD, IR_ADDR), tls, zend_jit_const_addr(jit, EG_TLS_OFFSET(vm_interrupt))),
		ir_const_u8(&jit->ctx, 0));
	ref = zend_jit_load(jit, IR_U8,
		ir_fold2(&jit->ctx, IR_OPT(IR_ADD, IR_ADDR), tls, zend_jit_const_addr(jit, EG_TLS_OFFSET(timed_out))));
#else
	zend_jit_store(jit,
		zend_jit_const_addr(jit, (uintptr_t)&EG(vm_interrupt)),
		ir_const_u8(&jit->ctx, 0));
	ref = zend_jit_load(jit, IR_U8,
		zend_jit_const_addr(jit, (uintptr_t)&EG(timed_out)));
#endif
...
//???		|1:
		...
		ref = ir_fold2(&jit->ctx, IR_OPT(IR_ADD, IR_ADDR),
			zend_jit_tls(jit),
			zend_jit_const_addr(jit, EG_TLS_OFFSET(current_execute_data)));
#else
		ref = zend_jit_const_addr(jit, (uintptr_t)&EG(current_execute_data));
#endif
...
static int zend_jit_free(zend_jit_ctx *jit)
{
	if (jit->name) {
...
static int zend_jit_bb_end(zend_jit_ctx *jit, int b)
{
	...
	if (bb->successors_count == 1) {
		...
		/* Use only the following successor of SWITCH */
...
static int zend_jit_cond_jmp(zend_jit_ctx *jit, const zend_op *next_opline, int target_block)
{
	...
	ref = ir_emit2(&jit->ctx, IR_OPT(IR_NE, IR_BOOL), ref, ir_const_u32(&jit->ctx, (uint32_t)(uintptr_t)next_opline));
#else
	if (zend_jit_vm_kind == ZEND_VM_KIND_HYBRID) {
		ref = zend_jit_ip(jit);
	} else {
	...
	ref = ir_emit2(&jit->ctx, IR_OPT(IR_NE, IR_BOOL), ref, zend_jit_const_addr(jit, (uintptr_t)next_opline));
#endif
...
static int zend_jit_set_cond(zend_jit_ctx *jit, const zend_op *next_opline, uint32_t var)
{
	...
	ref = ir_emit2(&jit->ctx, IR_OPT(IR_EQ, IR_BOOL), ref, ir_const_u32(&jit->ctx, (uint32_t)(uintptr_t)next_opline));
#else
	if (zend_jit_vm_kind == ZEND_VM_KIND_HYBRID) {
		ref = zend_jit_ip(jit);
	} else {
	...
	ref = ir_emit2(&jit->ctx, IR_OPT(IR_EQ, IR_BOOL), ref, zend_jit_const_addr(jit, (uintptr_t)next_opline));
#endif
...
static int zend_jit_defined(zend_jit_ctx *jit, const zend_op *opline, zend_uchar smart_branch_opcode, uint32_t target_label, uint32_t target_label2, const void *exit_addr)
{
	...
	ref2 = ir_fold2(&jit->ctx, IR_OPT(IR_ADD, IR_ADDR),
		zend_jit_tls(jit),
		zend_jit_const_addr(jit, EG_TLS_OFFSET(zend_constants)));
#else
	ref2 = zend_jit_const_addr(jit, (uintptr_t)&EG(zend_constants));
#endif
...
static int zend_jit_trace_opline_guard(zend_jit_ctx *jit, const zend_op *opline)
{
	...
	ref = ir_emit2(&jit->ctx, IR_OPT(IR_EQ, IR_BOOL), ref, ir_const_u32(&jit->ctx, (uint32_t)(uintptr_t)opline));
#else
	if (zend_jit_vm_kind == ZEND_VM_KIND_HYBRID) {
		ref = zend_jit_ip(jit);
	} else {
	...
	ref = ir_emit2(&jit->ctx, IR_OPT(IR_EQ, IR_BOOL), ref, zend_jit_const_addr(jit, (uintptr_t)opline));
#endif
...
static int zend_jit_init_fcall_guard(zend_jit_ctx *jit, uint32_t level, const zend_function *func, const zend_op *to_opline)
{
	...
	if (func->type == ZEND_USER_FUNCTION &&
		...
		// if (call->func.op_array.opcodes != opcodes) goto exit_addr;
		zend_jit_guard(jit,
			ir_fold2(&jit->ctx, IR_OPT(IR_EQ, IR_BOOL),
				zend_jit_load(jit, IR_ADDR,
					ir_fold2(&jit->ctx, IR_OPT(IR_ADD, IR_ADDR),
		...
		// if (call->func != func) goto exit_addr;
		zend_jit_guard(jit,
			ir_fold2(&jit->ctx, IR_OPT(IR_EQ, IR_BOOL),
				zend_jit_load(jit, IR_ADDR,
					ir_fold2(&jit->ctx, IR_OPT(IR_ADD, IR_ADDR),
...
#ifdef ZTS
			ir_ref addr = ir_fold2(&jit->ctx, IR_OPT(IR_ADD, IR_ADDR),
				zend_jit_tls(jit),
				zend_jit_const_addr(jit, EG_TLS_OFFSET(current_execute_data)));
#else
			ir_ref addr = zend_jit_const_addr(jit, (uintptr_t)&EG(current_execute_data));
#endif
...
//???	}
	...
	ref = ir_fold2(&jit->ctx, IR_OPT(IR_ADD, IR_ADDR),
		zend_jit_tls(jit),
		zend_jit_const_addr(jit, EG_TLS_OFFSET(jit_trace_num)));
#else
	ref = zend_jit_const_addr(jit, (uintptr_t)&EG(jit_trace_num));
#endif
