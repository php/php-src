--TEST--
OPcache PersistentStatic class blob survives dynamic method statics with JIT enabled
--EXTENSIONS--
opcache
--CONFLICTS--
server
...
file_put_contents(__DIR__ . '/persistent_static_007.php', <<<'PHP'
<?php
...
$logFile = __DIR__ . '/persistent_static_007.log';
...
@unlink(__DIR__ . '/persistent_static_007.log');

include 'php_cli_server.inc';
php_cli_server_start('-d opcache.enable=1 -d opcache.enable_cli=1 -d opcache.static_cache.persistent_size_mb=32 -d opcache.file_update_protection=0 -d opcache.jit=tracing -d opcache.jit_buffer_size=64M -d opcache.jit_hot_loop=0 -d opcache.jit_hot_func=2 -d opcache.jit_hot_return=0 -d opcache.jit_hot_side_exit=1');

echo file_get_contents('http://' . PHP_CLI_SERVER_ADDRESS . '/persistent_static_007.php?request=1');
echo file_get_contents('http://' . PHP_CLI_SERVER_ADDRESS . '/persistent_static_007.php?request=2');
echo file_get_contents('http://' . PHP_CLI_SERVER_ADDRESS . '/persistent_static_007.php?request=3');

?>
--CLEAN--
<?php
@unlink(__DIR__ . '/persistent_static_007.php');
@unlink(__DIR__ . '/persistent_static_007.log');
?>
--EXPECT--
1,7,6,21,1
