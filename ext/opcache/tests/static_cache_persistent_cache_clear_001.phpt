--TEST--
OPcache persistent_clear clears only the persistent cache API namespace
--EXTENSIONS--
opcache
--CONFLICTS--
server
--FILE--
<?php

file_put_contents(__DIR__ . '/persistent_cache_clear_001.php', <<<'PHP'
<?php

$action = $_GET['action'] ?? 'seed';

if ($action === 'seed') {
	OPcache\volatile_clear();
	OPcache\persistent_clear();
	var_dump(OPcache\volatile_store('local', 'volatile-value'));
	OPcache\persistent_store('shared', 'persistent-value');
	return;
}

if ($action === 'clear') {
	OPcache\persistent_clear();
	var_dump(OPcache\volatile_fetch('local', 'missing-volatile'));
	var_dump(OPcache\persistent_fetch('shared', 'missing-persistent'));
	return;
}

var_dump(OPcache\volatile_fetch('local', 'missing-volatile'));
var_dump(OPcache\persistent_fetch('shared', 'missing-persistent'));
PHP);

$php = getenv('TEST_PHP_EXECUTABLE');
if ($php) {
	$php = realpath(__DIR__ . '/../../../' . $php) ?: $php;
	putenv('TEST_PHP_EXECUTABLE=' . $php);
}

include 'php_cli_server.inc';
php_cli_server_start('-d opcache.enable=1 -d opcache.enable_cli=1 -d opcache.static_cache.volatile_size_mb=32 -d opcache.static_cache.persistent_size_mb=32 -d opcache.file_update_protection=0');

$base = 'http://' . PHP_CLI_SERVER_ADDRESS . '/persistent_cache_clear_001.php';
echo file_get_contents($base . '?action=seed');
echo file_get_contents($base . '?action=clear');
echo file_get_contents($base . '?action=read');

?>
--CLEAN--
<?php
@unlink(__DIR__ . '/persistent_cache_clear_001.php');
?>
--EXPECT--
bool(true)
string(14) "volatile-value"
string(18) "missing-persistent"
string(14) "volatile-value"
string(18) "missing-persistent"
