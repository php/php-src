--TEST--
OPcache volatile cache request-local lookup cache sees cross-thread updates on next fetch
--EXTENSIONS--
opcache
--SKIPIF--
<?php
...
function runScenario(string $binary, string $scenario, string $label): void
{
...
runScenario($binary, __DIR__ . '/helpers/volatile_cache_zts_threads_003_hit.inc', 'lookup_hit');
runScenario($binary, __DIR__ . '/helpers/volatile_cache_zts_threads_003_miss.inc', 'lookup_miss');
runScenario($binary, __DIR__ . '/helpers/volatile_cache_zts_threads_003_delete.inc', 'lookup_delete');
runScenario($binary, __DIR__ . '/helpers/volatile_cache_zts_threads_003_clear.inc', 'lookup_clear');
runScenario($binary, __DIR__ . '/helpers/volatile_cache_zts_threads_003_materialized_clear.inc', 'materialized_clear');
runScenario($binary, __DIR__ . '/helpers/volatile_cache_zts_threads_003_volatile_static_class.inc', 'volatile_static_class');

?>
--CLEAN--
<?php
@unlink(__DIR__ . '/helpers/volatile_cache_zts_threads_003.out');
@unlink(__DIR__ . '/helpers/volatile_cache_zts_threads_003.exe');
?>
--EXPECT--
lookup_hit: ok
lookup_miss: ok
lookup_delete: ok
lookup_clear: ok
materialized_clear: ok
volatile_static_class: ok
