--TEST--
OPcache User Cache: invalid cache keys are rejected consistently
--EXTENSIONS--
opcache
--INI--
opcache.enable=1
...
--FILE--
<?php
$cache = new Opcache\UserCache('invalid-keys');
...
function show_error(string $label, Closure $callback): void
{
    try {
        ...
        $message = preg_replace('/^Opcache\\\\UserCache::[^(]+\\(\\): /', '', $e->getMessage());
...
storeMultiple empty key: ValueError: Argument #1 ($values) must be an array with non-empty string keys that do not contain 0x1F
storeMultiple delimiter key: ValueError: Argument #1 ($values) must be an array with non-empty string keys that do not contain 0x1F
