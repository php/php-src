--TEST--
OPcache VolatileStatic tracking reference graphs are not republished after volatile_clear() or opcache_invalidate() in the same request under FPM
--EXTENSIONS--
opcache
--SKIPIF--
<?php
...
function tracking_clear_invalidate_fpm_request(FPM\Tester $tester, string $query): void
{
	echo $tester->request($query)->getBody(), "\n";
}

tracking_clear_invalidate_fpm_request($tester, 'action=reset');
foreach (['class', 'property', 'method'] as $kind) {
	tracking_clear_invalidate_fpm_request($tester, 'action=seed&kind=' . $kind);
	tracking_clear_invalidate_fpm_request($tester, 'action=read&kind=' . $kind);
	tracking_clear_invalidate_fpm_request($tester, 'action=clear_after_access&kind=' . $kind);
	tracking_clear_invalidate_fpm_request($tester, 'action=read&kind=' . $kind);
	tracking_clear_invalidate_fpm_request($tester, 'action=seed&kind=' . $kind);
	tracking_clear_invalidate_fpm_request($tester, 'action=read&kind=' . $kind);
	tracking_clear_invalidate_fpm_request($tester, 'action=reset_after_access&kind=' . $kind);
	tracking_clear_invalidate_fpm_request($tester, 'action=read&kind=' . $kind);
	tracking_clear_invalidate_fpm_request($tester, 'action=seed&kind=' . $kind);
	tracking_clear_invalidate_fpm_request($tester, 'action=read&kind=' . $kind);
	tracking_clear_invalidate_fpm_request($tester, 'action=invalidate_after_access&kind=' . $kind);
	tracking_clear_invalidate_fpm_request($tester, 'action=read&kind=' . $kind);
}
