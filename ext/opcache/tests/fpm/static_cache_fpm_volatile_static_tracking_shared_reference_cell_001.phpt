--TEST--
OPcache VolatileStatic tracking publishes shared reference-cell mutations before restore and detaches roots after restore under FPM
--EXTENSIONS--
opcache
--SKIPIF--
<?php
...
function tracking_shared_reference_fpm_request(FPM\Tester $tester, string $query): void
{
	echo $tester->request($query)->getBody(), "\n";
}

tracking_shared_reference_fpm_request($tester, 'action=reset');
foreach (['class', 'property', 'method', 'blob'] as $kind) {
	tracking_shared_reference_fpm_request($tester, 'action=seed&kind=' . $kind);
	tracking_shared_reference_fpm_request($tester, 'action=read&kind=' . $kind);
	tracking_shared_reference_fpm_request($tester, 'action=mutate&kind=' . $kind);
	tracking_shared_reference_fpm_request($tester, 'action=read&kind=' . $kind);
}
