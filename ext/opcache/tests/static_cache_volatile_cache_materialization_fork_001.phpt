--TEST--
OPcache volatile cache should isolate post-mutation object fetches across processes
--EXTENSIONS--
opcache
pcntl
--SKIPIF--
...
$readyFile = sys_get_temp_dir() . '/opcache_volatile_cache_materialization_plain_' . getmypid() . '.ready';
@unlink($readyFile);
...
if ($pid === 0) {
	...
	$secondFetch = OPcache\VolatileCache::getInstance('default')->fetch('materialization_plain_payload');

	var_dump($readOnly === $expectedText
		&& (!$canMeasureRequestAllocations || ($afterFetch - $before) < 262144));
	var_dump($fetched->rows[100]['text'] === 'changed'
		&& $secondFetch->rows[100]['text'] === $expectedText
		&& (!$canMeasureRequestAllocations || ($afterMutate - $afterFetch) > 131072));
	exit(0);
}

if (!OPcache\VolatileCache::getInstance('default')->store('materialization_plain_payload', new LargePayload(build_rows(), 'plain'))) {
...
--CLEAN--
<?php
foreach (glob(sys_get_temp_dir() . '/opcache_volatile_cache_materialization_plain_[0-9]*.ready') ?: [] as $path) {
	@unlink($path);
}
?>
