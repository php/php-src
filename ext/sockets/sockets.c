...
/* {{{ proto void socket_export_stream(resource socket)
...
PHP_FUNCTION(socket_export_stream)
{
	...
	if (socket->type == PF_INET
		...
		getsockopt(socket->bsd_socket, SOL_SOCKET, SO_PROTOCOL, &protoid, &protoidlen);

		if (protoid == IPPROTO_TCP) {
			protocol = "tcp";
			protocollen = 3;
		} else if (protoid == IPPROTO_UDP) {
			protocol = "udp";
			protocollen = 3;
		}
#ifdef PF_UNIX
	...
	if (stream == NULL) {
		...
		if (stream == NULL) {
			php_error_docref(NULL TSRMLS_CC, E_WARNING, "failed to create stream");
			RETURN_FALSE;
		}
	}
