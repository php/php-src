							   ...
							   zval **arg4 TSRMLS_DC)
{
	...
	case IP_MULTICAST_TTL:
		...
		if (Z_IVAL_PP(arg4) < 0L || Z_IVAL_PP(arg4) > 255L) {
			php_error_docref(NULL TSRMLS_CC, E_WARNING,
					"Expected a value between 0 and 255");
			return FAILURE;
		}
ipv4_loop_ttl:
		ipv4_mcast_ttl_lback = (unsigned char) Z_IVAL_PP(arg4);
								 ...
								 zval **arg4 TSRMLS_DC)
{
	...
	case IPV6_MULTICAST_HOPS:
		convert_to_int_ex(arg4);
		if (Z_IVAL_PP(arg4) < -1L || Z_IVAL_PP(arg4) > 255L) {
			php_error_docref(NULL TSRMLS_CC, E_WARNING,
					"Expected a value between -1 and 255");
			return FAILURE;
		}
ipv6_loop_hops:
		ov = (int) Z_IVAL_PP(arg4);
		opt_ptr = &ov;
		optlen	= sizeof(ov);
		goto dosockopt;
	}
