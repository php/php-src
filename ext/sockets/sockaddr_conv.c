...
bool php_set_common_addr(struct sockaddr *sin, int family, zend_string *string, php_socket *php_sock) /* {{{ */
{
	...
	if (family == AF_INET6 && inet_pton(AF_INET6, ZSTR_VAL(string), &tmp6)) {
		memcpy(&(sin6->sin6_addr.s6_addr), &(tmp6.s6_addr), sizeof(struct in6_addr));
	} else if (family == AF_INET && inet_pton(AF_INET, ZSTR_VAL(string), &tmp4)) {
