	...
	zend_string *subject_str, size_t limit, size_t *replace_count)
{
	...
	if (replace_ht) {
		...
		ZEND_ARRAY_FOREACH_VAL(regex, regex_entry) {
	...
	zend_string *subject, size_t limit, size_t *replace_count, zend_long flags)
{
	...
	if (regex_str) {
		...
		/* For each entry in the regex array, get the entry */
		ZEND_ARRAY_FOREACH_VAL(regex_ht, regex_entry) {
			/* Make sure we're dealing with strings. */
...
PHP_FUNCTION(preg_replace_callback_array)
{
	...
	ZEND_ARRAY_FOREACH_STR_KEY_VAL(pattern, str_idx_regex, replace) {
		if (!str_idx_regex) {
			php_error_docref(NULL, E_WARNING, "Delimiter must not be alphanumeric or backslash");
			RETVAL_NULL();
			goto error;
...
PHPAPI void  php_pcre_grep_impl(pcre_cache_entry *pce, zval *input, zval *return_value, zend_long flags) /* {{{ */
{
	...
	ZEND_ARRAY_FOREACH_KEY_VAL(Z_ARRVAL_P(input), num_key, string_key, entry) {
