...
#define SET_OPTIONAL_LONG_ARG(key, varname, defval)	\
	if (optional_args && zend_hash_find(Z_ARRVAL_P(optional_args), key, sizeof(key), (void**)&item) == SUCCESS && Z_TYPE_PP(item) == IS_INT) \
		varname = Z_IVAL_PP(item); \
	else \
		varname = defval
...
static int php_openssl_parse_config(struct php_x509_request * req, zval * optional_args TSRMLS_DC) /* {{{ */
{
	...
	if (req->priv_key_encrypt && optional_args && zend_hash_find(Z_ARRVAL_P(optional_args), "encrypt_key_cipher", sizeof("encrypt_key_cipher"), (void**)&item) == SUCCESS 
		&& Z_TYPE_PP(item) == IS_INT) {
		php_int_t cipher_algo = Z_IVAL_PP(item);
...
static X509 * php_openssl_x509_from_zval(zval ** val, int makeresource, php_int_t * resourceval TSRMLS_DC)
{
	...
	if (Z_STRSIZE_PP(val) > 7 && memcmp(Z_STRVAL_PP(val), "file://", sizeof("file://") - 1) == 0) {
		...
		in = BIO_new_mem_buf(Z_STRVAL_PP(val), Z_STRSIZE_PP(val));
		if (in == NULL) {
			return NULL;
		}
...
int php_openssl_x509_fingerprint(X509 *peer, const char *method, zend_bool raw, char **out, php_size_t *out_len TSRMLS_DC)
{
...
PHP_FUNCTION(openssl_x509_read)
{
	...
	x509 = php_openssl_x509_from_zval(cert, 1, &Z_IVAL_P(return_value) TSRMLS_CC);
...
PHP_FUNCTION(openssl_x509_free)
{
	...
	zend_list_delete(Z_IVAL_P(x509));
}
/* }}} */
...
PHP_FUNCTION(openssl_pkey_get_public)
{
	...
	Z_TYPE_P(return_value) = IS_RESOURCE;
	pkey = php_openssl_evp_from_zval(cert, 1, NULL, 1, &Z_IVAL_P(return_value) TSRMLS_CC);
...
PHP_FUNCTION(openssl_pkey_get_private)
{
	...
	EVP_PKEY *pkey;
	char * passphrase = "";
	php_size_t passphrase_len;

	if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "Z|S", &cert, &passphrase, &passphrase_len) == FAILURE) {
		return;
	}
	Z_TYPE_P(return_value) = IS_RESOURCE;
	pkey = php_openssl_evp_from_zval(cert, 0, passphrase, 1, &Z_IVAL_P(return_value) TSRMLS_CC);
...
PHP_FUNCTION(openssl_pkcs7_verify)
{
	...
	php_size_t extracerts_len = 0;
	char * signersfilename = NULL;
	php_size_t signersfilename_len = 0;
	char * datafilename = NULL;
	php_size_t datafilename_len = 0;
...
PHP_FUNCTION(openssl_pkcs7_encrypt)
{
	...
	php_size_t infilename_len;
	char * outfilename = NULL;
	php_size_t outfilename_len;
...
PHP_FUNCTION(openssl_pkcs7_sign)
{
	...
	php_size_t extracertsfilename_len;
...
PHP_FUNCTION(openssl_private_encrypt)
{
	...
	switch (pkey->type) {
		case EVP_PKEY_RSA:
		case EVP_PKEY_RSA2:
			successful = (RSA_private_encrypt(data_len, (unsigned char *)data, cryptedbuf, pkey->pkey.rsa, padding) == cryptedlen);
			break;
		default:
			php_error_docref(NULL TSRMLS_CC, E_WARNING, "key type not supported in this PHP build!");
	}
...
PHP_FUNCTION(openssl_private_decrypt)
{
	...
	switch (pkey->type) {
		case EVP_PKEY_RSA:
		case EVP_PKEY_RSA2:
			cryptedlen = RSA_private_decrypt(data_len, (unsigned char *)data, crypttemp, pkey->pkey.rsa, padding);
			if (cryptedlen > -1) {
				cryptedbuf = emalloc(cryptedlen + 1);
...
PHP_FUNCTION(openssl_public_encrypt)
{
	...
	switch (pkey->type) {
		case EVP_PKEY_RSA:
		case EVP_PKEY_RSA2:
			successful = (RSA_public_encrypt(data_len, (unsigned char *)data, cryptedbuf, pkey->pkey.rsa, padding) == cryptedlen);
			break;
		default:
			php_error_docref(NULL TSRMLS_CC, E_WARNING, "key type not supported in this PHP build!");
...
PHP_FUNCTION(openssl_public_decrypt)
{
	...
	switch (pkey->type) {
		case EVP_PKEY_RSA:
		case EVP_PKEY_RSA2:
			cryptedlen = RSA_public_decrypt(data_len, (unsigned char *)data, crypttemp, pkey->pkey.rsa, padding);
			if (cryptedlen != -1) {
				cryptedbuf = emalloc(cryptedlen + 1);
				memcpy(cryptedbuf, crypttemp, cryptedlen);
				successful = 1;
...
PHP_FUNCTION(openssl_open)
{
	...
	if (ekey_len > INT_MAX) {
		php_error_docref(NULL TSRMLS_CC, E_WARNING, "Envelope key is too long; it needs to be at most %d bytes, not " ZEND_UINT_FMT,
				INT_MAX, ekey_len);
		RETURN_FALSE;
	}
...
PHP_FUNCTION(openssl_digest)
{
	zend_bool raw_output = 0;
	char *data, *method;
	php_size_t data_len, method_len;
	const EVP_MD *mdtype;
	EVP_MD_CTX md_ctx;
	int siglen;
	unsigned char *sigbuf;
...
static zend_bool php_openssl_validate_iv(char **piv, php_size_t *piv_len, php_size_t iv_required_len TSRMLS_DC)
{
	char *iv_new;
...
PHP_FUNCTION(openssl_encrypt)
{
	...
	php_size_t data_len, method_len, password_len, iv_len = 0, max_iv_len;
...
PHP_FUNCTION(openssl_decrypt)
{
	php_int_t options = 0;
	char *data, *method, *password, *iv = "";
	php_size_t data_len, method_len, password_len, iv_len = 0;
	const EVP_CIPHER *cipher_type;
	EVP_CIPHER_CTX cipher_ctx;
	int i, outlen, keylen;
	unsigned char *outbuf, *key;
...
PHP_FUNCTION(openssl_dh_compute_key)
{
	...
	if (pub_len > INT_MAX) {
		php_error_docref(NULL TSRMLS_CC, E_WARNING, "Public key is too long; it needs to be at most %d bytes, not " ZEND_UINT_FMT,
				INT_MAX, pub_len);
		RETURN_FALSE;
	}
...
PHP_FUNCTION(openssl_random_pseudo_bytes)
{
	...
	if (buffer_length > INT_MAX) {
		php_error_docref(NULL TSRMLS_CC, E_WARNING, "Public key is too long; it needs to be at most %d bytes, not " ZEND_INT_FMT,
