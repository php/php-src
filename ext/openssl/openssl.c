...
static zend_result php_openssl_csr_add_subj_entry(zval *item, X509_NAME *subj, int nid)
{
				...
				(unsigned char*)ZSTR_VAL(str_item), -1, -1, 0)))
	{
		...
		zend_string_release_outline(str_item);
		return FAILURE;
	}
	...
	if (php_openssl_cipher_init(cipher_type, cipher_ctx, &mode,
		...
		if (mode.is_aead && tag) {
			...
			if (EVP_CIPHER_CTX_ctrl(cipher_ctx, mode.aead_get_tag_flag, tag_len, ZSTR_VAL(tag_str)) == 1) {
				...
				zend_string_release_ex_outline(tag_str, 0);
				zend_string_release_ex_outline(outbuf, 0);
				outbuf = NULL;
			}
		} else if (tag) {
...
PHP_OPENSSL_API zend_string* php_openssl_random_pseudo_bytes(zend_long buffer_length)
{
	...
	if (UNEXPECTED(RAND_bytes((unsigned char*)ZSTR_VAL(buffer), (int)buffer_length) <= 0)) {
		zend_string_release_ex_outline(buffer, 0);
