...
PHP_FUNCTION(finfo_file)
{
	...
	if (object) {
		if (zend_parse_parameters(ZEND_NUM_ARGS(), "P|lr!", &filepath, &options, &zcontext) == FAILURE) {
			RETURN_THROWS();
		}
		FILEINFO_FROM_OBJECT(finfo, object);
		magic = finfo->magic;
	} else {
		if (zend_parse_parameters(ZEND_NUM_ARGS(), "rP|lr!", &zfinfo, &filepath, &options, &zcontext) == FAILURE) {
			RETURN_THROWS();
		}
		if ((finfo = (php_fileinfo *)zend_fetch_resource(Z_RES_P(zfinfo), "file_info", le_fileinfo)) == NULL) {
			RETURN_THROWS();
	...
	if (ZSTR_LEN(filepath) == 0) {
		zend_argument_value_error(object ? 1 : 2, "cannot be empty");
		RETURN_THROWS();
	}
...
PHP_FUNCTION(finfo_buffer)
{
	...
	if (object) {
		if (zend_parse_parameters(ZEND_NUM_ARGS(), "S|lr!", &buffer, &options, &zcontext) == FAILURE) {
			RETURN_THROWS();
		}
		FILEINFO_FROM_OBJECT(finfo, object);
	} else {
		if (zend_parse_parameters(ZEND_NUM_ARGS(), "rS|lr!", &zfinfo, &buffer, &options, &zcontext) == FAILURE) {
			RETURN_THROWS();
		}
		if ((finfo = (php_fileinfo *)zend_fetch_resource(Z_RES_P(zfinfo), "file_info", le_fileinfo)) == NULL) {
			RETURN_THROWS();
