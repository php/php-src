--TEST--
PDO PgSQL PDOStatement::getAttribute(Pdo\Pgsql::ATTR_RESULT_MEMORY_SIZE)
--EXTENSIONS--
pdo_pgsql
--SKIPIF--
<?php
...
PDOTest::skip();
if (!defined('Pdo\Pgsql::ATTR_RESULT_MEMORY_SIZE')) die('skip constant Pdo\Pgsql::ATTR_RESULT_MEMORY_SIZE does not exist');
--FILE--
<?php
...
$result_1 = $statement->getAttribute(Pdo\Pgsql::ATTR_RESULT_MEMORY_SIZE);
var_dump($result_1);

echo 'Result set with many rows: ';
$result = $db->query('select generate_series(1, 10000)');
$result_2 = $result->getAttribute(Pdo\Pgsql::ATTR_RESULT_MEMORY_SIZE);
var_dump($result_2);
...
$result_3 = $statement->getAttribute(Pdo\Pgsql::ATTR_RESULT_MEMORY_SIZE);
var_dump($result_3);
