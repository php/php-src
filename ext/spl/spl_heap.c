...
static bool spl_heap_object_allows_user_cache_state(zend_class_entry *ce)
{
...
static bool spl_heap_object_user_cache_is_pqueue(zend_class_entry *ce)
{
	return instanceof_function(ce, spl_ce_SplPriorityQueue);
}

static void spl_ptr_heap_user_cache_ensure_capacity(spl_ptr_heap *heap, size_t count)
{
	size_t alloc_size;
...
static void spl_heap_object_user_cache_append_zval(spl_heap_object *intern, zval *value)
{
	...
	spl_ptr_heap_user_cache_ensure_capacity(intern->heap, intern->heap->count + 1);
		...
		zend_opcache_user_cache_safe_direct_clone_value_func_t clone_value)
{
	...
	if (clone_value == NULL ||
		!spl_heap_object_allows_user_cache_state(old_object->ce) ||
		!spl_heap_object_allows_user_cache_state(new_object->ce)
	) {
		return false;
	}
	...
	is_pqueue = spl_heap_object_user_cache_is_pqueue(old_object->ce);
		...
		zend_opcache_user_cache_safe_direct_value_has_unstorable_func_t value_has_unstorable)
{
	...
	if (!spl_heap_object_allows_user_cache_state(Z_OBJCE_P(object))) {
		return true;
	}
	...
	is_pqueue = spl_heap_object_user_cache_is_pqueue(Z_OBJCE_P(object));
...
static bool spl_heap_object_serialize_user_cache_state(const zval *object, zval *state)
{
	spl_heap_object *intern;
	...
	for (i = 0; i < intern->heap->count; i++) {
		if (is_pqueue) {
			...
			zend_hash_next_index_insert(Z_ARRVAL(elements), heap_elem);
...
static const zend_opcache_user_cache_safe_direct_handlers *spl_heap_object_get_user_cache_handlers(void)
{
	static const zend_opcache_user_cache_safe_direct_handlers handlers = {
		false,
		spl_heap_object_copy_user_cache_state,
		spl_heap_object_user_cache_state_has_unstorable,
		spl_heap_object_serialize_user_cache_state,
		spl_heap_object_unserialize_user_cache_state
	};
