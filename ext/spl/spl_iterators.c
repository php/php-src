...
static int spl_recursive_it_valid_ex(spl_recursive_it_object *object, zval *zthis)
{
	...
	if (object->endIteration && object->in_iteration) {
		zend_call_method_with_0_params_ex(Z_OBJ_P(zthis), object->ce, &object->endIteration, ZSTR_KNOWN(ZEND_STR_END_ITERATION), NULL);
	}
...
next_step:
		...
		switch (object->iterators[object->level].state) {
			...
			case RS_TEST:
				...
				if (object->callHasChildren) {
					zend_call_method_with_0_params_ex(Z_OBJ_P(zthis), object->ce, &object->callHasChildren, ZSTR_KNOWN(ZEND_STR_CALL_HAS_CHILDREN), &retval);
				} else {
					zend_call_method_with_0_params_ex(Z_OBJ_P(zobject), ce, NULL, ZSTR_KNOWN(ZEND_STR_HAS_CHILDREN), &retval);
				}
				if (EG(exception)) {
					if (!(object->flags & RIT_CATCH_GET_CHILD)) {
				...
				if (object->nextElement) {
					zend_call_method_with_0_params_ex(Z_OBJ_P(zthis), object->ce, &object->nextElement, ZSTR_KNOWN(ZEND_STR_NEXT_ELEMENT), NULL);
				}
				object->iterators[object->level].state = RS_NEXT;
				if (EG(exception)) {
					if (!(object->flags & RIT_CATCH_GET_CHILD)) {
			...
			case RS_CHILD:
				...
				if (object->callGetChildren) {
					zend_call_method_with_0_params_ex(Z_OBJ_P(zthis), object->ce, &object->callGetChildren, ZSTR_KNOWN(ZEND_STR_CALL_GET_CHILDREN), &child);
				} else {
					zend_call_method_with_0_params_ex(Z_OBJ_P(zobject), ce, NULL, ZSTR_KNOWN(ZEND_STR_GET_CHILDREN), &child);
				}
				...
				if (object->beginChildren) {
					zend_call_method_with_0_params_ex(Z_OBJ_P(zthis), object->ce, &object->beginChildren, ZSTR_KNOWN(ZEND_STR_BEGIN_CHILDREN), NULL);
					if (EG(exception)) {
						if (!(object->flags & RIT_CATCH_GET_CHILD)) {
							return;
						} else {
		...
		if (object->level > 0) {
			if (object->endChildren) {
				zend_call_method_with_0_params_ex(Z_OBJ_P(zthis), object->ce, &object->endChildren, ZSTR_KNOWN(ZEND_STR_END_CHILDREN), NULL);
				if (EG(exception)) {
					if (!(object->flags & RIT_CATCH_GET_CHILD)) {
						return;
					} else {
...
static void spl_recursive_it_rewind_ex(spl_recursive_it_object *object, zval *zthis)
{
	...
	if (!EG(exception) && object->beginIteration && !object->in_iteration) {
		zend_call_method_with_0_params_ex(Z_OBJ_P(zthis), object->ce, &object->beginIteration, ZSTR_KNOWN(ZEND_STR_BEGIN_ITERATION), NULL);
	}
	...
	switch (rit_type) {
		case RIT_RecursiveTreeIterator: {
			...
			if (instanceof_function(Z_OBJCE_P(iterator), zend_ce_aggregate)) {
				zend_call_method_with_0_params_ex(Z_OBJ_P(iterator), Z_OBJCE_P(iterator), &Z_OBJCE_P(iterator)->iterator_funcs_ptr->zf_new_iterator, ZSTR_KNOWN(ZEND_STR_GET_ITERATOR), &aggregate_retval);
...
PHP_METHOD(RecursiveIteratorIterator, callHasChildren)
{
	...
	if (Z_TYPE_P(zobject) == IS_UNDEF) {
		...
		zend_call_method_with_0_params_ex(Z_OBJ_P(zobject), ce, NULL, ZSTR_KNOWN(ZEND_STR_HAS_CHILDREN), return_value);
...
PHP_METHOD(RecursiveIteratorIterator, callGetChildren)
{
	...
	if (Z_TYPE_P(zobject) == IS_UNDEF) {
		...
		zend_call_method_with_0_params_ex(Z_OBJ_P(zobject), ce, NULL, ZSTR_KNOWN(ZEND_STR_GET_CHILDREN), return_value);
		if (Z_TYPE_P(return_value) == IS_UNDEF) {
			RETURN_NULL();
		}
	}
...
static void spl_recursive_tree_iterator_get_prefix(spl_recursive_it_object *object, zval *return_value)
{
	...
	for (level = 0; level < object->level; ++level) {
		zend_call_method_with_0_params_ex(Z_OBJ(object->iterators[level].zobject), object->iterators[level].ce, NULL, ZSTR_KNOWN(ZEND_STR_HASNEXT), &has_next);
...
static spl_dual_it_object* spl_dual_it_construct(INTERNAL_FUNCTION_PARAMETERS, zend_class_entry *ce_base, zend_class_entry *ce_inner, dual_it_type dit_type)
{
	...
	switch (dit_type) {
		...
		case DIT_IteratorIterator: {
			...
			if (!instanceof_function(ce, zend_ce_iterator)) {
				...
				if (instanceof_function(ce, zend_ce_aggregate)) {
					zend_call_method_with_0_params_ex(Z_OBJ_P(zobject), ce, &ce->iterator_funcs_ptr->zf_new_iterator, ZSTR_KNOWN(ZEND_STR_GET_ITERATOR), &retval);
					if (EG(exception)) {
						zval_ptr_dtor(&retval);
						return NULL;
					}
		...
		case DIT_AppendIterator:
			...
			zend_call_method_with_0_params_ex(Z_OBJ(intern->u.append.zarrayit), spl_ce_ArrayIterator, &spl_ce_ArrayIterator->constructor, ZSTR_KNOWN(ZEND_STR_MAGIC_CONSTRUCT), NULL);
...
static inline void spl_filter_it_fetch(zval *zthis, spl_dual_it_object *intern)
{
	...
	while (spl_dual_it_fetch(intern, 1) == SUCCESS) {
		zend_call_method_with_0_params_ex(Z_OBJ_P(zthis), intern->std.ce, NULL, ZSTR_KNOWN(ZEND_STR_ACCEPT), &retval);
		if (Z_TYPE(retval) != IS_UNDEF) {
...
PHP_METHOD(RecursiveFilterIterator, hasChildren)
{
	...
	zend_call_method_with_0_params_ex(Z_OBJ(intern->inner.zobject), intern->inner.ce, NULL, ZSTR_KNOWN(ZEND_STR_HAS_CHILDREN), return_value);
} /* }}} */
...
PHP_METHOD(RecursiveFilterIterator, getChildren)
{
	...
	zend_call_method_with_0_params_ex(Z_OBJ(intern->inner.zobject), intern->inner.ce, NULL, ZSTR_KNOWN(ZEND_STR_GET_CHILDREN), &retval);
...
PHP_METHOD(RecursiveRegexIterator, accept)
{
	...
	zend_call_method_with_0_params_ex(Z_OBJ_P(ZEND_THIS), spl_ce_RegexIterator, NULL, ZSTR_KNOWN(ZEND_STR_ACCEPT), return_value);
}
...
static inline void spl_limit_it_seek(spl_dual_it_object *intern, zend_long pos)
{
	...
	if (pos != intern->current.pos && instanceof_function(intern->inner.ce, spl_ce_SeekableIterator)) {
		ZVAL_LONG(&zpos, pos);
		spl_dual_it_free(intern);
		zend_call_method_with_1_params_ex(Z_OBJ(intern->inner.zobject), intern->inner.ce, NULL, ZSTR_KNOWN(ZEND_STR_SEEK), NULL, &zpos);
		if (!EG(exception)) {
...
static inline void spl_caching_it_next(spl_dual_it_object *intern)
{
	if (spl_dual_it_fetch(intern, 1) == SUCCESS) {
		...
		if (intern->dit_type == DIT_RecursiveCachingIterator) {
			...
			zend_call_method_with_0_params_ex(Z_OBJ(intern->inner.zobject), intern->inner.ce, NULL, ZSTR_KNOWN(ZEND_STR_HAS_CHILDREN), &retval);
			if (EG(exception)) {
				zval_ptr_dtor(&retval);
				...
				if (zend_is_true(&retval)) {
					zend_call_method_with_0_params_ex(Z_OBJ(intern->inner.zobject), intern->inner.ce, NULL, ZSTR_KNOWN(ZEND_STR_GET_CHILDREN), &zchildren);
					if (EG(exception)) {
