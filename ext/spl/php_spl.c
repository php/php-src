...
PHP_FUNCTION(spl_autoload_register)
{
	...
	if (ZEND_FCI_INITIALIZED(fci)) {
		...
		zend_string *name = zend_string_init("spl_autoload", strlen("spl_autoload"), false);
		fci.size = sizeof(zend_fcall_info);
		ZVAL_STR(&fci.function_name, name);
		fci.object = NULL;
		fci.params = NULL;
		fci.param_count = 0;
		fci.named_params = NULL;
		fcc.function_handler = zend_hash_find_ptr(EG(function_table), name);
		fcc.object = NULL;
		zend_register_class_autoloader(&fci, &fcc, prepend);
