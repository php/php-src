...
/* Following lines are commented out to prevent bogus error message at
   start up. i.e. Save handler modules are not initialized before Session
   module. */
/*      if(!PS(mod)) { */
...
extern const char* get_temporary_directory();
...
static void php_session_initialize(TSRMLS_D)
{
    ...
    /* Check that we have a valid save_path and default if not. */
    if (!*(PS(save_path))) {
        const char* path = get_temporary_directory();
        zend_alter_ini_entry("session.save_path", sizeof("session.save_path"), (char*)path, strlen(path), PHP_INI_USER, PHP_INI_STAGE_RUNTIME);
    }
