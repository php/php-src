	...
	switch (option) {
		case PHP_FTP_OPT_TIMEOUT_SEC:
			if (Z_TYPE_P(z_value) != IS_LONG) {
				zend_argument_type_error(3, "must be of type int, %s given", zend_zval_type_name(z_value));
				RETURN_THROWS();
			}
		...
		case PHP_FTP_OPT_AUTOSEEK:
			if (Z_TYPE_P(z_value) != IS_TRUE && Z_TYPE_P(z_value) != IS_FALSE) {
				zend_argument_type_error(3, "must be of type bool, %s given", zend_zval_type_name(z_value));
				RETURN_THROWS();
			}
