...
ftp_quit(ftpbuf_t *ftp)
{
	...
	if (!ftp_putcmd(ftp, "QUIT", strlen("QUIT"), NULL, (size_t) 0)) {
		return 0;
	}
...
ftp_login(ftpbuf_t *ftp, const char *user, const size_t user_len, const char *pass, const size_t pass_len)
{
	...
	if (ftp->use_ssl && !ftp->ssl_active) {
		if (!ftp_putcmd(ftp, "AUTH", strlen("AUTH"), "TLS", strlen("TLS"))) {
			return 0;
		}
		if (!ftp_getresp(ftp)) {
			return 0;
		...
		if (ftp->resp != 234) {
			if (!ftp_putcmd(ftp, "AUTH", strlen("AUTH"), "SSL", strlen("SSL"))) {
				return 0;
			}
			if (!ftp_getresp(ftp)) {
				return 0;
		...
		if (!ftp->old_ssl) {
			...
			if (!ftp_putcmd(ftp, "PBSZ", strlen("PBSZ"), "0", strlen("0"))) {
				return 0;
			}
			if (!ftp_getresp(ftp)) {
				return 0;
			...
			if (!ftp_putcmd(ftp, "PROT", strlen("PROT"), "P", strlen("P"))) {
				return 0;
			}
			if (!ftp_getresp(ftp)) {
				return 0;
	...
	if (!ftp_putcmd(ftp, "USER", strlen("USER"), user, user_len)) {
		return 0;
	}
	if (!ftp_getresp(ftp)) {
		return 0;
	...
	if (!ftp_putcmd(ftp, "PASS", strlen("PASS"), pass, pass_len)) {
		return 0;
	}
	if (!ftp_getresp(ftp)) {
		return 0;
...
ftp_reinit(ftpbuf_t *ftp)
{
	...
	if (!ftp_putcmd(ftp, "REIN", strlen("REIN"), NULL, (size_t) 0)) {
		return 0;
	}
	if (!ftp_getresp(ftp) || ftp->resp != 220) {
		return 0;
...
ftp_syst(ftpbuf_t *ftp)
{
	...
	if (!ftp_putcmd(ftp, "SYST", strlen("SYST"), NULL, (size_t) 0)) {
		return NULL;
	}
...
ftp_pwd(ftpbuf_t *ftp)
{
	...
	if (!ftp_putcmd(ftp, "PWD", strlen("PWD"), NULL, (size_t) 0)) {
		return NULL;
	}
...
ftp_exec(ftpbuf_t *ftp, const char *cmd, const size_t cmd_len)
{
	...
	if (!ftp_putcmd(ftp, "SITE EXEC", strlen("SITE EXEC"), cmd, cmd_len)) {
		return 0;
	}
...
ftp_chdir(ftpbuf_t *ftp, const char *dir, const size_t dir_len)
{
	...
	if (!ftp_putcmd(ftp, "CWD", strlen("CWD"), dir, dir_len)) {
		return 0;
	}
...
ftp_cdup(ftpbuf_t *ftp)
{
	...
	if (!ftp_putcmd(ftp, "CDUP", strlen("CDUP"), NULL, (size_t) 0)) {
		return 0;
	}
	if (!ftp_getresp(ftp) || ftp->resp != 250) {
		return 0;
...
ftp_mkdir(ftpbuf_t *ftp, const char *dir, const size_t dir_len)
{
	...
	if (!ftp_putcmd(ftp, "MKD", strlen("MKD"), dir, dir_len)) {
		return NULL;
	}
	if (!ftp_getresp(ftp) || ftp->resp != 257) {
		return NULL;
...
ftp_rmdir(ftpbuf_t *ftp, const char *dir, const size_t dir_len)
{
	...
	if (!ftp_putcmd(ftp, "RMD", strlen("RMD"), dir, dir_len)) {
		return 0;
	}
	if (!ftp_getresp(ftp) || ftp->resp != 250) {
		return 0;
...
ftp_chmod(ftpbuf_t *ftp, const int mode, const char *filename, const int filename_len)
{
	...
	if (!ftp_putcmd(ftp, "SITE", strlen("SITE"), buffer, buffer_len)) {
		efree(buffer);
		return 0;
	}
...
ftp_alloc(ftpbuf_t *ftp, const zend_long size, zend_string **response)
{
	...
	if (!ftp_putcmd(ftp, "ALLO", strlen("ALLO"), buffer, buffer_len)) {
		return 0;
	}
...
{
	return ftp_genlist(ftp, "NLST", strlen("NLST"), path, path_len);
}
/* }}} */
...
{
	return ftp_genlist(ftp, ((recursive) ? "LIST -R" : "LIST"), ((recursive) ? strlen("LIST -R") : strlen("LIST")), path, path_len);
}
/* }}} */
...
{
	return ftp_genlist(ftp, "MLSD", strlen("MLSD"), path, path_len);
}
/* }}} */
...
ftp_mlsd_parse_line(HashTable *ht, const char *input) {
	...
	zend_hash_str_update(ht, "name", strlen("name"), &zstr);
	end = sp;
...
ftp_type(ftpbuf_t *ftp, ftptype_t type)
{
	...
	if (!ftp_putcmd(ftp, "TYPE", strlen("TYPE"), typechar, 1)) {
		return 0;
	}
	if (!ftp_getresp(ftp) || ftp->resp != 200) {
		return 0;
...
ftp_pasv(ftpbuf_t *ftp, int pasv)
{
	...
	if (sa->sa_family == AF_INET6) {
		...
		/* try EPSV first */
		if (!ftp_putcmd(ftp, "EPSV", strlen("EPSV"), NULL, (size_t) 0)) {
			return 0;
		}
		if (!ftp_getresp(ftp)) {
			return 0;
	...
	if (!ftp_putcmd(ftp, "PASV",  strlen("PASV"), NULL, (size_t) 0)) {
		return 0;
	}
...
ftp_get(ftpbuf_t *ftp, php_stream *outstream, const char *path, const size_t path_len, ftptype_t type, zend_long resumepos)
{
	...
	if (resumepos > 0) {
		...
		if (!ftp_putcmd(ftp, "REST", strlen("REST"), arg, arg_len)) {
			goto bail;
		}
	...
	if (!ftp_putcmd(ftp, "RETR", strlen("RETR"), path, path_len)) {
		goto bail;
	}
...
ftp_put(ftpbuf_t *ftp, const char *path, const size_t path_len, php_stream *instream, ftptype_t type, zend_long startpos)
{
	...
	if (!ftp_putcmd(ftp, "STOR", strlen("STOR"), path, path_len)) {
		goto bail;
	}
	if (!ftp_getresp(ftp) || (ftp->resp != 150 && ftp->resp != 125)) {
		goto bail;
...
ftp_append(ftpbuf_t *ftp, const char *path, const size_t path_len, php_stream *instream, ftptype_t type)
{
	...
	if (!ftp_putcmd(ftp, "APPE", strlen("APPE"), path, path_len)) {
		goto bail;
	}
	if (!ftp_getresp(ftp) || (ftp->resp != 150 && ftp->resp != 125)) {
		goto bail;
...
ftp_size(ftpbuf_t *ftp, const char *path, const size_t path_len)
{
	...
	if (!ftp_putcmd(ftp, "SIZE", strlen("SIZE"), path, path_len)) {
		return -1;
	}
...
ftp_mdtm(ftpbuf_t *ftp, const char *path, const size_t path_len)
{
	...
	if (!ftp_putcmd(ftp, "MDTM", strlen("MDTM"), path, path_len)) {
		return -1;
	}
	if (!ftp_getresp(ftp) || ftp->resp != 213) {
		return -1;
...
ftp_delete(ftpbuf_t *ftp, const char *path, const size_t path_len)
{
	...
	if (!ftp_putcmd(ftp, "DELE", strlen("DELE"), path, path_len)) {
		return 0;
	}
	if (!ftp_getresp(ftp) || ftp->resp != 250) {
		return 0;
...
ftp_rename(ftpbuf_t *ftp, const char *src, const size_t src_len, const char *dest, const size_t dest_len)
{
	...
	if (!ftp_putcmd(ftp, "RNFR", strlen("RNFR"), src, src_len)) {
		return 0;
	}
	if (!ftp_getresp(ftp) || ftp->resp != 350) {
		return 0;
	}
	if (!ftp_putcmd(ftp, "RNTO", strlen("RNTO"), dest, dest_len)) {
		return 0;
	}
	if (!ftp_getresp(ftp) || ftp->resp != 250) {
		return 0;
...
ftp_site(ftpbuf_t *ftp, const char *cmd, const size_t cmd_len)
{
	...
	if (!ftp_putcmd(ftp, "SITE", strlen("SITE"), cmd, cmd_len)) {
		return 0;
	}
...
ftp_getdata(ftpbuf_t *ftp)
{
	...
	if (sa->sa_family == AF_INET6) {
		...
		if (!ftp_putcmd(ftp, "EPRT", strlen("EPRT"), eprtarg, eprtarg_len)) {
			goto bail;
		}
	...
	if (!ftp_putcmd(ftp, "PORT", strlen("PORT"), arg, arg_len)) {
		goto bail;
	}
	if (!ftp_getresp(ftp) || ftp->resp != 200) {
		goto bail;
