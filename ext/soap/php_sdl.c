...
static void load_wsdl_ex(zval *this_ptr, char *struri, sdlCtx *ctx, int include, const char *import_namespace)
{
	...
	while (trav != NULL) {
		...
		if (node_is_equal(trav,"types")) {
			...
			if (namespace_attr) {
				/* TODO: the value must not match the enclosing's target namespace, but for BC reasons we allow it. */
			...
			if (location_attr) {
				...
				load_wsdl_ex(this_ptr, (char*)uri, ctx, 1, new_import_namespace);
				xmlFree(uri);
			}
...
{
	xmlNodePtr trav, part, message = NULL;
	HashTable* parameters = NULL;

	message = qname_hash_table_find(fallback_ns, &ctx->messages, message_name);
	if (!message) {
...
*/
		...
		zend_hash_move_forward(&ctx.services.inner);
	}
