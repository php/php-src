...
static void mbfl_filt_ident_2022jpms(unsigned char c, mbfl_identify_filter *filter);
...
void mbfl_filt_conv_2022jpms_wchar(int c, mbfl_convert_filter *filter)
{
	...
	case 1:
		...
		if (c > 0x20 && c < 0x7F) {
			...
			(*filter->output_function)((c1 << 8) | c | MBFL_WCSGROUP_THROUGH, filter->data);
		}
		break;
	...
	case 2:
		if (c == '$') {
			...
			filter->status &= ~0xF;
			(*filter->output_function)(0x1B00 | c | MBFL_WCSGROUP_THROUGH, filter->data);
		}
		break;
	...
	case 3:
		if (c == '@' || c == 'B') {
			...
			filter->status &= ~0xF;
			(*filter->output_function)(0x1B2400 | c | MBFL_WCSGROUP_THROUGH, filter->data);
		}
		break;
	...
	case 4:
		if (c == '@' || c == 'B') {
			...
			filter->status &= ~0xF;
			(*filter->output_function)(0x1B242800 | c | MBFL_WCSGROUP_THROUGH, filter->data);
		}
		break;
	...
	case 5:
		if (c == 'B' || c == 'J') {
			...
			filter->status &= ~0xF;
			(*filter->output_function)(0x1B2800 | c | MBFL_WCSGROUP_THROUGH, filter->data);
		}
	}
}
	...
	if (s1 >= 0) {
		if (s1 < 0x80) { /* latin */
			...
			if ((filter->status & 0xFF00) != 0x100) {
				(*filter->output_function)(0x1B, filter->data); /* ESC */
				(*filter->output_function)('(', filter->data);
				(*filter->output_function)('I', filter->data);
			}
			filter->status = 0x100;
			(*filter->output_function)(s1 & 0x7F, filter->data);
			...
			filter->status = 0x800;
			(*filter->output_function)(((s1 >> 8) - 0x5E) & 0x7F, filter->data);
			(*filter->output_function)(s1 & 0x7F, filter->data);
		}
	} else {
...
static void mbfl_filt_ident_2022jpms_cp932(unsigned char c, mbfl_identify_filter *filter);
static void mbfl_filt_ident_2022jpms_udc(unsigned char c, mbfl_identify_filter *filter);

static int handle_esc_sequence_2022jpms(unsigned char c, mbfl_identify_filter *filter)
{
	...
	case 2: /* ESC $ */
		if (c == '@' || c == 'B') {
			filter->filter_function = mbfl_filt_ident_2022jpms_cp932;
			filter->status = 0;
		} else if (c == '(') {
			filter->status++;
		} else {
		...
		case 3: /* ESC $ ( */
			if (c == '?') {
				filter->filter_function = mbfl_filt_ident_2022jpms_udc;
			} else if (c != '@' && c != 'B') {
				filter->flag = 1; /* bad */
			}
			filter->status = 0;
...
{
	if (!filter->status) {
		if (handle_esc_sequence_2022jpms(c, filter)) {
			return;
		}
		if (c < 0x21 || c > 0x34) {
			filter->flag = 1;
...
static void mbfl_filt_ident_2022jpms(unsigned char c, mbfl_identify_filter *filter)
{
	if (!handle_esc_sequence_2022jpms(c, filter) && c >= 0x80 && (c < 0xA1 || c > 0xDF)) {
		filter->flag = 1;
	}
}
