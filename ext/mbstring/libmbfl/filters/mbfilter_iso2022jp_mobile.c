...
static void mbfl_filt_conv_2022jp_mobile_wchar_flush(mbfl_convert_filter *filter);
static void mbfl_filt_conv_wchar_2022jp_mobile_flush(mbfl_convert_filter *filter);
static void mbfl_filt_ident_2022jp_mobile(unsigned char c, mbfl_identify_filter *filter);
...
void mbfl_filt_conv_2022jp_mobile_wchar(int c, mbfl_convert_filter *filter)
{
	int c1, s, w, snd = 0;
...
void mbfl_filt_conv_wchar_2022jp_mobile(int c, mbfl_convert_filter *filter)
{
	int c1, c2, s1 = 0, s2 = 0;
...
static void mbfl_filt_ident_2022jp_cp932(unsigned char c, mbfl_identify_filter *filter);

static int handle_esc_sequence_2022jp_mobile(int c, mbfl_identify_filter *filter)
{
	...
	case 2: /* Already saw ESC $ */
		if (c == '@' || c == 'B') {
			filter->filter_function = mbfl_filt_ident_2022jp_cp932;
			filter->status = 0;
		} else if (c == '(') {
			filter->status = 4;
		} else {
	...
	case 3: /* Already saw ESC ( */
		if (c == 'B' || c == 'J' || c == 'I') {
			filter->filter_function = mbfl_filt_ident_2022jp_mobile;
		} else {
			filter->flag = 1;
		}
		filter->status = 0;
...
static void mbfl_filt_ident_2022jp_mobile(unsigned char c, mbfl_identify_filter *filter)
{
	if (!handle_esc_sequence_2022jp_mobile(c, filter) && ((c >= 0x80 && c <= 0xA0) || c >= 0xE0)) {
		filter->flag = 1;
	}
}
...
static void mbfl_filt_ident_2022jp_cp932(unsigned char c, mbfl_identify_filter *filter)
{
	if (handle_esc_sequence_2022jp_mobile(c, filter)) {
		return;
	}
	...
	if (filter->status == 0) {
		...
		if (c < 0x21 || c > 0x7F) {
			...
			int s = ((filter->status - 0x21) * 94) + c - 0x21;
			filter->status = 0;

			if (s >= (84 * 94) && s < (91 * 94)) {
				s += 22 * 94;
				int dummy;
				if (mbfilter_sjis_emoji_kddi2unicode(s, &dummy)) {
					return; /* OK */
				}
			}

			if (s >= cp932ext1_ucs_table_min && s < cp932ext1_ucs_table_max) {
				if (!cp932ext1_ucs_table[s - cp932ext1_ucs_table_min]) {
					filter->flag = 1;
				}
			} else if (s >= 0 && s < jisx0208_ucs_table_size) {
				if (!jisx0208_ucs_table[s]) {
					filter->flag = 1;
				}
			} else {
				filter->flag = 1;
