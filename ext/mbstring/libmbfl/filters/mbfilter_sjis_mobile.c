...
static const char *mbfl_encoding_sjis_docomo_aliases[] = {
	"SJIS-DOCOMO", "shift_jis-imode", "x-sjis-emoji-docomo", NULL
};
static const char *mbfl_encoding_sjis_kddi_aliases[] = {"SJIS-KDDI", "shift_jis-kddi", "x-sjis-emoji-kddi", NULL};
static const char *mbfl_encoding_sjis_sb_aliases[] = {
	"SJIS-SOFTBANK", "shift_jis-softbank", "x-sjis-emoji-softbank", NULL
};
...
mbfilter_sjis_emoji_kddi2unicode(int s, int *snd) {
	...
	*snd = 0;
	if (s >= mb_tbl_code2uni_kddi1_min && s <= mb_tbl_code2uni_kddi1_max) {
		...
		if (si == 0x0008) { /* ES */
			*snd = NFLAGS(nflags_s[2][0]);
			w = NFLAGS(nflags_s[2][1]);
		} else if (si == 0x0009) { /* RU */
			*snd = NFLAGS(nflags_s[8][0]);
			w = NFLAGS(nflags_s[8][1]);
		} else if (si >= 0x008d && si <= 0x0092) {
			c = nflags_order_kddi[si - 0x008d];
			*snd = NFLAGS(nflags_s[c][0]);
			w = NFLAGS(nflags_s[c][1]);
		} else if (si == 0x0104) {
			*snd = 0x0023;
			w = 0x20E3;
		} else {
		...
		if (si == 100) { /* JP */
			*snd = NFLAGS(nflags_s[6][0]);
			w = NFLAGS(nflags_s[6][1]);
		} else if (si >= 0x00ba && si <= 0x00c2) {
			*snd = si - 0x00ba + 0x0031;
			w = 0x20E3;
		} else if (si == 0x010b) { /* US */
			*snd = NFLAGS(nflags_s[9][0]);
			w = NFLAGS(nflags_s[9][1]);
		} else if (si == 0x0144) {
			*snd = 0x0030;
			w = 0x20E3;
		} else {
		...
		case 1:        /* kanji second char */
			...
			if (c >= 0x40 && c <= 0xfc && c != 0x7f) {
				...
				if (w == 0) {
					...
					} else if (s >= cp932ext3_ucs_table_min &&
