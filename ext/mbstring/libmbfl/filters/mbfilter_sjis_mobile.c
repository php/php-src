...
static void mbfl_filt_ident_sjis_docomo(unsigned char c, mbfl_identify_filter *filter);
static void mbfl_filt_ident_sjis_kddi(unsigned char c, mbfl_identify_filter *filter);
static void mbfl_filt_ident_sjis_softbank(unsigned char c, mbfl_identify_filter *filter);
...
#define EMIT_FLAG_EMOJI(country) \
	*snd = NFLAGS((country)[0]); \
	return NFLAGS((country)[1])
...
void mbfl_filt_conv_sjis_mobile_wchar(int c, mbfl_convert_filter *filter)
{
	...
	case 1: /* Kanji, second byte */
		...
		if (c >= 0x40 && c <= 0xFC && c != 0x7F) {
			...
			if (w == 0) {
				...
				if (w == 0 && s >= cp932ext3_ucs_table_min && s < cp932ext3_ucs_table_max) {		/* vendor ext3 (115ku - 119ku) */
	...
	case 2:
		if (c == '$') {
			...
			(*filter->output_function)((filter->cache << 8) | c | MBFL_WCSGROUP_THROUGH, filter->data);
...
static void mbfl_filt_ident_sjis_docomo(unsigned char c, mbfl_identify_filter *filter)
{
	if (filter->status) { /* Kanji second char */
		if (c < 0x40 || c > 0xFC || c == 0x7F) { /* bad */
			...
			if (w >= cp932ext1_ucs_table_min && w < cp932ext1_ucs_table_max) {
				if (!cp932ext1_ucs_table[w - cp932ext1_ucs_table_min]) {
					filter->flag = 1;
				}
			} else if (w >= 0 && w < jisx0208_ucs_table_size) {
				if (!jisx0208_ucs_table[w]) {
					filter->flag = 1;
				}
			} else if (w >= cp932ext2_ucs_table_min && w < cp932ext2_ucs_table_max) {
				if (!cp932ext2_ucs_table[w - cp932ext2_ucs_table_min]) {
					filter->flag = 1;
				}
			} else if (w >= cp932ext3_ucs_table_min && w < cp932ext3_ucs_table_max) {
				if (!cp932ext3_ucs_table[w - cp932ext3_ucs_table_min]) {
					filter->flag = 1;
				}
			} else if (w < mb_tbl_code2uni_docomo1_min || w > mb_tbl_code2uni_docomo1_max ||
					  !mb_tbl_code2uni_docomo1[w - mb_tbl_code2uni_docomo1_min]) {
				filter->flag = 1;
			}
		}
		filter->status = 0;
		...
		;
	} else if (c > 0x80 && c <= 0xFC && c != 0xA0) { /* Kanji first char */
		filter->status = c;
	} else { /* bad */
		filter->flag = 1;
	}
...
static void mbfl_filt_ident_sjis_kddi(unsigned char c, mbfl_identify_filter *filter)
{
	if (filter->status) { /* Kanji second char */
		if (c < 0x40 || c > 0xFC || c == 0x7F) { /* bad */
			...
			} else if (w >= mb_tbl_code2uni_kddi1_min && w <= mb_tbl_code2uni_kddi1_max) {
				if (!mb_tbl_code2uni_kddi1[w - mb_tbl_code2uni_kddi1_min]) {
					filter->flag = 1;
				}
			} else if (w < mb_tbl_code2uni_kddi2_min || w > mb_tbl_code2uni_kddi2_max ||
					  !mb_tbl_code2uni_kddi2[w - mb_tbl_code2uni_kddi2_min]) {
				filter->flag = 1;
			}
		}
		filter->status = 0;
...
static void mbfl_filt_ident_sjis_softbank_esc(unsigned char c1, unsigned char c2, mbfl_identify_filter *filter)
{
	if (c2 == 0xF) {
		filter->filter_function = mbfl_filt_ident_sjis_softbank;
	} else {
		int s  = (c1 - 0x21)*94 + c2 - 0x21, snd;
		int w = mbfilter_sjis_emoji_sb2unicode(s, &snd);
		if (w <= 0) {
			filter->flag = 1;
		}
	}
...
static void mbfl_filt_ident_sjis_softbank_esc_G(unsigned char c, mbfl_identify_filter *filter)
{
	mbfl_filt_ident_sjis_softbank_esc(0x91, c, filter);
}

static void mbfl_filt_ident_sjis_softbank_esc_E(unsigned char c, mbfl_identify_filter *filter)
{
	mbfl_filt_ident_sjis_softbank_esc(0x8D, c, filter);
}

static void mbfl_filt_ident_sjis_softbank_esc_F(unsigned char c, mbfl_identify_filter *filter)
{
	mbfl_filt_ident_sjis_softbank_esc(0x8E, c, filter);
}

static void mbfl_filt_ident_sjis_softbank_esc_O(unsigned char c, mbfl_identify_filter *filter)
{
	mbfl_filt_ident_sjis_softbank_esc(0x92, c, filter);
}

static void mbfl_filt_ident_sjis_softbank_esc_P(unsigned char c, mbfl_identify_filter *filter)
{
	mbfl_filt_ident_sjis_softbank_esc(0x95, c, filter);
}

static void mbfl_filt_ident_sjis_softbank_esc_Q(unsigned char c, mbfl_identify_filter *filter)
{
	mbfl_filt_ident_sjis_softbank_esc(0x96, c, filter);
}

static void mbfl_filt_ident_sjis_softbank(unsigned char c, mbfl_identify_filter *filter)
{
	if (filter->status == 0x1B) {
		if (c == '$') {
			filter->status = 0x1B24;
		} else {
			filter->flag = 1;
		}
	} else if (filter->status == 0x1B24) {
		if (c == 'G') {
			filter->filter_function = mbfl_filt_ident_sjis_softbank_esc_G;
			filter->status = 0;
		} else if (c == 'E') {
			filter->filter_function = mbfl_filt_ident_sjis_softbank_esc_E;
			filter->status = 0;
		} else if (c == 'F') {
			filter->filter_function = mbfl_filt_ident_sjis_softbank_esc_F;
			filter->status = 0;
		} else if (c == 'O') {
			filter->filter_function = mbfl_filt_ident_sjis_softbank_esc_O;
			filter->status = 0;
		} else if (c == 'P') {
			filter->filter_function = mbfl_filt_ident_sjis_softbank_esc_P;
			filter->status = 0;
		} else if (c == 'Q') {
			filter->filter_function = mbfl_filt_ident_sjis_softbank_esc_Q;
			filter->status = 0;
		} else {
			filter->flag = 1;
		}
	} else if (filter->status) { /* Kanji second char */
		if (c < 0x40 || c > 0xfc || c == 0x7f) { /* bad */
		    filter->flag = 1;
		} else {
			int s1, s2;
			...
			} else if (w >= mb_tbl_code2uni_sb1_min && w <= mb_tbl_code2uni_sb1_max) {
				if (!mb_tbl_code2uni_sb1[w - mb_tbl_code2uni_sb1_min]) {
					filter->flag = 1;
				}
			} else if (w >= mb_tbl_code2uni_sb2_min && w <= mb_tbl_code2uni_sb2_max) {
				if (!mb_tbl_code2uni_sb2[w - mb_tbl_code2uni_sb2_min]) {
					filter->flag = 1;
				}
			} else if (w < mb_tbl_code2uni_sb3_min || w > mb_tbl_code2uni_sb3_max ||
					  !mb_tbl_code2uni_sb3[w - mb_tbl_code2uni_sb3_min]) {
				filter->flag = 1;
			}
		}
		filter->status = 0;
