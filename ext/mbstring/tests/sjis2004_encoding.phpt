...
$fp = fopen(realpath(__DIR__ . '/../docs/SJIS-2004.txt'), 'r+');
while ($line = fgets($fp, 256)) {
	...
	if (sscanf($line, "0x%x\tU+%x+%x", $bytes, $codepoint1, $codepoint2) >= 2) {
		if ($bytes < 256) {
			$validChars[chr($bytes)] = pack('N', $codepoint1);
		} else if ($codepoint2) {
			$validChars[pack('n', $bytes)] = pack('NN', $codepoint1, $codepoint2);
		} else {
			...
			if (preg_match('/Fullwidth: U\+([0-9A-F]+)/', $line, $match))
				$validChars[pack('n', $bytes)] = pack('N', hexdec($match[1]));
			else
				$validChars[pack('n', $bytes)] = pack('N', $codepoint1);
		}
	}
}
...
findInvalidChars($validChars, $invalidChars, $truncated,
	map(range(0x81, 0x9F), 2, map(range(0xE0, 0xFC), 2)));
...
testAllInvalidChars($invalidChars, $fromUnicode, 'UTF-32BE', 'SJIS-2004', '%');
