...
static void dom_ordered_set_parser(HashTable *token_set, const char *data)
{
	...
	while (*data != '\0') {
		...
		data += strcspn(data, ascii_whitespace);
...
static char *dom_ordered_set_serializer(HashTable *token_set)
{
	...
	ZEND_HASH_MAP_FOREACH_STR_KEY(token_set, token) {
		...
		size_t needed_size = ZSTR_LEN(token) + 1; /* +1 for the space */
...
static zend_always_inline const xmlChar *dom_token_list_get_class_value(const xmlAttr *attr)
{
...
static void dom_token_list_update_set(dom_token_list_object *intern, HashTable *token_set)
{
	...
	const xmlChar *value = dom_token_list_get_class_value(attr);
	if (value != NULL) {
...
static void dom_token_list_item_read(dom_token_list_object *token_list, zval *retval, zend_long index)
{
	...
	if (index >= 0 && index < zend_hash_num_elements(token_set)) {
		 ...
		 * It allows for things like `item(x) ?? ...` */
		ZVAL_NULL(retval);
	}
}
...
{
	/* If a token exists, then it cannot be empty. */
	...
	if (UNEXPECTED(failed)) {
		...
		return dom_token_list_item_exists(php_dom_token_list_from_obj(object), index);
	}
}
...
static zend_always_inline HashPosition get_hash_current_pos_ex(HashTable *ht, HashPosition pos)
{
	ZEND_ASSERT(!HT_IS_PACKED(ht));
	/* TODO: this is a hack because zend_hash_get_current_pos_ex doesn't exist, but this'll do. */
...
static zend_result dom_token_list_it_valid(zend_object_iterator *iter)
{
	...
	iterator->pos = get_hash_current_pos_ex(token_set, iterator->pos);
...
static void dom_token_list_it_move_forward(zend_object_iterator *iter)
{
	...
	HashPosition validated = get_hash_current_pos_ex(token_set, iterator->pos);
