...
PHP_FUNCTION(dom_document_adopt_node)
{
	...
	// Adopt node: https://dom.spec.whatwg.org/#concept-node-adopt
	// 1. Let oldDocument be node’s node document.
	oldDocument = nodep->doc;

	// 2.) If node’s parent is not null, remove node from its parent.
	if (nodep->parent != NULL) {
		xmlNodePtr children;

		children = nodep->parent->children;
		if (!children) {
	...
	if (oldDocument != docp) {
		// TODO: check errors
		php_libxml_decrement_doc_ref((php_libxml_node_object *)nodeobj);
		php_libxml_increment_doc_ref((php_libxml_node_object *)nodeobj, docp);

		xmlNodePtr inclusiveDescendant = nodep;
		...
		while (inclusiveDescendant != NULL) {
			...
			inclusiveDescendant->ns = NULL;
			inclusiveDescendant->nsDef = NULL;
			inclusiveDescendant->doc = docp;
			...
			if (inclusiveDescendant->type == XML_ELEMENT_NODE) {
				attr = inclusiveDescendant->properties;
			...
			// 3. For each inclusiveDescendant in node’s shadow-including
			// inclusive descendants, in shadow-including tree order, run the
			// adopting steps with inclusiveDescendant and oldDocument.
			/* traversal code from libxml2 xmlReconciliateNs: Browse the full subtree, deep first */
			if ((inclusiveDescendant->children != NULL) && (inclusiveDescendant->type != XML_ENTITY_REF_NODE)) {
				/* deep first */
				inclusiveDescendant = inclusiveDescendant->children;
			} else if ((inclusiveDescendant != nodep) && (inclusiveDescendant->next != NULL)) {
				/* then siblings */
				inclusiveDescendant = inclusiveDescendant->next;
			} else if (inclusiveDescendant != nodep) {
				/* go up to parents->next if needed */
				while (inclusiveDescendant != nodep) {
					if (inclusiveDescendant->parent != NULL) {
						inclusiveDescendant = inclusiveDescendant->parent;
					}

					if ((inclusiveDescendant != nodep) && (inclusiveDescendant->next != NULL)) {
						inclusiveDescendant = inclusiveDescendant->next;
						break;
					}
					if (inclusiveDescendant->parent == NULL) {
						inclusiveDescendant = NULL;
						break;
					}
				}
				/* exit condition */
				if (inclusiveDescendant == nodep) {
					inclusiveDescendant = NULL;
				}
			} else {
				break;
