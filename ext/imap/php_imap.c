...
PHP_FUNCTION(imap_mail_copy)
{
	...
	if (mail_copy_full(imap_le_struct->imap_stream, seq->val, folder->val, options) == T) {
		RETURN_TRUE;
	} else {
		RETURN_FALSE;
	}
...
PHP_FUNCTION(imap_delete)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), "rz|l", &streamind, &sequence, &flags) == FAILURE) {
		return;
	}
	...
	mail_setflag_full(imap_le_struct->imap_stream, Z_STRVAL_P(sequence), "\\DELETED", flags);
	RETVAL_TRUE;
}
/* }}} */
...
PHP_FUNCTION(imap_undelete)
{
	...
	mail_clearflag_full(imap_le_struct->imap_stream, Z_STRVAL_P(sequence), "\\DELETED", flags);
	RETVAL_TRUE;
}
/* }}} */
...
PHP_FUNCTION(imap_clearflag_full)
{
	...
	mail_clearflag_full(imap_le_struct->imap_stream, sequence->val, flag->val, flags);
	RETURN_TRUE;
}
/* }}} */
...
PHP_FUNCTION(imap_mail)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), "SSS|SSSS", &to, &subject, &message, 
...
PHP_FUNCTION(imap_search)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), "rs|lS", &streamind, &criteria, &flags, &charset) == FAILURE) {
		return;
	}
	...
	mail_search_full(imap_le_struct->imap_stream, (charset != NULL ? charset->val : NIL), pgm, flags);
