...
static int curl_sockoptfunction(void *clientp, curl_socket_t curlfd, curlsocktype purpose)
{
	...
	if (Z_ISUNDEF(retval)) {
		/* The callback threw: zend_call_function() releases and undefines the
		 * return value when an exception is pending. Match the abort behaviour of
		 * opensocket/ssh_hostkey/prereq — for a security-sensitive hook,
		 * connecting anyway after an exception would be surprising. */
...
static int curl_closesocketfunction(void *clientp, curl_socket_t item)
{
	...
	/* libcurl documents 0 as success and 1 as error for this callback. */
	int rval = 0;

	if (Z_ISUNDEF(retval)) {
		/* The callback threw: report the failure to libcurl. The descriptor is
		 * still closed below, so signalling an error cannot leak it. */
		rval = 1;
	} else {
		_php_curl_verify_handlers(ch, /* reporterror */ true);
		zval_ptr_dtor(&retval);
