...
static int php_curl_option_str(php_curl *ch, php_int_t option, const char *str, const php_size_t len, zend_bool make_copy TSRMLS_DC)
{
...
static int _php_curl_setopt(php_curl *ch, php_int_t option, zval **zvalue TSRMLS_DC) /* {{{ */
{
	...
	switch (option) {
		...
		case CURLOPT_PRIVATE:
			convert_to_string_ex(zvalue);
			return php_curl_option_str(ch, option, Z_STRVAL_PP(zvalue), Z_STRSIZE_PP(zvalue), 1 TSRMLS_CC);

		/* Curl url option */
		case CURLOPT_URL:
			convert_to_string_ex(zvalue);
			return php_curl_option_url(ch, Z_STRVAL_PP(zvalue), Z_STRSIZE_PP(zvalue) TSRMLS_CC);
