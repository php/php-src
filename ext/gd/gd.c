...
static void php_imagepolygon(INTERNAL_FUNCTION_PARAMETERS, int filled)
{
	...
	if (zend_parse_parameters_ex(ZEND_PARSE_PARAMS_QUIET, ZEND_NUM_ARGS(), "Oall", &IM, gd_image_ce, &POINTS, &NPOINTS, &COL) == FAILURE) {
		if (zend_parse_parameters(ZEND_NUM_ARGS(), "Oal", &IM, gd_image_ce, &POINTS, &COL) == FAILURE) {
			return;
		}
		...
		php_error_docref(NULL, E_DEPRECATED, "the $num_points parameter is deprecated");
	}
