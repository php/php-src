...
struct gdGradientStruct {
    ...
    union {
        struct { double x0, y0, dx, dy, inverse_length_squared, scale; } linear;
        struct { double x0, y0, r0, x1, y1, r1, cdx, cdy, dr, quadratic_a, scale; } radial;
    } geometry;
};

void gdGradientDestroy(gdGradient *gradient);
int gdExtendFold(double raw, gdExtendMode mode, double *folded);
gdPremulPixelF gdGradientSample(const gdGradient *gradient,
