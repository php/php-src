 ...
 */
static avifBool setEncoderTilesAndThreads(avifEncoder *encoder,
										  avifRGBImage *rgb) {
...
static avifBool isAvifSrgbImage(avifImage *avifIm) {
	return (avifIm->colorPrimaries == AVIF_COLOR_PRIMARIES_BT709 ||
		   ...
		   (avifIm->transferCharacteristics ==
...
/*
   ...
   function.
*/
static avifResult readFromCtx(avifIO *io, uint32_t readFlags, uint64_t offset,
...
static avifIO *createAvifIOFromCtx(gdIOCtx *ctx) {
	...
	reader->io.sizeHint =
		0; // sadly, we don't get this information from the gdIOCtx.
	reader->io.data = ctx;
...
*/
static avifBool _gdImageAvifCtx(gdImagePtr im, gdIOCtx *outfile, int quality,
								int speed) {
	avifResult result;
	avifRGBImage rgb = {0};
	...
	result =
		avifEncoderAddImage(encoder, avifIm, 1, AVIF_ADD_IMAGE_FLAG_SINGLE);
