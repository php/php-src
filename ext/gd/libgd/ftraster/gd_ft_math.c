...
GD_FT_Long GD_FT_MulFix(GD_FT_Long a, GD_FT_Long b)
{
    ...
    c = (GD_FT_Long)(((GD_FT_Int64)a * b + 0x8000L) >> 16);
...
GD_FT_Long GD_FT_MulDiv(GD_FT_Long a, GD_FT_Long b, GD_FT_Long c)
{
    ...
    d = (GD_FT_Long)(c > 0 ? ((GD_FT_Int64)a * b + (c >> 1)) / c : 0x7FFFFFFFL);
...
#define GD_FT_TRIG_SCALE 0xDBD95B16UL

/* the highest bit in overflow-safe vector components, */
/* MSB of 0.858785336480436 * sqrt(0.5) * 2^30         */
#define GD_FT_TRIG_SAFE_MSB 29

/* this table was generated for GD_FT_PI = 180L << 16, i.e. degrees */
...
static GD_FT_Fixed ft_trig_downscale(GD_FT_Fixed val)
{
    ...
    v = (val * (GD_FT_Int64)GD_FT_TRIG_SCALE) + 0x100000000UL;
    val = (GD_FT_Fixed)(v >> 32);
...
static GD_FT_Int ft_trig_prenorm(GD_FT_Vector* vec)
{
    ...
    shift = GD_FT_MSB(GD_FT_ABS(x) | GD_FT_ABS(y));

    if (shift <= GD_FT_TRIG_SAFE_MSB) {
        shift = GD_FT_TRIG_SAFE_MSB - shift;
        vec->x = (GD_FT_Pos)((GD_FT_ULong)x << shift);
        vec->y = (GD_FT_Pos)((GD_FT_ULong)y << shift);
    } else {
        shift -= GD_FT_TRIG_SAFE_MSB;
        vec->x = x >> shift;
        vec->y = y >> shift;
        shift = -shift;
    }
...
static void ft_trig_pseudo_rotate(GD_FT_Vector* vec, GD_FT_Angle theta)
{
    ...
    while (theta < -GD_FT_ANGLE_PI4) {
        xtemp = y;
        y = -x;
        x = xtemp;
        theta += GD_FT_ANGLE_PI2;
    }
    ...
    for (i = 1, b = 1; i < GD_FT_TRIG_MAX_ITERS; b <<= 1, i++) {
        GD_FT_Fixed v1 = ((y + b) >> i);
        GD_FT_Fixed v2 = ((x + b) >> i);
        if (theta < 0) {
            xtemp = x + v1;
            y = y - v2;
            x = xtemp;
...
static void ft_trig_pseudo_polarize(GD_FT_Vector* vec)
{
    ...
    if (y > x) {
        if (y > -x) {
            ...
            theta = y > 0 ? GD_FT_ANGLE_PI : -GD_FT_ANGLE_PI;
            x = -x;
            y = -y;
        }
    } else {
    ...
    /* round theta */
    if (theta >= 0)
        theta = GD_FT_PAD_ROUND(theta, 32);
    else
        theta = -GD_FT_PAD_ROUND(-theta, 32);
...
GD_FT_Fixed GD_FT_Tan(GD_FT_Angle angle)
{
    ...
    return GD_FT_DivFix(v.y, v.x);
}

/* documentation is in fttrigon.h */

GD_FT_Angle GD_FT_Atan2(GD_FT_Fixed dx, GD_FT_Fixed dy)
{
    GD_FT_Vector v;
...
#define GD_FT_SIGN_LONG(x) ((x) >> (GD_FT_SIZEOF_LONG * 8 - 1))
#define GD_FT_SIGN_INT(x) ((x) >> (GD_FT_SIZEOF_INT * 8 - 1))
#define GD_FT_SIGN_INT32(x) ((x) >> 31)
#define GD_FT_SIGN_INT16(x) ((x) >> 15)
...
void GD_FT_Vector_Rotate(GD_FT_Vector* vec, GD_FT_Angle angle)
{
    ...
    if (angle && (v.x != 0 || v.y != 0)) {
        ...
        if (shift > 0) {
            GD_FT_Int32 half = (GD_FT_Int32)1L << (shift - 1);

            vec->x = (v.x + half + GD_FT_SIGN_LONG(v.x)) >> shift;
            vec->y = (v.y + half + GD_FT_SIGN_LONG(v.y)) >> shift;
        } else {
            shift = -shift;
            vec->x = (GD_FT_Pos)((GD_FT_ULong)v.x << shift);
            vec->y = (GD_FT_Pos)((GD_FT_ULong)v.y << shift);
        }
    }
}
...
GD_FT_Fixed GD_FT_Vector_Length(GD_FT_Vector* vec)
{
    ...
    if (v.x == 0) {
        return GD_FT_ABS(v.y);
    } else if (v.y == 0) {
        return GD_FT_ABS(v.x);
    }
