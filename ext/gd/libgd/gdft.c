...
#define GD_FT_IS_PATH(path)                                                   \
	(IS_ABSOLUTE_PATH(path, strlen(path)) || strchr(path, '/') ||             \
	 strchr(path, '\\'))
 ...
 */
BGD_DECLARE(char *)
gdImageStringTTF(gdImagePtr im, int *brect, int fg, const char *fontlist,
				 double ptsize, double angle, int x, int y,
				 const char *string) {
	/* 2.0.6: valid return */
	return gdImageStringFT(im, brect, fg, fontlist, ptsize, angle, x, y,
						   string);
}
...
static ssize_t textLayout(uint32_t *text, size_t len, FT_Face face,
						  gdFTStringExtraPtr strex, glyphInfo **glyph_info) {
	size_t count;
	glyphInfo *info;
	...
	for (count = 0; count < len; count++) {
		...
		if (!(strex && (strex->flags & gdFTEX_DISABLE_KERNING)) &&
			!FT_IS_FIXED_WIDTH(face) && FT_HAS_KERNING(face) && previous &&
			glyph_index)
...
static int tweenColorTest(void *element, void *key) {
	...
	return (a->pixel == b->pixel && a->bgcolor == b->bgcolor &&
	...
	if (fg < 0) {
		...
		if (im->trueColor) {
			...
			a->tweencolor = gdTrueColorAlpha(
				gdTrueColorGetRed(fg), gdTrueColorGetGreen(fg),
				gdTrueColorGetBlue(fg),
				gdAlphaMax - (gdTrueColorGetAlpha(fg) * pixel / GD_NUMCOLORS));
		} else {
			a->tweencolor = gdImageColorResolve(
				im, (pixel * im->red[fg] + npixel * im->red[bg]) / GD_NUMCOLORS,
...
static char *gdft_draw_bitmap(gdCache_head_t *tc_cache, gdImagePtr im, int fg,
			...
			for (col = 0; col < bitmap.width; col++, pc++) {
				...
				if ((fg >= 0) && (im->trueColor)) {
					/* Consider alpha in the foreground color itself to be an
					   ...
					   color indexes. */
					level = level * (gdAlphaMax - gdTrueColorGetAlpha(fg)) /
							gdAlphaMax;
				}
				...
				if (fg < 0) {
					...
					if (im->alphaBlendingFlag) {
						...
						if (gdTrueColorGetAlpha(opixel) != gdAlphaTransparent) {
							*tpixel = gdAlphaBlend(opixel, (level << 24) +
															   (fg & 0xFFFFFF));
						} else {
	...
	for (row = 0; row < bitmap.rows; row++) {
		...
		for (col = 0; col < bitmap.width; col++, pc++) {
			if (bitmap.pixel_mode == ft_pixel_mode_grays) {
				...
				/* 2.0.5: mode_mono fix from Giuliano Pochini */
				tc_key.pixel =
					((bitmap.buffer[(col >> 3) + pcr]) & (1 << (~col & 0x07)))
						? GD_NUMCOLORS
						: 0;
			} else {
				return "Unsupported ft_pixel_mode";
...
BGD_DECLARE(void) gdFontCacheMutexSetup(void) {
	gdMutexSetup(gdFontCacheMutex);
}

BGD_DECLARE(void) gdFontCacheMutexShutdown(void) {
...
gdImageStringFT(gdImagePtr im, int *brect, int fg, const char *fontlist,
	...
	return gdImageStringFTEx(im, brect, fg, fontlist, ptsize, angle, x, y,
							 string, 0);
}
...
gdImageStringFTEx(gdImagePtr im, int *brect, int fg, const char *fontlist,
	...
	FT_Matrix matrix;
	FT_Vector penf, oldpenf, total_min = {0, 0}, total_max = {0, 0}, glyph_min,
							 glyph_max;
	FT_Face face;
	...
	tc_cache = gdCacheCreate(TWEENCOLORCACHESIZE, tweenColorTest,
							 tweenColorFetch, tweenColorRelease);
	...
	if (strex)
		fontkey.flags =
			strex->flags & (gdFTEX_FONTPATHNAME | gdFTEX_FONTCONFIG);
	else
		fontkey.flags = 0;
	...
	if (FT_Set_Char_Size(face, 0, (FT_F26Dot6)(ptsize * 64), METRIC_RES,
						 METRIC_RES)) {
		gdCacheDelete(tc_cache);
		gdMutexUnlock(gdFontCacheMutex);
	...
	for (i = 0; i < count; i++) {
		...
		if (render) {
			...
			gdft_draw_bitmap(tc_cache, im, fg, bm->bitmap,
							 (int)(x +
								   (pen_x * cos_a + pen_y * sin_a) * hdpi /
									   (METRIC_RES * 64) +
								   bm->left),
							 (int)(y -
								   (pen_x * sin_a - pen_y * cos_a) * vdpi /
									   (METRIC_RES * 64) -
								   bm->top));
	...
	for (i = 0; i < sizeof(postscript_alias) / sizeof(*postscript_alias); i++) {
		if (strcmp(fontname, postscript_alias[i].name) == 0) {
			...
			FcPattern *pattern = FcPatternBuild(
				0, FC_FAMILY, FcTypeString, postscript_alias[i].family,
				FC_STYLE, FcTypeString, postscript_alias[i].style, (char *)0);
...
static char *font_path(char **fontpath, char *name_list) {
	...
	*fontpath = NULL;
	...
	for (name = gd_strtok_r(fontlist, LISTSEPARATOR, &strtok_ptr); name;
		...
		fullname =
			gdReallocEx(fullname, strlen(fontsearchpath) + strlen(name) + 8);
		if (!fullname) {
			gdFree(fontlist);
			gdFree(path);
