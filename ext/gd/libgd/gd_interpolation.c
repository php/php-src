...
typedef struct {
   ContributionType *ContribRow; /* Row (or column) of contribution weights */
	unsigned int
		WindowSize, /* Filter window size (of affecting source pixels) */
...
static double filter_blackman(const double x, const double support) {
	ARG_NOT_USED(support);
	return (0.42f + 0.5f * (double)cos(M_PI * x) +
			0.08f * (double)cos(2.0f * M_PI * x));
}
...
static double filter_lanczos8(const double x1, const double support) {
	...
	if (x < support) {
		return support * sin(x * M_PI) * sin(x * M_PI / support) /
			   (x * M_PI * x * M_PI);
	}
	return 0.0;
}
...
static inline int _color_blend(const int dst, const int src) {
    ...
    if( src_alpha == gdAlphaOpaque ) {
		...
		if( dst_alpha == gdAlphaTransparent ) {
			...
			const int dst_weight =
				(gdAlphaTransparent - dst_alpha) * src_alpha / gdAlphaMax;
...
static inline int _setEdgePixel(const gdImagePtr src, unsigned int x,
								unsigned int y, gdFixed coverage,
								const int bgColor) {
...
static inline int getPixelOverflowTCClipped(gdImagePtr im, const int x,
											const int y, const int bgColor,
											const gdRectPtr clip) {
	if (clip != NULL &&
		(x < clip->x || y < clip->y || x >= clip->x + clip->width ||
...
#define colorIndex2RGBA(c)                                                     \
	gdTrueColorAlpha(im->red[(c)], im->green[(c)], im->blue[(c)],              \
					 im->alpha[(c)])
#define colorIndex2RGBcustomA(c, a)                                            \
	gdTrueColorAlpha(im->red[(c)], im->green[(c)], im->blue[(c)],              \
					 im->alpha[(a)])
static inline int
getPixelOverflowPalette(gdImagePtr im, const int x, const int y,
						const int bgColor /* 31bit ARGB TC */) {
	if (gdImageBoundsSafe(im, x, y)) {
...
static inline int getPixelOverflowPaletteClipped(gdImagePtr im, const int x,
												 const int y, const int bgColor,
												 const gdRectPtr clip) {
	if (clip != NULL &&
		(x < clip->x || y < clip->y || x >= clip->x + clip->width ||
...
static inline interpolation_method
gdImageGetEffectiveInterpolation(const gdImagePtr im) {
...
static int getPixelInterpolateWeightClipped(gdImagePtr im, const double x,
											const double y, const int bgColor,
											const gdRectPtr clip) {
	/* Closest pixel <= (xf,yf) */
	int sx = (int)(x);
	int sy = (int)(y);
	...
	const int c1 =
		im->trueColor == 1
			? getPixelOverflowTCClipped(im, sx, sy, bgColor, clip)
	...
	r = (int)(m1 * gdTrueColorGetRed(c1) + m2 * gdTrueColorGetRed(c2) +
			  m3 * gdTrueColorGetRed(c3) + m4 * gdTrueColorGetRed(c4));
 ...
 */
static int getPixelInterpolatedClipped(gdImagePtr im, const double x,
									   const double y, const int bgColor,
									   const gdRectPtr clip) {
	const int xi=(int)(x);
	const int yi=(int)(y);
	...
	const interpolation_method interpolation =
		gdImageGetEffectiveInterpolation(im);
	...
	for (yii = yi-1; yii < yi+3; yii++) {
		...
		if (im->trueColor) {
			for (xii=xi-1; xii<xi+3; xii++) {
				const int rgbs =
					getPixelOverflowTCClipped(im, xii, yii, bgColor, clip);
			...
			for (xii=xi-1; xii<xi+3; xii++) {
				const int rgbs =
					getPixelOverflowPaletteClipped(im, xii, yii, bgColor, clip);
...
_gdContributionsAlloc(unsigned int line_length, unsigned int windows_size) {
	...
	res->ContribRow =
		(ContributionType *)gdMalloc(line_length * sizeof(ContributionType));
	...
	for (iSrc = iLeft; iSrc <= iRight; iSrc++) {
			dTotalWeight +=
				(res->ContribRow[u].Weights[iSrc - iLeft] =
					 scale_f_d *
					 (*pFilter)(scale_f_d * (dCenter - (double)iSrc), support));
	}
...
static inline void _gdScaleOneAxis(gdImagePtr pSrc, gdImagePtr dst,
	...
	for (ndx = 0; ndx < dst_len; ndx++) {
		...
		int *dest = (axis == HORIZONTAL) ? &dst->tpixels[row][ndx]
										 : &dst->tpixels[ndx][row];
	...
	for (i = left; i <= right; i++) {
		const int left_channel = i - left;
			const int srcpx = (axis == HORIZONTAL) ? pSrc->tpixels[row][i]
												   : pSrc->tpixels[i][row];
...
static inline int _gdScalePass(const gdImagePtr pSrc,
...
static gdImagePtr gdImageScaleTwoPass(const gdImagePtr src,
									  const unsigned int new_width,
									  const unsigned int new_height) {
	...
	if (src_width == new_width) {
		...
		scale_pass_res = _gdScalePass(src, src_width, tmp_im, new_width,
	...
	if (dst != NULL) {
		gdImageSetInterpolationMethod(dst, src->interpolation_id);
			scale_pass_res = _gdScalePass(tmp_im, src_height, dst, new_height,
										  new_width, VERTICAL, filter);
		if (scale_pass_res != 1) {
			gdImageDestroy(dst);
				if (src != tmp_im) {
			gdImageDestroy(tmp_im);
	...
	for (i=0; i<new_height; i++) {
		...
		if (im->trueColor) {
			for (j=0; j<new_width; j++) {
				const long m =
					gdClampInt((int)((double)i * dy), 0, gdImageSY(im) - 1);
				const long n =
					gdClampInt((int)((double)j * dx), 0, gdImageSX(im) - 1);

				dst_img->tpixels[dst_offset_y][dst_offset_x++] =
					im->tpixels[m][n];
			}
		} else {
			for (j=0; j<new_width; j++) {
...
static gdImagePtr gdImageScaleBilinearPalette(gdImagePtr im,
	...
	if (transparent < 0) {
		...
		new_img->transparent =
			gdTrueColorAlpha(im->red[transparent], im->green[transparent],
		...
		for (j=0; j < _width; j++) {
...

			{
				const unsigned char red =
					uchar_clamp(f_w1 * gdTrueColorGetRed(pixel1) +
									f_w2 * gdTrueColorGetRed(pixel2) +
									f_w3 * gdTrueColorGetRed(pixel3) +
									f_w4 * gdTrueColorGetRed(pixel4),
								0xFF);
				const unsigned char green =
					uchar_clamp(f_w1 * gdTrueColorGetGreen(pixel1) +
									f_w2 * gdTrueColorGetGreen(pixel2) +
									f_w3 * gdTrueColorGetGreen(pixel3) +
									f_w4 * gdTrueColorGetGreen(pixel4),
								0xFF);
				const unsigned char blue =
					uchar_clamp(f_w1 * gdTrueColorGetBlue(pixel1) +
									f_w2 * gdTrueColorGetBlue(pixel2) +
									f_w3 * gdTrueColorGetBlue(pixel3) +
									f_w4 * gdTrueColorGetBlue(pixel4),
								0xFF);
				const unsigned char alpha =
					uchar_clamp(f_w1 * gdTrueColorGetAlpha(pixel1) +
									f_w2 * gdTrueColorGetAlpha(pixel2) +
									f_w3 * gdTrueColorGetAlpha(pixel3) +
									f_w4 * gdTrueColorGetAlpha(pixel4),
								0x7F);
...
static gdImagePtr gdImageScaleBilinearTC(gdImagePtr im,
										 const unsigned int new_width,
										 const unsigned int new_height) {
	...
	*/
	...
	for (i=0; i < new_height; i++) {
		...
		for (j=0; j < new_width; j++) {
			...
			const double f_g = src_x - (double)n;
			long k;
			double red_acc = 0.0, green_acc = 0.0, blue_acc = 0.0,
				   alpha_acc = 0.0;
			...
			for (k = -1; k < 3; k++) {
				int l;
				const int src_y_sample =
					gdClampInt((int)m + (int)k, 0, src_h - 1);
				const double weight_y =
					filter_cubic_spline((double)k - f_f, 0.0);

				for (l = -1; l < 3; l++) {
					const int src_x_sample =
						gdClampInt((int)n + (int)l, 0, src_w - 1);
					const double weight =
						weight_y * filter_cubic_spline((double)l - f_g, 0.0);
...
static int gdRotatedImageSize(gdImagePtr src, const float angle,
							  gdRectPtr bbox) {
    gdRect src_area;
    double m[6];
 ...
 */
BGD_DECLARE(gdImagePtr)
gdImageRotateInterpolated(const gdImagePtr src, const float angle,
						  int bgcolor) {
	...
	if (!gdImageTrueColor(src)) {
		if (bgcolor < gdMaxColors) {
			bgcolor = gdTrueColorAlpha(src->red[bgcolor], src->green[bgcolor],
									   src->blue[bgcolor], src->alpha[bgcolor]);
		}
...
static inline int gdAffineSampleIntersectsRegion(const double x, const double y,
	...
	const double right =
		(double)(region->x + region->width - 1) + GD_AFFINE_KERNEL_SUPPORT;
	const double bottom =
		(double)(region->y + region->height - 1) + GD_AFFINE_KERNEL_SUPPORT;

	return x >= left && x <= right && y >= top && y <= bottom;
}

static inline int gdAffineSample(const gdAffineSampleContext *ctx,
								 const double x, const double y, int *color) {
	int c;
...
gdTransformAffineCopy(gdImagePtr dst, int dst_x, int dst_y,
	...
	if (src_region->x > 0 || src_region->y > 0 ||
		...
		gdImageGetClip(src, &backup_clipx1, &backup_clipy1, &backup_clipx2,
					   &backup_clipy2);
	...
	if (!gdTransformAffineBoundingBox(src_region, affine, &bbox)) {
		if (backclip) {
			gdImageSetClip(src, backup_clipx1, backup_clipy1, backup_clipx2,
						   backup_clipy2);
		}
		return GD_FALSE;
	}
...
gdTransformAffineBoundingBox(gdRectPtr src, const double affine[6],
	...
	if (!isfinite(bbox_x_d) || !isfinite(bbox_y_d) || !isfinite(bbox_width_d) ||
		...
		bbox_y_d < INT_MIN || bbox_y_d > INT_MAX || bbox_width_d < 0.0 ||
		bbox_width_d > INT_MAX || bbox_height_d < 0.0 ||
		bbox_height_d > INT_MAX) {
		return GD_FALSE;
	}
	bbox_x = (int)bbox_x_d;
	bbox_y = (int)bbox_y_d;
