...
static int _gdImageGifAnimAddCtx(gdImagePtr im, gdIOCtxPtr out, int LocalCM,
...
static int _gdImageGifCtx(gdImagePtr im, gdIOCtxPtr out) {
	...
	/* All set, let's do it. */
	GIFEncode(out, tim->sx, tim->sy, interlace, 0, tim->transparent,
...
gdImageGifAnimAddPtr(gdImagePtr im, int *size, int LocalCM, int LeftOfs,
	...
	if (out == NULL)
		return NULL;
	if (!_gdImageGifAnimAddCtx(im, out, LocalCM, LeftOfs, TopOfs, Delay,
							   Disposal, previm)) {
		rv = gdDPExtractData(out, size);
	} else {
		rv = NULL;
...
gdImageGifAnimAddCtx(gdImagePtr im, gdIOCtxPtr out, int LocalCM, int LeftOfs,
					 int TopOfs, int Delay, int Disposal, gdImagePtr previm) {
	_gdImageGifAnimAddCtx(im, out, LocalCM, LeftOfs, TopOfs, Delay, Disposal,
						  previm);
}
...
static int _gdImageGifAnimAddCtx(gdImagePtr im, gdIOCtxPtr out, int LocalCM,
	...
	/* All set, let's do it. */
	GIFAnimEncode(out, tim->sx, tim->sy, LeftOfs, TopOfs, interlace,
				  transparent, Delay, Disposal, BitsPerPixel,
