...
static char *date_format(char *format, php_size_t format_len, timelib_time *t, int localtime)
{
	smart_str            string = {0};
	php_size_t    i, length = 0;
	char                 buffer[97];
...
PHPAPI char *php_format_date(char *format, php_size_t format_len, time_t ts, int localtime TSRMLS_DC) /* {{{ */
{
	timelib_time   *t;
	timelib_tzinfo *tzi;
	char *string;
...
PHPAPI int php_date_initialize(php_date_obj *dateobj, /*const*/ char *time_str, php_size_t time_str_len, char *format, zval *timezone_object, int ctor TSRMLS_DC)
{
	timelib_time   *now;
	timelib_tzinfo *tzi = NULL;
...
PHP_FUNCTION(date_create_from_format)
{
	...
	php_size_t   time_str_len = 0, format_str_len = 0;
		...
		if (zend_hash_find(myht, "timezone_type", 14, (void**) &z_timezone_type) == SUCCESS) {
			...
			if (zend_hash_find(myht, "timezone", 9, (void**) &z_timezone) == SUCCESS) {
				...
				switch (Z_IVAL_PP(z_timezone_type)) {
					case TIMELIB_ZONETYPE_OFFSET:
					case TIMELIB_ZONETYPE_ABBR: {
						char *tmp = emalloc(Z_STRSIZE_PP(z_date) + Z_STRSIZE_PP(z_timezone) + 2);
						int ret;
						snprintf(tmp, Z_STRSIZE_PP(z_date) + Z_STRSIZE_PP(z_timezone) + 2, "%s %s", Z_STRVAL_PP(z_date), Z_STRVAL_PP(z_timezone));
						ret = php_date_initialize(*dateobj, tmp, Z_STRSIZE_PP(z_date) + Z_STRSIZE_PP(z_timezone) + 1, NULL, NULL, 0 TSRMLS_CC);
						efree(tmp);
						return 1 == ret;
					}
					...
					case TIMELIB_ZONETYPE_ID: {
						...
						ret = php_date_initialize(*dateobj, Z_STRVAL_PP(z_date), Z_STRSIZE_PP(z_date), NULL, tmp_obj, 0 TSRMLS_CC);
...
{
	char                           *date, *format;
	php_size_t                   date_len, format_len;
	struct timelib_error_container *error;
	timelib_time                   *parsed_time;
...
static int php_date_modify(zval *object, char *modify, php_size_t modify_len TSRMLS_DC)
{
	php_date_obj *dateobj;
...
static int date_interval_initialize(timelib_rel_time **rt, /*const*/ char *format, php_size_t format_length TSRMLS_DC)
{
...
#define SET_VALUE_FROM_STRUCT(n,m)            \
	if (strcmp(Z_STRVAL_P(member), m) == 0) { \
		if (value->type != IS_INT) {         \
			...
			convert_to_int(&tmp_value);      \
			value = &tmp_value;               \
		}                                     \
		obj->diff->n = Z_IVAL_P(value);       \
		if (value == &tmp_value) {            \
			zval_dtor(value);                 \
		}                                     \
		break;								  \
...
PHP_METHOD(DateInterval, __construct)
{
	...
	php_size_t interval_string_length;
...
#define PHP_DATE_INTERVAL_READ_PROPERTY(element, member, itype, def) \
	do { \
		...
		if (zend_hash_find(myht, element, strlen(element) + 1, (void**) &z_arg) == SUCCESS) { \
			convert_to_int(*z_arg); \
			(*intobj)->diff->member = (itype)Z_IVAL_PP(z_arg); \
		} else { \
...
static char *date_interval_format(char *format, php_size_t format_len, timelib_rel_time *t)
{
	smart_str            string = {0};
	php_size_t        i, length;
...
static int date_period_initialize(timelib_time **st, timelib_time **et, timelib_rel_time **d, php_int_t *recurrences, /*const*/ char *format, php_size_t format_length TSRMLS_DC)
{
	timelib_time     *b = NULL, *e = NULL;
	timelib_rel_time *p = NULL;
	int               r = 0;
...
static int php_date_period_initialize_from_hash(php_period_obj *period_obj, HashTable *myht TSRMLS_DC)
{
	...
	if (zend_hash_find(myht, "recurrences", sizeof("recurrences"), (void**) &ht_entry) == SUCCESS &&
			Z_TYPE_PP(ht_entry) == IS_INT && Z_IVAL_PP(ht_entry) >= 0 && Z_IVAL_PP(ht_entry) <= INT_MAX) {
		period_obj->recurrences = Z_IVAL_PP(ht_entry);
	} else {
		return 0;
	}
