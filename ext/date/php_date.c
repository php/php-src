...
static PHP_INI_MH(OnUpdate_date_timezone)
{
	if (new_value && !timelib_timezone_id_is_valid(ZSTR_VAL(new_value), DATE_TIMEZONEDB)) {
		const char *format = "Invalid date.timezone value '%S', using '%s' instead";
		php_error_docref(
			NULL, E_WARNING,
			format,
			new_value,
...
PHPAPI bool php_date_initialize(php_date_obj *dateobj, const zend_string *time_string, const char *format, zval *timezone_object, int flags) /* {{{ */
{
	timelib_time   *now;
	timelib_tzinfo *tzi = NULL;
	...
	if (time_string) {
		time_str = ZSTR_VAL(time_string);
		time_str_len = ZSTR_LEN(time_string);
	}
	...
	if ((flags & PHP_DATE_INIT_CTOR) && err && err->error_count) {
		...
		const char *exception_format = "Failed to parse time string (%S) at position %d (%c): %s";
		zend_throw_exception_ex(date_ce_date_malformed_string_exception, 0, exception_format, time_string,
...
PHP_FUNCTION(date_create)
{
	...
	if (!php_date_initialize(Z_PHPDATE_P(return_value), time_str, NULL, timezone_object, 0)) {
...
PHP_FUNCTION(date_create_from_format)
{
	...
	if (UNEXPECTED(zend_str_has_nul_byte(time_str))) {
	...
	if (!php_date_initialize(Z_PHPDATE_P(return_value), time_str, format_str, timezone_object, PHP_DATE_INIT_FORMAT)) {
		zval_ptr_dtor(return_value);
		RETURN_FALSE;
	}
}
...
PHP_METHOD(DateTime, __construct)
{
	...
	if (time_str && UNEXPECTED(zend_str_has_nul_byte(time_str))) {
		zend_argument_value_error(2, "must not contain any null bytes");
		RETURN_THROWS();
	}

	php_date_initialize(Z_PHPDATE_P(ZEND_THIS), time_str, NULL, timezone_object, PHP_DATE_INIT_CTOR);
}
/* }}} */
...
static bool php_date_initialize_from_hash(php_date_obj **dateobj, const HashTable *myht)
{
	...
	switch (Z_LVAL_P(z_timezone_type)) {
		...
		case TIMELIB_ZONETYPE_ABBR: {
			...
			bool ret = php_date_initialize(*dateobj, tmp, NULL, NULL, 0);
...
static bool php_date_modify(zval *object, zend_string *modify) /* {{{ */
{
	php_date_obj *dateobj;
	...
	tmp_time = timelib_strtotime(ZSTR_VAL(modify), ZSTR_LEN(modify), &err, DATE_TIMEZONEDB, php_date_parse_tzfile_wrapper);

	/* update last errors and warnings */
	update_errors_warnings(&err);

	if (err && err->error_count) {
		const char *format = "Failed to parse time string (%S) at position %d (%c): %s";
		/* spit out the first library error message, at least */
		php_error_docref(NULL, E_WARNING, format, modify,
...
PHP_FUNCTION(date_modify)
{
	...
	if (zend_parse_method_parameters(ZEND_NUM_ARGS(), getThis(), "OS", &object, date_ce_date, &modify) == FAILURE) {
		RETURN_THROWS();
	}

	if (!php_date_modify(object, modify)) {
		RETURN_FALSE;
	}
...
PHP_METHOD(DateTimeImmutable, modify)
{
	...
	zend_replace_error_handling(EH_THROW, date_ce_date_malformed_string_exception, &zeh);
	if (!php_date_modify(&new_object, modify)) {
		zval_ptr_dtor(&new_object);
		zend_restore_error_handling(&zeh);
		RETURN_THROWS();
	}
...
static bool date_interval_initialize(timelib_rel_time **rt, const zend_string *format) /* {{{ */
{
	...
	timelib_strtointerval(ZSTR_VAL(format), ZSTR_LEN(format), &b, &e, &p, &r, &errors);

	if (errors->error_count > 0) {
		const char *exception_format = "Unknown or bad format (%S)";
		zend_throw_exception_ex(date_ce_date_malformed_interval_string_exception, 0, exception_format, format);
		retval = false;
		if (p) {
			...
			*rt = p;
			...
			if (b && e) {
				...
				*rt = timelib_diff(b, e);
				...
				const char *exception_format = "Failed to parse interval (%S)";
				zend_throw_exception_ex(date_ce_date_malformed_interval_string_exception, 0, exception_format, format);
				retval = false;
			}
		}
...
PHP_METHOD(DateInterval, __construct)
{
	...
	if (UNEXPECTED(zend_str_has_nul_byte(interval_string))) {
...
static void php_date_interval_initialize_from_hash(php_interval_obj *intobj, const HashTable *myht) /* {{{ */
{
	...
	if (date_str && Z_TYPE_P(date_str) == IS_STRING) {
		...
		if (err->error_count > 0)  {
			const zend_string *date_string;
			if (Z_TYPE_P(date_str) == IS_STRING) {
				date_string = Z_STR_P(date_str);
			} else {
				date_string = ZSTR_EMPTY_ALLOC();
			}
			const char *format = "Unknown or bad format (%S) at position %d (%c) while unserializing: %s";
...
PHP_FUNCTION(date_interval_create_from_date_string)
{
	...
	if (err->error_count > 0)  {
		const char *format = "Unknown or bad format (%S) at position %d (%c): %s";
		php_error_docref(NULL, E_WARNING, format, time_str,
			err->error_messages[0].position, err->error_messages[0].character ? err->error_messages[0].character : ' ', err->error_messages[0].message);
		RETVAL_FALSE;
		goto cleanup;
	}
...
PHP_METHOD(DateInterval, createFromDateString)
{
	...
	if (err->error_count > 0)  {
		const char *format = "Unknown or bad format (%S) at position %d (%c): %s";
		zend_throw_exception_ex( date_ce_date_malformed_interval_string_exception, 0, format, time_str,
			err->error_messages[0].position, err->error_messages[0].character ? err->error_messages[0].character : ' ', err->error_messages[0].message);
		goto cleanup;
	}
...
static bool date_period_initialize(timelib_time **st, timelib_time **et, timelib_rel_time **d, zend_long *recurrences, const zend_string *format) /* {{{ */
{
	timelib_time     *b = NULL, *e = NULL;
	timelib_rel_time *p = NULL;
	int               r = 0;
	...
	if (errors->error_count > 0) {
		retval = false;
		const char *exception_format = "Unknown or bad format (%S)";
		zend_throw_exception_ex(date_ce_date_malformed_period_string_exception, 0, exception_format, format);
		if (b) {
			timelib_time_dtor(b);
		}
		if (e) {
...
static bool date_period_init_iso8601_string(php_period_obj *dpobj, zend_class_entry* base_ce, const zend_string *isostr, zend_long *recurrences)
{
	if (!date_period_initialize(&(dpobj->start), &(dpobj->end), &(dpobj->interval), recurrences, isostr)) {
		return false;
	}

	if (dpobj->start == NULL) {
		zend_string *func = get_active_function_or_method_name();
		zend_throw_exception_ex(date_ce_date_malformed_period_string_exception, 0, "%s(): ISO interval must contain a start date, \"%s\" given", ZSTR_VAL(func), ZSTR_VAL(isostr));
		zend_string_release(func);
		return false;
	}
	if (dpobj->interval == NULL) {
		zend_string *func = get_active_function_or_method_name();
		zend_throw_exception_ex(date_ce_date_malformed_period_string_exception, 0, "%s(): ISO interval must contain an interval, \"%s\" given", ZSTR_VAL(func), ZSTR_VAL(isostr));
		zend_string_release(func);
		return false;
	}
	if (dpobj->end == NULL && *recurrences == 0) {
		zend_string *func = get_active_function_or_method_name();
		zend_throw_exception_ex(date_ce_date_malformed_period_string_exception, 0, "%s(): ISO interval must contain an end date or a recurrence count, \"%s\" given", ZSTR_VAL(func), ZSTR_VAL(isostr));
		zend_string_release(func);
		return false;
	}
...
PHP_METHOD(DatePeriod, createFromISO8601String)
{
	...
	if (!date_period_init_iso8601_string(dpobj, date_ce_immutable, isostr, &recurrences)) {
		RETURN_THROWS();
	}
...
PHP_METHOD(DatePeriod, __construct)
{
	...
	if (zend_parse_parameters_ex(ZEND_PARSE_PARAMS_QUIET, ZEND_NUM_ARGS(), "OOl|l", &start, date_ce_interface, &interval, date_ce_interval, &recurrences, &options) == FAILURE) {
		if (zend_parse_parameters_ex(ZEND_PARSE_PARAMS_QUIET, ZEND_NUM_ARGS(), "OOO|l", &start, date_ce_interface, &interval, date_ce_interval, &end, date_ce_interface, &options) == FAILURE) {
			if (zend_parse_parameters_ex(ZEND_PARSE_PARAMS_QUIET, ZEND_NUM_ARGS(), "S|l", &isostr, &options) == FAILURE) {
	...
	if (isostr && UNEXPECTED(zend_str_has_nul_byte(isostr))) {
		zend_argument_value_error(1, "must not contain any null bytes");
		RETURN_THROWS();
	}
	...
	if (isostr) {
		...
		if (!date_period_init_iso8601_string(dpobj, date_ce_date, isostr, &recurrences)) {
			RETURN_THROWS();
		}
	} else {
...
PHP_FUNCTION(date_default_timezone_set)
{
	...
	if (!timelib_timezone_id_is_valid(ZSTR_VAL(zone), DATE_TIMEZONEDB)) {
		const char *format = "Timezone ID '%S' is invalid";
		php_error_docref(NULL, E_NOTICE, format, zone);
		RETURN_FALSE;
	}
	if (DATEG(timezone)) {
		efree(DATEG(timezone));
