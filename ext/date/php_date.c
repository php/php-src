...
#define PHP_DATE_USER_CACHE_STATE_DATE "date"
#define PHP_DATE_USER_CACHE_STATE_DATE_LEN (sizeof(PHP_DATE_USER_CACHE_STATE_DATE) - 1)
#define PHP_DATE_USER_CACHE_STATE_TIMEZONE_TYPE "timezone_type"
#define PHP_DATE_USER_CACHE_STATE_TIMEZONE_TYPE_LEN (sizeof(PHP_DATE_USER_CACHE_STATE_TIMEZONE_TYPE) - 1)
...
static void php_date_user_cache_add_assoc_int64(zval *array_zv, const char *name, int64_t value)
{
#if PHP_DATE_SIZEOF_LONG == 8
	add_assoc_long(array_zv, name, (zend_long) value);
#else
	add_assoc_str(array_zv, name, zend_strpprintf(0, "%lld", (long long) value));
#endif
}
...
static bool php_date_serialize_datetime_user_cache_state(php_date_obj *dateobj, zval *state)
{
	...
	ZVAL_NULL(&zv);
	zend_hash_str_update(Z_ARRVAL_P(state), PHP_DATE_USER_CACHE_STATE_DATE, PHP_DATE_USER_CACHE_STATE_DATE_LEN, &zv);

	ZVAL_LONG(&zv, dateobj->time->zone_type);
	zend_hash_str_update(Z_ARRVAL_P(state), PHP_DATE_USER_CACHE_STATE_TIMEZONE_TYPE, PHP_DATE_USER_CACHE_STATE_TIMEZONE_TYPE_LEN, &zv);
	...
	zend_hash_str_update(Z_ARRVAL_P(state), PHP_DATE_USER_CACHE_STATE_TIMEZONE, PHP_DATE_USER_CACHE_STATE_TIMEZONE_LEN, &zv);
...
static bool php_date_serialize_user_cache_state(const zval *object, zval *state)
{
	php_date_obj *dateobj;
	php_timezone_obj *tzobj;
	php_interval_obj *intervalobj;
	...
	if (instanceof_function(Z_OBJCE_P(object), date_ce_date) ||
		...
		dateobj = Z_PHPDATE_P((zval *) object);

		if (dateobj->time == NULL || !dateobj->time->is_localtime) {
			return false;
		}

		return php_date_serialize_datetime_user_cache_state(dateobj, state);
	}
	...
	if (instanceof_function(Z_OBJCE_P(object), date_ce_interval)) {
		...
		if (intervalobj->from_string) {
			...
			array_init_size(state, 2);
			add_assoc_bool(state, "from_string", true);
			add_assoc_str(state, "date_string", zend_string_copy(intervalobj->date_string));

			return true;
		}

		array_init_size(state, 18);
		add_assoc_long(state, "y", intervalobj->diff->y);
		add_assoc_long(state, "m", intervalobj->diff->m);
		add_assoc_long(state, "d", intervalobj->diff->d);
		add_assoc_long(state, "h", intervalobj->diff->h);
		add_assoc_long(state, "i", intervalobj->diff->i);
		add_assoc_long(state, "s", intervalobj->diff->s);
		add_assoc_double(state, "f", (double) intervalobj->diff->us / 1000000.0);
		add_assoc_long(state, "invert", intervalobj->diff->invert);

		if (intervalobj->diff->days != TIMELIB_UNSET) {
			php_date_user_cache_add_assoc_int64(state, "days", intervalobj->diff->days);
		} else {
			add_assoc_bool(state, "days", false);
		}

		add_assoc_bool(state, "from_string", false);
...
static bool php_date_unserialize_user_cache_state(zval *object, zval *state)
{
	...
	if (instanceof_function(Z_OBJCE_P(object), date_ce_date) ||
		...
		if (php_date_unserialize_datetime_user_cache_state(object, state)) {
			return true;
		}
		if (EG(exception)) {
			return false;
...
static const zend_opcache_user_cache_safe_direct_handlers *php_date_get_user_cache_handlers(void)
{
	static const zend_opcache_user_cache_safe_direct_handlers handlers = {
		true,
		php_date_copy_user_cache_state,
		NULL,
		php_date_serialize_user_cache_state,
...
static void php_date_register_user_cache_handlers(void)
{
	...
	handlers = php_date_get_user_cache_handlers();
	zend_opcache_user_cache_safe_direct_register_class(date_ce_date, handlers);
	zend_opcache_user_cache_safe_direct_register_class(date_ce_immutable, handlers);
	zend_opcache_user_cache_safe_direct_register_class(date_ce_timezone, handlers);
	zend_opcache_user_cache_safe_direct_register_class(date_ce_interval, handlers);
}
