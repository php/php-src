...
#define PHP_STREAM_POLL_CONTEXT_OBJ_FROM_ZV(_zv) php_stream_poll_context_object_from_zend_object(Z_OBJ_P(_zv))

static inline php_stream_poll_context_object *php_stream_poll_context_object_from_zend_object(zend_object *obj)
{
    return (php_stream_poll_context_object *)((char *)(obj) - XtOffsetOf(php_stream_poll_context_object, std));
}
...
static inline zend_object *php_stream_poll_context_create_object_ex(zend_class_entry *ce, php_poll_ctx **ctx)
{
    php_stream_poll_context_object *intern = zend_object_alloc(sizeof(php_stream_poll_context_object), ce);
...
{
    zend_long max_events_long = 1024, backend_long = PHP_POLL_BACKEND_AUTO;
    bool max_events_is_null = 1;
    php_poll_ctx *poll_ctx;
    
    ZEND_PARSE_PARAMETERS_START(0, 2)
        Z_PARAM_OPTIONAL
        Z_PARAM_LONG_OR_NULL(max_events_long, max_events_is_null)
        Z_PARAM_LONG(backend_long)
    ZEND_PARSE_PARAMETERS_END();

    if (!max_events_is_null && max_events_long <= 0) {
        zend_throw_exception(stream_poll_exception_class_entry, "max_events must be positive", 0);
        RETURN_THROWS();
    }

    int max_events = max_events_is_null ? 1024 : (int)max_events_long;
    php_poll_backend_type backend = (php_poll_backend_type)backend_long;

    poll_ctx = php_poll_create(max_events, backend, false);
    if (!poll_ctx) {
        zend_throw_exception(stream_poll_exception_class_entry, "Failed to create polling context", 0);
        RETURN_THROWS();
    }

    if (php_poll_init(poll_ctx) != SUCCESS) {
        php_poll_destroy(poll_ctx);
        zend_throw_exception(stream_poll_exception_class_entry, "Failed to initialize polling context", 0);
        RETURN_THROWS();
    }
...
{
    if (Z_TYPE_P(obj) != IS_OBJECT || !instanceof_function(Z_OBJCE_P(obj), stream_poll_context_class_entry)) {
        return NULL;
    }
...
PHP_FUNCTION(stream_poll_modify)
{
    ...
    if (php_poll_modify(context->ctx, (int)fd, (uint32_t)events, entry) != SUCCESS) {
        zend_throw_exception(stream_poll_exception_class_entry, "Failed to modify stream in poll", 0);
        RETURN_THROWS();
    }
}
...
PHP_FUNCTION(stream_poll_remove)
{
    ...
    if (php_poll_remove(context->ctx, (int)fd) != SUCCESS) {
        zend_throw_exception(stream_poll_exception_class_entry, "Failed to remove stream from poll", 0);
        RETURN_THROWS();
    }
...
PHP_FUNCTION(stream_poll_wait)
{
    ...
    events = emalloc(sizeof(php_poll_event) * context->max_events);
    
    num_events = php_poll_wait(context->ctx, events, context->max_events, (int)timeout);
...
{
    zend_throw_error(NULL, "Cannot directly construct StreamPollContext, use stream_poll_create() instead");
}
...
PHP_MINIT_FUNCTION(stream_poll)
{
    ...
    stream_poll_context_class_entry->default_object_handlers = &php_stream_poll_context_object_handlers;
    ...
    memcpy(&php_stream_poll_context_object_handlers, &std_object_handlers, sizeof(zend_object_handlers));
    php_stream_poll_context_object_handlers.offset = XtOffsetOf(php_stream_poll_context_object, std);
