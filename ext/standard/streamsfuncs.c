...
PHP_FUNCTION(stream_socket_recvfrom)
{
	...
	if (UNEXPECTED(to_read <= 0)) {
		...
		RETURN_THROWS();
	} else if (ZEND_LONG_ZSTR_LEN_OVFL(to_read)) {
		zend_argument_value_error(2, "must be less than or equal to %zu", ZSTR_MAX_LEN);
		RETURN_THROWS();
	}
...
PHP_FUNCTION(stream_copy_to_stream)
{
	...
	if (maxlen_is_null) {
		maxlen = PHP_STREAM_COPY_ALL;
	} else if (ZEND_LONG_SIZE_T_OVFL(maxlen)) {
	...
	if (php_stream_copy_to_stream_ex(src, dest, (size_t) maxlen, &len) != SUCCESS) {
		RETVAL_FALSE;
	} else {
		RETVAL_LONG(len);
	}
