...
for ($i = 0; $i < count($servers); $i++) {
    pt_stream_poll_add($poll_ctx, $servers[$i], POLL_EVENT_READ, "server{$i}_data");
}
...
$expected_events = [];
for ($i = 0; $i < 20; $i++) {
    $expected_events[] = ['events' => POLL_EVENT_READ, 'data' => "server{$i}_data", 'read' => "test $i data"];
}
...
pt_expect_events($poll_ctx->wait(100), [
    ['events' => POLL_EVENT_READ, 'data' => 'server1_data', 'read' => 'more data'],
