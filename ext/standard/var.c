	...
	switch (Z_TYPE_P(struc)) {
		...
		case IS_OBJECT: {
			...
			if (ce == zend_standard_class_def) {
				smart_str_appends(buf, "(object) array(\n");
			} else {
				smart_str_appendc(buf, '\\');
				...
				if (is_enum) {
					...
					smart_str_appends(buf, "::__set_state(array(\n");
				}
			}
...
again:
	switch (Z_TYPE_P(struc)) {
		case IS_FALSE:
			smart_str_appends(buf, "b:0;");
			return;

		case IS_TRUE:
			smart_str_appends(buf, "b:1;");
			return;
