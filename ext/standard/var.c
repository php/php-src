...
PHP_FUNCTION(unserialize)
{
	...
	if(options != NULL) {
		if(zend_hash_find(Z_ARRVAL_P(options), "allowed_classes", sizeof("allowed_classes"), (void **)&classes) == SUCCESS 
			&& (Z_TYPE_PP(classes) == IS_ARRAY || !zend_is_true(*classes TSRMLS_CC))) {
			ALLOC_HASHTABLE(class_hash);
			zend_hash_init(class_hash, (Z_TYPE_PP(classes) == IS_ARRAY)?zend_hash_num_elements(Z_ARRVAL_PP(classes)):0, NULL, NULL, 0);
		}
		if(class_hash && Z_TYPE_PP(classes) == IS_ARRAY) {
			HashPosition pos;
			HashTable *ht = Z_ARRVAL_PP(classes);
			zval **data;
			char *lc_name;
