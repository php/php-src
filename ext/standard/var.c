...
again:
	switch (Z_TYPE_P(struc)) {
		...
		case IS_ARRAY:
			...
			php_printf("%sarray(%d) {\n", COMMON, count);
			ZEND_ARRAY_FOREACH_KEY_VAL(myht, num, key, val) {
...
PHPAPI void php_debug_zval_dump(zval *struc, int level) /* {{{ */
{
	...
	case IS_ARRAY:
		...
		ZEND_ARRAY_FOREACH_KEY_VAL(myht, index, key, val) {
...
again:
	switch (Z_TYPE_P(struc)) {
		...
		case IS_OBJECT:
			...
			if (myht) {
				if (!is_enum) {
					ZEND_ARRAY_FOREACH_KEY_VAL_IND(myht, index, key, val) {
		...
		HashTable *ht, zval *struc, HashTable *sleep_retval) /* {{{ */
{
	 ...
	 * name manglings? */
	ZEND_ARRAY_FOREACH_VAL_IND(sleep_retval, name_val) {
...
static void php_var_serialize_nested_data(smart_str *buf, zval *struc, HashTable *ht, uint32_t count, bool incomplete_class, php_serialize_data_t var_hash) /* {{{ */
{
	...
	if (count > 0) {
		...
		ZEND_ARRAY_FOREACH_KEY_VAL_IND(ht, index, key, data) {
	...
	switch (Z_TYPE_P(struc)) {
		...
		case IS_OBJECT: {
				...
				if (ce->__serialize) {
					...
					smart_str_appendl(buf, ":{", 2);
					ZEND_ARRAY_FOREACH_KEY_VAL(Z_ARRVAL(retval), index, key, data) {
						if (!key) {
							php_var_serialize_long(buf, index);
						} else {
							php_var_serialize_string(buf, ZSTR_VAL(key), ZSTR_LEN(key));
...
PHPAPI void php_unserialize_with_options(zval *return_value, const char *buf, const size_t buf_len, HashTable *options, const char* function_name)
{
	...
	if (options != NULL) {
		...
		if(class_hash && Z_TYPE_P(classes) == IS_ARRAY) {
			...
			ZEND_ARRAY_FOREACH_VAL(Z_ARRVAL_P(classes), entry) {
