...
PHP_FUNCTION(next)
{
	...
	switch (Z_TYPE_P(target)) {
		case IS_OBJECT: {
			/* If the target is an instance of Iterator then use it's methods */
			if (instanceof_function(Z_OBJCE_P(target), spl_ce_Iterator)) {
				zend_call_method_with_0_params(target, NULL, NULL, "next", NULL);
				if (USED_RET()) {
					zval entry;
...
PHP_FUNCTION(reset)
{
	...
	switch (Z_TYPE_P(target)) {
		case IS_OBJECT: {
			/* If the target is an instance of Iterator then use it's methods */
			if (instanceof_function(Z_OBJCE_P(target), spl_ce_Iterator)) {
				zend_call_method_with_0_params(target, NULL, NULL, "rewind", NULL);
				if (USED_RET()) {
					zval entry;
					zend_call_method_with_0_params(target, NULL, NULL, "current", &entry);
					if (Z_TYPE(entry) != IS_UNDEF) {
