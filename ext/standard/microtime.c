...
static void _php_gettimeofday(INTERNAL_FUNCTION_PARAMETERS, int mode)
{
	...
	if (get_as_float) {
		RETURN_DOUBLE((double)(sec + nsec / 1000000000.00));
	}

	if (mode) {
		timelib_time_offset *offset;

		offset = timelib_get_time_zone_info(sec, get_timezone_info());

		array_init(return_value);
		add_assoc_long(return_value, "sec", sec);
		add_assoc_long(return_value, "usec", nsec / 1000);
		...
		RETURN_NEW_STR(zend_strpprintf(0, "0.%08ld %lld", nsec / 10, (long long) sec));
	}
}
