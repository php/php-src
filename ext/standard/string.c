...
static zend_string *php_str_rot13(zend_string *str)
{
	...
	if (e - p > 15) {
		...
		const __m128i _M = _mm_set1_epi8('M' + 1);
		const __m128i N_ = _mm_set1_epi8('N' - 1);
		const __m128i _Z = _mm_set1_epi8('Z' + 1);
		...
		do {
			...
			gt = _mm_cmpgt_epi8(in, A_);
			lt = _mm_cmplt_epi8(in, _M);
			cmp = _mm_and_si128(lt, gt);
			if (_mm_movemask_epi8(cmp)) {
				cmp = _mm_and_si128(cmp, add);
				delta = _mm_or_si128(delta, cmp);
			...
			gt = _mm_cmpgt_epi8(in, N_);
			lt = _mm_cmplt_epi8(in, _Z);
			cmp = _mm_and_si128(lt, gt);
			if (_mm_movemask_epi8(cmp)) {
				cmp = _mm_and_si128(cmp, sub);
				delta = _mm_or_si128(delta, cmp);
