...
$extra_arg = 10;
var_dump( gzencode(b"$data", $level, $encoding_mode, $extra_arg) );

echo "\n-- Testing with Unicode string --\n";
var_dump(gzencode($data, $level));

echo "\n-- Testing with incorrect compression level --\n";
$bad_level = 99;
var_dump(gzencode(b"$data", $bad_level));

echo "\n-- Testing with incorrect encoding_mode --\n";
$bad_mode = 99; 
var_dump(gzencode(b"$data", $level, $bad_mode));
...
var_dump(gzencode(b"$data", $testclass));
var_dump(gzencode(b"$data", -1, 99.99));
var_dump(gzencode(b"$data", -1, $testclass));
var_dump(gzencode(b"$data", "a very none numeric string\n"));
...
Warning: gzencode() expects parameter 1 to be strictly a binary string, Unicode string given in %s on line %d
NULL
...
Warning: gzencode() expects parameter 1 to be binary string, object given in %s on line %d
NULL
...
Warning: gzencode() expects parameter 2 to be long, Unicode string given in %s on line %d
NULL
===Done===