...
$extra_arg = 10;
var_dump( gzcompress(b"$data", $level, $encoding, $extra_arg) );

echo "\n-- Testing with Unicode string --\n";
var_dump(gzcompress($data, $level));

echo "\n-- Testing with incorrect compression level --\n";
$bad_level = 99;
var_dump(gzcompress(b"$data", $bad_level));

echo "\n-- Testing with invalid encoding --\n";
$data = b'string_val';
$encoding = 99;
var_dump(gzcompress(b"$data", $level, $encoding));
...
Warning: gzcompress() expects parameter 1 to be strictly a binary string, Unicode string given in %s on line %d
NULL
...
Warning: gzcompress() expects parameter 1 to be binary string, object given in %s on line %d
NULL
===Done===