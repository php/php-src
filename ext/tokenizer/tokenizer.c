	...
	if (Z_TYPE_P(kind) == IS_LONG) {
		...
		ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(kind), entry) {
			...
			if (Z_TYPE_P(entry) == IS_LONG) {
				...
				zend_argument_type_error(1, "must only have elements of type string|int, %s given", zend_zval_type_error_name(entry));
				RETURN_THROWS();
			}
		} ZEND_HASH_FOREACH_END();
		RETURN_FALSE;
	} else {
		zend_argument_type_error(1, "must be of type string|int|array, %s given", zend_zval_type_error_name(kind));
		RETURN_THROWS();
	}
}
