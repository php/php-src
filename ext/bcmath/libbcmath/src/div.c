...
static inline void bc_standard_div(
	 ...
	 * n1_high = n2_high * B
	 * And n1_low is also present only in the numerator, but since it is a negative number, use the minimum value, i.e. 0.
	 * n1_low = 0
	 *
	 ...
	 * MAX_E < 10^(b - x)
	 *
	 * Therefore, found that if the number of digits in exponent B and n2_high are equal, the error will be less
