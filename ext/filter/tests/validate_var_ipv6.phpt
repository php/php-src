...
foreach ($ipv6_test as $ip => $exp) {
	try {
    $out = validate_var($ip, FILTER_VALIDATE_IP, FILTER_FLAG_IPV6);
	} catch (Exception $e) {
		var_dump($e->getMessage());
	}
	var_dump($out); //zif_validate_var() returns FALSE, but $out became TURE because exception prevents return value assignment to $out.
    $out = ($out === $ip);
    if ($exp !== $out) {
