...
static int php_filter_check_options_array(zval *options) /* {{{ */
{
	...
	const char *opts[] = {"min_range", "max_range", "min_byte", "max_byte", "encoding"
	...
	ZEND_HASH_FOREACH_KEY_VAL(HASH_OF(options), idx, key, val) {
		...
		if (!found) {
			php_error_docref(NULL, E_WARNING,
							 "Invalid spec: found key(%s)", ZSTR_VAL(key));
		}
	} ZEND_HASH_FOREACH_END();
...
static int php_filter_check_definition(zval *definitions) /* {{{ */
{
	...
	ZEND_HASH_FOREACH_KEY_VAL(HASH_OF(definitions), idx, key, val) {
		if (Z_TYPE_P(val) == IS_ARRAY) {
			...
			if (key) {
				php_error_docref(NULL, E_WARNING,
								 "Invalid option value. Option should be array or long (key=%s)", key);
			} else {
				php_error_docref(NULL, E_WARNING,
								 "Invalid option value. Option should be array or long (key=" ZEND_LONG_FMT ")", idx);
			}
		}
	} ZEND_HASH_FOREACH_END();
