...
static void php_zval_filter(zval **value, php_int_t filter, php_int_t flags, zval *options, char* charset, zend_bool copy TSRMLS_DC) /* {{{ */
{
	...
	if (
		...
		(!(flags & FILTER_NULL_ON_FAILURE) && Z_TYPE_PP(value) == IS_BOOL && Z_IVAL_PP(value) == 0)) &&
...
static void php_filter_array_handler(zval *input, zval **op, zval *return_value, zend_bool add_empty TSRMLS_DC) /* {{{ */
{
	...
	} else if (Z_TYPE_PP(op) == IS_INT) {
		zval_dtor(return_value);
		MAKE_COPY_ZVAL(&input, return_value);
		php_filter_call(&return_value, Z_IVAL_PP(op), NULL, 0, FILTER_REQUIRE_ARRAY TSRMLS_CC);
...
PHP_FUNCTION(filter_input)
{
	...
	if (!input || !HASH_OF(input) || zend_hash_find(HASH_OF(input), var, var_len + 1, (void **)&tmp) != SUCCESS) {
		...
		zval **option, **opt, **def;
		if (filter_args) {
			if (Z_TYPE_PP(filter_args) == IS_INT) {
				filter_flags = Z_IVAL_PP(filter_args);
...
PHP_FUNCTION(filter_input_array)
{
	...
	if (op
		...
		&& (Z_TYPE_PP(op) == IS_INT && !PHP_FILTER_ID_EXISTS(Z_IVAL_PP(op)))
		) {
		RETURN_FALSE;
	}
	...
	if (!array_input || !HASH_OF(array_input)) {
		...
		zval **option;
		if (op) {
			if (Z_TYPE_PP(op) == IS_INT) {
				filter_flags = Z_IVAL_PP(op);
