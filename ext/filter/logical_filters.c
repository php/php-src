...
void php_filter_validate_url(PHP_INPUT_FILTER_PARAM_DECL) /* {{{ */
{
	...
	void *uri = php_uri_parse(uri_handler, Z_STR_P(value));
	if (uri == NULL) {
		RETURN_VALIDATION_FAILED
	}

	zval scheme;
	result = php_uri_get_scheme(uri_handler, uri, &scheme);
	if (result == FAILURE) {
	...
	zval host;
	result = php_uri_get_host(uri_handler, uri, &host);
	if (result == FAILURE) {
	...
	zval path;
	result = php_uri_get_path(uri_handler, uri, &path);
	if (result == FAILURE) {
		php_uri_free(uri_handler, uri);
		zval_ptr_dtor(&scheme);
		zval_ptr_dtor(&host);
	...
	zval query;
	result = php_uri_get_query(uri_handler, uri, &query);
	if (result == FAILURE) {
		php_uri_free(uri_handler, uri);
		zval_ptr_dtor(&scheme);
		zval_ptr_dtor(&host);
	...
	zval user;
	result = php_uri_get_user(uri_handler, uri, &user);
	if (result == FAILURE) {
		php_uri_free(uri_handler, uri);
		RETURN_VALIDATION_FAILED
	}
	...
	zval password;
	result = php_uri_get_password(uri_handler, uri, &password);
	if (result == FAILURE) {
		php_uri_free(uri_handler, uri);
		zval_ptr_dtor(&user);
		RETURN_VALIDATION_FAILED
