...
static void reflection_free_objects_storage(zend_object *object) /* {{{ */
{
	...
	if (intern->ptr) {
		...
		case REF_TYPE_TYPE:
			...
			if (typ_reference->type == REF_INFO_ARG) {
				_free_function(typ_reference->scope.fptr);
			}
			efree(intern->ptr);
			break;
...
ZEND_METHOD(reflection_type, allowsNull)
{
	...
	switch (param->type) {
		case REF_INFO_ARG:
			RETVAL_BOOL(ZEND_TYPE_ALLOW_NULL(param->info.arg->type));
			break;
		case REF_INFO_PROP:
			RETVAL_BOOL(ZEND_TYPE_ALLOW_NULL(param->info.prop->type));
			break;
	}
}
/* }}} */
...
ZEND_METHOD(reflection_type, isBuiltin)
{
	...
	switch (param->type) {
		case REF_INFO_ARG:
			RETVAL_BOOL(ZEND_TYPE_IS_CODE(param->info.arg->type));
			break;
		case REF_INFO_PROP:
			RETVAL_BOOL(ZEND_TYPE_IS_CODE(param->info.prop->type));
			break;
	}
}
/* }}} */
...
static zend_string *reflection_type_name(type_reference *param) {
	...
	if (ZEND_TYPE_IS_CLASS(type)) {
		...
		char *name = zend_get_type_by_const(ZEND_TYPE_CODE(type));
...
ZEND_METHOD(reflection_property, getValue)
{
	...
	if (ref->prop.flags & ZEND_ACC_STATIC) {
		...
		member_p = zend_std_get_static_property(ref->ce, ref->unmangled_name, BP_VAR_R, &dummy);
		EG(fake_scope) = old_scope;
...
ZEND_METHOD(reflection_property, setValue)
{
	...
	if (ref->prop.flags & ZEND_ACC_STATIC) {
		...
		old_scope = EG(fake_scope);
		EG(fake_scope) = ref->ce;
		variable_ptr = zend_std_get_static_property(ref->ce, ref->unmangled_name, BP_VAR_W, &dummy);
		EG(fake_scope) = old_scope;
