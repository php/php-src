...
/* Struct for class constants */
typedef struct _class_constant_reference {
	zend_class_entry *ce;
...
static void _class_constant_string(string *str, zend_class_constant_info *const_info, char *indent);
...
static void _class_string(string *str, zend_class_entry *ce, zval *obj, char *indent)
{
	...
	/* Constants */
	if (&ce->constants_info) {
		string_printf(str, "\n");
		count = zend_hash_num_elements(&ce->constants_info);
		string_printf(str, "%s  - Constants [%d] {\n", indent, count);
		if (count > 0) {
			zend_class_constant_info *const_info;

			ZEND_HASH_FOREACH_PTR(&ce->constants_info, const_info) {
				_class_constant_string(str, const_info, indent);
...
/* {{{ _class_constant_string */
static void _class_constant_string(string *str, zend_class_constant_info *const_info, char *indent)
{
	char *type = zend_zval_type_name(OBJ_CONST_NUM(const_info->ce, const_info->offset));
	char *visibility = NULL;
	zend_string *value_str = zval_get_string(OBJ_CONST_NUM(const_info->ce, const_info->offset));

	switch (const_info->flags & ZEND_ACC_PPP_MASK) {
	...
	string_printf(str, "%s    Constant [ %s %s %s ] { %s }\n",
			indent, visibility, type, ZSTR_VAL(const_info->name), ZSTR_VAL(value_str));
...
static void reflection_class_constant_factory(zend_class_entry *ce, zend_class_constant_info *const_info, zval *object)
{
	reflection_object *intern;
	zval name;
	zval classname;
	class_constant_reference *reference;

	ZVAL_STRINGL(&name, ZSTR_VAL(const_info->name), ZSTR_LEN(const_info->name));
	ZVAL_STR_COPY(&classname, ce->name);

	reflection_instantiate(reflection_class_constant_ptr, object);
	intern = Z_REFLECTION_P(object);
	reference = (class_constant_reference*) emalloc(sizeof(class_constant_reference));
	reference->ce = const_info->ce;
	reference->const_info = *const_info;
	intern->ptr = reference;
	intern->ref_type = REF_TYPE_CLASS_CONSTANT;
	intern->ce = const_info->ce;
	intern->ignore_visibility = 0;
	reflection_update_property(object, "name", &name);
	reflection_update_property(object, "class", &classname);
}
...
ZEND_METHOD(reflection_class, hasConstant)
{
	...
	GET_REFLECTION_OBJECT_PTR(ce);
	if (zend_hash_exists(&ce->constants_info, name)) {
		RETURN_TRUE;
	} else {
		RETURN_FALSE;
	}
...
ZEND_METHOD(reflection_class, getConstants)
{
	...
	GET_REFLECTION_OBJECT_PTR(ce);
	array_init(return_value);
	ZEND_HASH_FOREACH_STR_KEY_PTR(&ce->constants_info, str, const_info) {
		val = OBJ_CONST_NUM(const_info->ce, const_info->offset);
		Z_TRY_ADDREF_P(val);
		zend_hash_add(HASH_OF(return_value), str, val);
	} ZEND_HASH_FOREACH_END();
}
/* }}} */
...
ZEND_METHOD(reflection_class, getReflectionConstants)
{
	...
	ZEND_HASH_FOREACH_PTR(&ce->constants_info, const_info) {
		zval class_const;
		reflection_class_constant_factory(ce, const_info, &class_const);
...
ZEND_METHOD(reflection_class, getConstant)
{
	...
	if ((const_info = zend_hash_find_ptr(&ce->constants_info, name)) == NULL) {
		RETURN_FALSE;
	}
	if ((value = OBJ_CONST_NUM(const_info->ce, const_info->offset)) == NULL) {
		RETURN_FALSE;
	}
...
ZEND_METHOD(reflection_class, getReflectionConstant)
{
	...
	reflection_class_constant_factory(ce, const_info, return_value);
}
/* }}} */
...
ZEND_METHOD(reflection_class_constant, __construct)
{
	...
	reflection_object *intern;
	zend_class_entry *ce;
	zend_class_constant_info *const_info = NULL;
	class_constant_reference *reference;
	...
	if ((const_info = zend_hash_find_ptr(&ce->constants_info, constname)) == NULL) {
...
ZEND_METHOD(reflection_class_constant, __toString)
{
	...
	GET_REFLECTION_OBJECT_PTR(ref);
	string_init(&str);
	_class_constant_string(&str, &ref->const_info, "");
	RETURN_NEW_STR(str.buf);
}
/* }}} */
...
static void _class_constant_check_flag(INTERNAL_FUNCTION_PARAMETERS, int mask) /* {{{ */
{
	...
	GET_REFLECTION_OBJECT_PTR(ref);
	RETURN_BOOL(ref->const_info.flags & mask);
...
ZEND_METHOD(reflection_class_constant, getModifiers)
{
	...
	RETURN_LONG(ref->const_info.flags);
}
/* }}} */
...
ZEND_METHOD(reflection_class_constant, getValue)
{
	...
	ZVAL_DUP(return_value, OBJ_CONST_NUM(ref->const_info.ce, ref->const_info.offset));
...
ZEND_METHOD(reflection_class_constant, getDeclaringClass)
{
	...
	zend_reflection_class_factory(ref->const_info.ce, return_value);
}
/* }}} */
