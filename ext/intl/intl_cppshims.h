...
template<typename T, typename S = std::size_t, typename std::enable_if<std::is_integral<S>::value && std::is_unsigned<S>::value>::type>
...
T zend_mm_safe_alloc(N num, S len) {
    return static_cast<T>(safe_emalloc(num, len, 0));
}
