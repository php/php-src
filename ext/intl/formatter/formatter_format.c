...
PHP_FUNCTION( numfmt_format )
{
	...
	if(Z_TYPE_PP(number) != IS_DOUBLE && Z_TYPE_PP(number) != IS_INT) {
	...
	switch(type) {
		case FORMAT_TYPE_INT32:
			...
			formatted_len = unum_format(FORMATTER_OBJECT(nfo), (int32_t)Z_IVAL_PP(number), 
		...
		{
			int64_t value = (Z_TYPE_PP(number) == IS_DOUBLE)?(int64_t)Z_DVAL_PP(number):Z_IVAL_PP(number);
