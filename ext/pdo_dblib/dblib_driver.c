...
static int dblib_set_attr(pdo_dbh_t *dbh, zend_long attr, zval *val)
{
	...
	switch(attr) {
		...
		case PDO_DBLIB_ATTR_DATETIME_FORMAT:
			H->datetime_format = zval_get_long(val);
			return 1;
		default:
			return 0;
	}
...
static int dblib_get_attribute(pdo_dbh_t *dbh, zend_long attr, zval *return_value)
{
	...
	switch (attr) {
		...
		case PDO_DBLIB_ATTR_DATETIME_FORMAT:
			ZVAL_LONG(return_value, H->datetime_format);
			break;
...
static int pdo_dblib_handle_factory(pdo_dbh_t *dbh, zval *driver_options)
{
	...
	if (driver_options) {
		...
		H->datetime_format = pdo_attr_lval(driver_options, PDO_DBLIB_ATTR_DATETIME_FORMAT, 0);
	}
