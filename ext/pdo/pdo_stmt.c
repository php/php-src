...
static void pdo_stmt_leave_operation(pdo_stmt_t *stmt)
{
	stmt->in_fetch = false;
	stmt->in_operation = false;
}

static void pdo_stmt_call_operation(pdo_stmt_t *stmt, zif_handler operation, INTERNAL_FUNCTION_PARAMETERS)
{
	...
	bool bailout = false;
	zend_try {
		operation(INTERNAL_FUNCTION_PARAM_PASSTHRU);
	} zend_catch {
		bailout = true;
	} zend_end_try();

	pdo_stmt_leave_operation(stmt);
	if (bailout) {
		zend_bailout();
	}
}
...
static ZEND_NAMED_FUNCTION(pdo_stmt_execute)
{
...
{
	PHP_STMT_GET_OBJ;
	pdo_stmt_call_operation(stmt, pdo_stmt_execute, INTERNAL_FUNCTION_PARAM_PASSTHRU);
}
...
static ZEND_NAMED_FUNCTION(pdo_stmt_fetch)
{
...
{
	PHP_STMT_GET_OBJ;
	pdo_stmt_call_operation(stmt, pdo_stmt_fetch, INTERNAL_FUNCTION_PARAM_PASSTHRU);
}

/* {{{ Fetches the next row and returns it as an object. */
static ZEND_NAMED_FUNCTION(pdo_stmt_fetch_object)
{
	zend_class_entry *ce = NULL;
...
{
	PHP_STMT_GET_OBJ;
	pdo_stmt_call_operation(stmt, pdo_stmt_fetch_object, INTERNAL_FUNCTION_PARAM_PASSTHRU);
}

/* {{{ Returns a data of the specified column in the result set. */
static ZEND_NAMED_FUNCTION(pdo_stmt_fetch_column)
{
	zend_long col_n = 0;
...
{
	PHP_STMT_GET_OBJ;
	pdo_stmt_call_operation(stmt, pdo_stmt_fetch_column, INTERNAL_FUNCTION_PARAM_PASSTHRU);
}
...
static ZEND_NAMED_FUNCTION(pdo_stmt_fetch_all)
{
	zend_long how = PDO_FETCH_USE_DEFAULT;
	zval *arg2 = NULL;
	zend_class_entry *old_ce;
...
{
	PHP_STMT_GET_OBJ;
	pdo_stmt_call_operation(stmt, pdo_stmt_fetch_all, INTERNAL_FUNCTION_PARAM_PASSTHRU);
}
...
static ZEND_NAMED_FUNCTION(pdo_stmt_bind_value)
{
	struct pdo_bound_param_data param;
	zend_long param_type = PDO_PARAM_STR;
	zval *parameter;
...
{
	PHP_STMT_GET_OBJ;
	pdo_stmt_call_operation(stmt, pdo_stmt_bind_value, INTERNAL_FUNCTION_PARAM_PASSTHRU);
}
...
static ZEND_NAMED_FUNCTION(pdo_stmt_bind_param)
{
...
{
	PHP_STMT_GET_OBJ;
	pdo_stmt_call_operation(stmt, pdo_stmt_bind_param, INTERNAL_FUNCTION_PARAM_PASSTHRU);
}
...
static ZEND_NAMED_FUNCTION(pdo_stmt_bind_column)
{
...
{
	PHP_STMT_GET_OBJ;
	pdo_stmt_call_operation(stmt, pdo_stmt_bind_column, INTERNAL_FUNCTION_PARAM_PASSTHRU);
}
...
/* {{{ Set an attribute */
static ZEND_NAMED_FUNCTION(pdo_stmt_set_attribute)
{
	zend_long attr;
	zval *value = NULL;
...
{
	PHP_STMT_GET_OBJ;
	pdo_stmt_call_operation(stmt, pdo_stmt_set_attribute, INTERNAL_FUNCTION_PARAM_PASSTHRU);
}
...
static ZEND_NAMED_FUNCTION(pdo_stmt_get_attribute)
{
	zend_long attr;
...
{
	PHP_STMT_GET_OBJ;
	pdo_stmt_call_operation(stmt, pdo_stmt_get_attribute, INTERNAL_FUNCTION_PARAM_PASSTHRU);
}
...
static ZEND_NAMED_FUNCTION(pdo_stmt_get_column_meta)
{
	zend_long colno;
...
{
	PHP_STMT_GET_OBJ;
	pdo_stmt_call_operation(stmt, pdo_stmt_get_column_meta, INTERNAL_FUNCTION_PARAM_PASSTHRU);
}
...
static ZEND_NAMED_FUNCTION(pdo_stmt_set_fetch_mode)
{
...
PHP_METHOD(PDOStatement, setFetchMode)
{
	...
	pdo_stmt_call_operation(stmt, pdo_stmt_set_fetch_mode, INTERNAL_FUNCTION_PARAM_PASSTHRU);
}
...
static ZEND_NAMED_FUNCTION(pdo_stmt_next_rowset)
{
	ZEND_PARSE_PARAMETERS_NONE();
...
{
	PHP_STMT_GET_OBJ;
	pdo_stmt_call_operation(stmt, pdo_stmt_next_rowset, INTERNAL_FUNCTION_PARAM_PASSTHRU);
}

/* {{{ Closes the cursor, leaving the statement ready for re-execution. */
static ZEND_NAMED_FUNCTION(pdo_stmt_close_cursor)
{
	ZEND_PARSE_PARAMETERS_NONE();
...
{
	PHP_STMT_GET_OBJ;
	pdo_stmt_call_operation(stmt, pdo_stmt_close_cursor, INTERNAL_FUNCTION_PARAM_PASSTHRU);
}

/* {{{ A utility for internals hackers to debug parameter internals */
static ZEND_NAMED_FUNCTION(pdo_stmt_debug_dump_params)
{
	ZEND_PARSE_PARAMETERS_NONE();
