...
<?php
try {
	$a = new PharData(__DIR__ . '/files/bzip2.zip');
	foreach ($a as $entry => $file) {
		echo $file->getContent();
	}
	$a = new Phar(__DIR__ . '/files/bz2_alias.phar.zip');
	var_dump($a->getAlias());
} catch (Exception $e) {
	echo $e->getMessage() . "\n";
}
