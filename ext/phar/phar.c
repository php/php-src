...
ZEND_INI_MH(phar_ini_modify_handler) /* {{{ */
{
	...
	if (entry->name->len == sizeof("phar.readonly")-1) {
		PHAR_G(readonly) = ini;
		if (PHAR_GLOBALS->request_init && GC_TYPE_INFO(&PHAR_GLOBALS->phar_fname_map)) {
...
finish_error:
				...
				GC_TYPE_INFO(&PHAR_GLOBALS->phar_fname_map) = 0;
				zend_hash_destroy(&(PHAR_G(phar_alias_map)));
				GC_TYPE_INFO(&PHAR_GLOBALS->phar_alias_map) = 0;
...
void phar_destroy_phar_data(phar_archive_data *phar) /* {{{ */
{
	...
	if (GC_TYPE_INFO(&phar->manifest)) {
		...
		GC_TYPE_INFO(&phar->manifest) = 0;
	}

	if (GC_TYPE_INFO(&phar->mounted_dirs)) {
		zend_hash_destroy(&phar->mounted_dirs);
		GC_TYPE_INFO(&phar->mounted_dirs) = 0;
	}

	if (GC_TYPE_INFO(&phar->virtual_dirs)) {
		zend_hash_destroy(&phar->virtual_dirs);
		GC_TYPE_INFO(&phar->virtual_dirs) = 0;
	}
...
PHP_RSHUTDOWN_FUNCTION(phar) /* {{{ */
{
	...
	if (PHAR_GLOBALS->request_init)
	{
		...
		GC_TYPE_INFO(&PHAR_GLOBALS->phar_persist_map) = 0;
		PHAR_GLOBALS->phar_SERVER_mung_list = 0;
