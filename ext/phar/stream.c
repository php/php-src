...
/*			fprintf(stderr, "Fragment:  %s\n", resource->fragment);*/
	...
	if (mode[0] == 'w' || (mode[0] == 'r' && mode[1] == '+')) {
		...
		if (PHAR_GLOBALS->request_init && GC_TYPE_INFO(&PHAR_GLOBALS->phar_fname_map) && NULL == (pphar = zend_hash_str_find_ptr(&(PHAR_GLOBALS->phar_fname_map), arch, arch_len))) {
			pphar = NULL;
		}
				  ...
				  php_stream_statbuf *ssb, php_stream_context *context) /* {{{ */
{
	...
	if (GC_TYPE_INFO(&phar->mounted_dirs) && zend_hash_num_elements(&phar->mounted_dirs)) {
		zend_string *str_key;
