...
";
    ...
    if ($test->sectionNotEmpty('SKIPIF')) {
        ...
        $output = $skipCache->checkSkip($commandLine, $test->getSection('SKIPIF'), $test_skipif, $temp_skipif, $env, $test->sectionNotEmpty('INI'));
...
class SkipCache
{
    ...
    public function checkSkip(string $php, string $code, string $checkFile, string $tempFile, array $env, bool $testHasIniSection): string
    {
        ...
        if ($useSubprocess) {
            ...
            $result = runCodeInLocalSandbox($code);
        }
