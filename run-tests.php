...
function main()
{
    ...
    verify_config();
    write_information($php, $php_cgi, $phpdbg, $php_info, $user_tests, $ini_overwrites, $pass_options, $exts_to_test, $valgrind, $no_file_cache);
...
if (!function_exists("hrtime")) {
    function hrtime(bool $as_num = false): array
    {
        $t = microtime(true);
...
function write_information($php, $php_cgi, $phpdbg, $php_info, $user_tests, $ini_overwrites, $pass_options, $exts_to_test, $valgrind, $no_file_cache): void
{
    // Get info from php
...
function find_files($dir, $is_ext_dir = false, $ignore = false): void
{
...
{
    global $test_results, $failed_tests_file, $result_tests_file, $php, $test_idx, $file_cache, $shuffle, $SHOW_ONLY_GROUPS, $valgrind;
    // Parallel testing
    ...
    if ($workers !== null && count($test_files) > 1 && !$workerID) {
        run_all_tests_parallel(
            ...
            $workers, $test_idx, $test_cnt, $test_results, $failed_tests_file, $result_tests_file,
            $PHP_FAILED_TESTS, $shuffle, $SHOW_ONLY_GROUPS, $valgrind
        );
...
function run_all_tests_parallel(
    $test_files, $env, $redir_tested,
    $workers, $test_idx, $test_results, $failed_tests_file, $result_tests_file,
    $PHP_FAILED_TESTS, $shuffle, $SHOW_ONLY_GROUPS, $valgrind
)
{
...
";
    ...
    if (
        array_key_exists('CGI', $section_text)
